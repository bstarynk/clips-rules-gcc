...
static void NegEntry_RetractBeta (Environment *, struct joinNode *,
				  struct partialMatch *,
				  struct partialMatch *, int);
...
/************************************************************/
void
CL_Network_Retract (Environment * theEnv,
		    struct patternMatch *listOfMatchedPatterns)
{
  ...
  while (tempMatch != NULL)
	...
	{
	  CL_PosEntry_RetractAlpha (theEnv, tempMatch->theMatch,
				    NETWORK_RETRACT);
	}

      if (tempMatch->theMatch->blockList != NULL)
	{
	  NegEntry_RetractAlpha (theEnv, tempMatch->theMatch,
				 NETWORK_RETRACT);
	}
...
/*************************/
void
CL_PosEntry_RetractAlpha (Environment * theEnv,
			  struct partialMatch *alphaMatch, int operation)
{
  ...
  while (betaMatch != NULL)
	...
	{
	  CL_PosEntry_RetractBeta (theEnv, betaMatch, betaMatch->children,
				   operation);
	}
	...
	{
	  CL_RemoveActivation (theEnv,
			       (struct activation *) betaMatch->marker, true,
			       true);
	}
...
/*************************/
static void
NegEntry_RetractAlpha (Environment * theEnv,
		       struct partialMatch *alphaMatch, int operation)
{
  ...
  while (betaMatch != NULL)
    {
	  ...
	  (!joinPtr->patternIsExists) && (!joinPtr->joinFromTheRight))
	{
      ...
      NegEntry_RetractBeta (theEnv, joinPtr, alphaMatch, betaMatch,
			    operation);
      betaMatch = alphaMatch->blockList;
    }
}
...
/************************/
static void
NegEntry_RetractBeta (Environment * theEnv,
		      struct joinNode *joinPtr,
		      struct partialMatch *alphaMatch,
		      struct partialMatch *betaMatch, int operation)
{
   /*======================================================*/
  ...
  if (FindNextConflictingMatch
      (theEnv, betaMatch, alphaMatch->nextInMemory, joinPtr, alphaMatch,
       operation))
    {
      return;
    }
  ...
  else if (joinPtr->firstJoin
	   && (joinPtr->patternIsNegated || joinPtr->joinFromTheRight)
	   && (!joinPtr->patternIsExists))
    {
      if (joinPtr->secondaryNetworkTest != NULL)
	{
	  if (CL_EvaluateSecondaryNetworkTest (theEnv, betaMatch, joinPtr) ==
	      false)
	    {
	      return;
	    }
  ...
  if ((operation == NETWORK_RETRACT)
      && CL_PartialMatchWillBeDeleted (theEnv, betaMatch))
    {
      return;
    }
...
/************************/
void
CL_PosEntry_RetractBeta (Environment * theEnv,
			 ...
			 struct partialMatch *betaMatch, int operation)
{
  ...
  while (betaMatch != NULL)
	...
	if ((((struct joinNode *) betaMatch->owner)->ruleToActivate !=
	     NULL) ? (betaMatch->marker != NULL) : false)
	{
	  CL_RemoveActivation (theEnv,
			       (struct activation *) betaMatch->marker, true,
			       true);
      ...
      if (betaMatch->rhsMemory)
	{
	  CL_UnlinkNonLeftLineage (theEnv,
				   (struct joinNode *) betaMatch->owner,
				   betaMatch, RHS);
	}
      else
	{
      ...
      if (betaMatch->dependents != NULL)
	CL_RemoveLogicalSupport (theEnv, betaMatch);
      CL_ReturnPartialMatch (theEnv, betaMatch);

      if (tempMatch == parentMatch)
	return;
      betaMatch = tempMatch;
    }
}
...
/******************************************************************/
static bool
FindNextConflictingMatch (Environment * theEnv,
			  struct partialMatch *theBind,
			  struct partialMatch *possibleConflicts,
			  struct joinNode *theJoin,
			  struct partialMatch *skipMatch, int operation)
{
  ...
  for (;
	...
	{			/* Do Nothing */
      ...
      else if ((operation == NETWORK_RETRACT)
	       && CL_PartialMatchWillBeDeleted (theEnv, possibleConflicts))
	{			/* Do Nothing */
	}
      ...
      else
	{
	  ...
	  result =
	    CL_EvaluateJoinExpression (theEnv, theJoin->networkTest, theJoin);
...
/***********************************************************/
static bool
PartialMatchDefunct (Environment * theEnv, struct partialMatch *thePM)
{
  unsigned short i;
  ...
  for (i = 0; i < thePM->bcount; i++)
    {
      if (thePM->binds[i].gm.theMatch == NULL)
	continue;
...
/*****************************************************************/
bool
CL_PartialMatchWillBeDeleted (Environment * theEnv,
...
/***************************************************/
void
CL_DeletePartial_Matches (Environment * theEnv,
  ...
  while (listOfPMs != NULL)
    {
      ...
      if (listOfPMs->dependents != NULL)
	CL_RemoveLogicalSupport (theEnv, listOfPMs);
...
/**************************************************************/
void
CL_ReturnPartialMatch (Environment * theEnv, struct partialMatch *waste)
{
   /*==============================================*/
  ...
  if (waste->dependents != NULL)
    RemovePM_Dependencies (theEnv, waste);
...
/***************************************************************/
void
CL_DestroyPartialMatch (Environment * theEnv, struct partialMatch *waste)
{
   /*======================================================*/
  /* If we're dealing with an alpha memory partial match, */
  /* then return the multifield markers associated with   */
  ...
  if (waste->dependents != NULL)
    DestroyPM_Dependencies (theEnv, waste);
...
/******************************************************/
static void
ReturnMarkers (Environment * theEnv, struct multifieldMarker *waste)
{
...
/*************************************************************/
void
CL_FlushGarbagePartial_Matches (Environment * theEnv)
{
  ...
  while (EngineData (theEnv)->GarbageAlpha_Matches != NULL)
    {
      ...
      rtn_struct (theEnv, alphaMatch,
		  EngineData (theEnv)->GarbageAlpha_Matches);
  ...
  while (EngineData (theEnv)->GarbagePartial_Matches != NULL)
    {
      ...
      CL_ReturnPartialMatch (theEnv,
			     EngineData (theEnv)->GarbagePartial_Matches);
