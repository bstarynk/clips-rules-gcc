...
/**********************************************/
void
Initialize_BsaveData (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, BSAVE_DATA, sizeof (struct bsaveData),
			      Deallocate_BsaveData);
}
...
/************************************************/
static void
Deallocate_BsaveData (Environment * theEnv)
{
...
/**************************************/
void
CL_BsaveCommand (Environment * theEnv,
...
/****************************/
bool
CL_Bsave (Environment * theEnv, const char *fileName)
{
  FILE *fp;
  ...
  CL_Gen_Write (&ExpressionData (theEnv)->ExpressionCount,
...
/*********************************************/
static void
InitializeFunctionNeededFlags (Environment * theEnv)
{
  struct functionDefinition *functionList;

  for (functionList = CL_GetFunctionList (theEnv);
       functionList != NULL; functionList = functionList->next)
    {
...
/**********************************************************/
static void
FindNeededItems (Environment * theEnv)
{
  struct BinaryItem *biPtr;
  ...
  for (biPtr = CL_BsaveData (theEnv)->ListOfBinaryItems;
       biPtr != NULL; biPtr = biPtr->next)
    {
      if (biPtr->findFunction != NULL)
	(*biPtr->findFunction) (theEnv);
    }
}
...
/****************************************************/
static void
CL_WriteNeededFunctions (Environment * theEnv, FILE * fp)
{
  ...
  for (functionList = CL_GetFunctionList (theEnv);
      ...
      if (functionList->neededFunction)
	{
	  ...
	  CL_Gen_Write ((void *) functionList->callFunctionName->contents,
			length, fp);
	}
    }
}
...
/*********************************************/
static size_t
FunctionBinarySize (Environment * theEnv)
{
  size_t size = 0;
  struct functionDefinition *functionList;
...
/***************************************************/
void
CL_Save_BloadCount (Environment * theEnv, unsigned long cnt)
{
  BLOADCNTSV *tmp, *prv;
...
/**************************************************/
void
Restore_BloadCount (Environment * theEnv, unsigned long *cnt)
{
  BLOADCNTSV *tmp;

  *cnt = CL_BsaveData (theEnv)->CL_BloadCount_SaveTop->val;
  tmp = CL_BsaveData (theEnv)->CL_BloadCount_SaveTop;
  CL_BsaveData (theEnv)->CL_BloadCount_SaveTop =
    CL_BsaveData (theEnv)->CL_BloadCount_SaveTop->nxt;
...
/**********************************************/
void
CL_MarkNeededItems (Environment * theEnv, struct expr *testPtr)
{
  while (testPtr != NULL)
    {
      switch (testPtr->type)
...
/******************************************************/
static void
CL_WriteBinaryHeader (Environment * theEnv, FILE * fp)
{
  CL_Gen_Write ((void *) CL_BloadData (theEnv)->BinaryPrefixID,
		strlen (CL_BloadData (theEnv)->BinaryPrefixID) + 1, fp);
  CL_Gen_Write ((void *) CL_BloadData (theEnv)->BinaryVersionID,
		strlen (CL_BloadData (theEnv)->BinaryVersionID) + 1, fp);
  CL_Gen_Write ((void *) CL_BloadData (theEnv)->BinarySizes,
		strlen (CL_BloadData (theEnv)->BinarySizes) + 1, fp);
}
...
/******************************************************/
static void
CL_WriteBinaryFooter (Environment * theEnv, FILE * fp)
{
  char footerBuffer[CONSTRUCT_HEADER_SIZE];

  CL_genstrncpy (footerBuffer, CL_BloadData (theEnv)->BinaryPrefixID,
		 CONSTRUCT_HEADER_SIZE);
...
/**********************************************************/
bool
CL_AddBinaryItem (Environment * theEnv,
		  const char *name,
		  int priority,
