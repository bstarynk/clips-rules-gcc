...
/***********************************************/
void
CL_ConstraintsToCode (Environment * theEnv,
		      const char *fileName,
		      const char *pathName,
		      char *fileNameBuffer,
		      unsigned fileID,
		      FILE * headerFP, unsigned imageID, unsigned maxIndices)
{
  ...
  for (i = 0; i < SIZE_CONSTRAINT_HASH; i++)
	   ...
	   tmpPtr != NULL; tmpPtr = tmpPtr->next)
	{
	  ...
	  fprintf (fp, "{%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", tmpPtr->anyAllowed, tmpPtr->symbolsAllowed, tmpPtr->stringsAllowed, tmpPtr->floatsAllowed, tmpPtr->integersAllowed, tmpPtr->instanceNamesAllowed, tmpPtr->instanceAddressesAllowed, tmpPtr->externalAddressesAllowed, tmpPtr->factAddressesAllowed, 0,	/* void allowed */
		   ...
		   tmpPtr->singlefieldsAllowed, tmpPtr->installed);

	  fprintf (fp, ",0,");	/* bsaveIndex */

	  CL_PrintHashedExpressionReference (theEnv, fp, tmpPtr->classList,
					     imageID, maxIndices);
	  fprintf (fp, ",");
	  CL_PrintHashedExpressionReference (theEnv, fp,
					     tmpPtr->restrictionList, imageID,
					     maxIndices);
	  fprintf (fp, ",");
	  CL_PrintHashedExpressionReference (theEnv, fp, tmpPtr->minValue,
					     imageID, maxIndices);
	  fprintf (fp, ",");
	  CL_PrintHashedExpressionReference (theEnv, fp, tmpPtr->maxValue,
					     imageID, maxIndices);
	  fprintf (fp, ",");
	  CL_PrintHashedExpressionReference (theEnv, fp, tmpPtr->minFields,
					     imageID, maxIndices);
	  fprintf (fp, ",");
	  CL_PrintHashedExpressionReference (theEnv, fp, tmpPtr->maxFields,
					     imageID, maxIndices);
...
/**********************************************************/
void
CL_PrintConstraintReference (Environment * theEnv,
			     FILE * fp,
