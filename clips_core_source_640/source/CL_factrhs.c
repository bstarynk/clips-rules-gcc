...
/**********************************************************************/
struct expr *
CL_BuildRHS_Assert (Environment * theEnv,
		    const char *logicalName,
		    struct token *theToken,
		    bool *error,
		    bool atLeastOne,
		    bool readFirstParen, const char *whereParsed)
{
  ...
  *error = false;
  ...
  while ((nextOne = CL_GetRHSPattern (theEnv, logicalName, theToken,
				      ...
				      true, RIGHT_PARENTHESIS_TOKEN)) != NULL)
    {
      ...
      stub =
	CL_GenConstant (theEnv, FCALL, CL_FindFunction (theEnv, "assert"));
  ...
  if (assertList->nextArg != NULL)
    {
      stub =
	CL_GenConstant (theEnv, FCALL, CL_FindFunction (theEnv, "progn"));
...
/***************************************************************/
struct expr *
CL_GetRHSPattern (Environment * theEnv,
		  const char *readSource,
		  struct token *tempToken,
		  bool *error,
		  bool constantsOnly,
		  bool readFirstParen,
		  bool checkFirstParen, TokenType endType)
{
  ...
  *error = false;
  ...
  if (CL_ReservedPatternSymbol (theEnv, templateName->contents, NULL))
    {
      CL_ReservedPatternSymbolErrorMsg (theEnv, templateName->contents,
					"a relation name");
      *error = true;
      return NULL;
    }
  ...
  theDeftemplate = (Deftemplate *)
    CL_FindImportedConstruct (theEnv, "deftemplate", NULL,
			      templateName->contents, &count, true, NULL);
  ...
  if (theDeftemplate == NULL)
	   ...
	   templateName->contents))
	{
	  CL_ImportExportConflictMessage (theEnv, "implied deftemplate",
					  templateName->contents, NULL, NULL);
	  *error = true;
	  return NULL;
	}
#endif
	...
	{
	  theDeftemplate =
	    CL_CreateImpliedDeftemplate (theEnv, templateName, true);
	}
    }
#else
    {
...
/********************************************************************/
struct expr *
Get_AssertArgument (Environment * theEnv,
		    const char *logicalName,
		    struct token *theToken,
		    bool *error,
		    TokenType endType, bool constantsOnly, bool *printError)
{
#if ! RUN_TIME
  ...
  *printError = true;
  ...
  if ((theToken->tknType == SYMBOL_TOKEN) ?
	...
	nextField = CL_Function1Parse (theEnv, logicalName);
      else
	nextField = CL_Function0Parse (theEnv, logicalName);
      if (nextField == NULL)
#endif
	{
	  *printError = false;
...
/****************************************************/
Fact *
CL_StringToFact (Environment * theEnv, const char *str)
{
  struct token theToken;
  Fact *factPtr;
  ...
  for (tempPtr = assertArgs->nextArg; tempPtr != NULL;
...
/*********************************************************/
static void
NoSuchTemplateError (Environment * theEnv, const char *templateName)
{
  ...
  CL_WriteString (theEnv, STDERR,
		  "' cannot be created with binary load in effect.\n");
}
