...
/**********************************************/
void
CL_WriteNeededAtomicValues (Environment * theEnv, FILE * fp)
{
...
/********************************************************/
void
CL_InitAtomicValueNeededFlags (Environment * theEnv)
{
  unsigned long i;
...
/*****************************************************************/
void
CL_WriteNeededSymbols (Environment * theEnv, FILE * fp)
{
  unsigned long i;
  size_t length;
  ...
  for (i = 0; i < SYMBOL_HASH_SIZE; i++)
	   ...
	   symbolPtr != NULL; symbolPtr = symbolPtr->next)
	{
	  if (symbolPtr->neededSymbol)
	    {
	      CL_Gen_Write (&symbolPtr->header.type, sizeof (unsigned short),
			    fp);
	    }
	}
    }
...
/*****************************************************************/
void
CL_WriteNeededFloats (Environment * theEnv, FILE * fp)
{
  int i;
...
/******************************************************************/
void
CL_WriteNeededIntegers (Environment * theEnv, FILE * fp)
{
  int i;
...
/*****************************************************************/
static void
CL_WriteNeededBitMaps (Environment * theEnv, FILE * fp)
{
  int i;
	  ...
	  if (bitMapPtr->neededBitMap)
	    {
	      numberOfUsedBitMaps++;
	      size +=
		(unsigned long) (bitMapPtr->size + sizeof (unsigned short));
	    }
	}
    }
	  ...
	  if (bitMapPtr->neededBitMap)
	    {
	      ...
	      CL_Gen_Write ((void *) bitMapPtr->contents, bitMapPtr->size,
			    fp);
	    }
	}
    }
...
/*********************************************/
void
CL_ReadNeededAtomicValues (Environment * theEnv)
{
...
/*******************************************/
void
CL_ReadNeededSymbols (Environment * theEnv)
{
  ...
  CL_GenReadBinary (theEnv, &SymbolData (theEnv)->NumberOfSymbols,
		    sizeof (long));
  ...
  types =
    (unsigned short *) CL_gm2 (theEnv,
			       sizeof (unsigned short) *
			       SymbolData (theEnv)->NumberOfSymbols);
  CL_GenReadBinary (theEnv, types,
		    sizeof (unsigned short) *
		    SymbolData (theEnv)->NumberOfSymbols);
  ...
  for (i = 0; i < SymbolData (theEnv)->NumberOfSymbols; i++)
	...
	{
	  SymbolData (theEnv)->SymbolArray[i] =
	    CL_CreateSymbol (theEnv, namePtr);
	}
      else if (types[i] == STRING_TYPE)
	{
	  SymbolData (theEnv)->SymbolArray[i] =
	    CL_CreateString (theEnv, namePtr);
	}
      else
	{
	  SymbolData (theEnv)->SymbolArray[i] =
	    CL_Create_InstanceName (theEnv, namePtr);
	}
  ...
  CL_rm (theEnv, types,
	 sizeof (unsigned short) * SymbolData (theEnv)->NumberOfSymbols);
...
/*****************************************/
void
CL_ReadNeededFloats (Environment * theEnv)
{
  double *floatValues;
  unsigned long i;
  ...
  CL_GenReadBinary (theEnv, &SymbolData (theEnv)->NumberOfFloats,
		    sizeof (long));
  ...
  floatValues =
    (double *) CL_gm2 (theEnv,
		       sizeof (double) * SymbolData (theEnv)->NumberOfFloats);
  CL_GenReadBinary (theEnv, floatValues,
		    (sizeof (double) * SymbolData (theEnv)->NumberOfFloats));
  ...
  for (i = 0; i < SymbolData (theEnv)->NumberOfFloats; i++)
    {
      SymbolData (theEnv)->FloatArray[i] =
	CL_CreateFloat (theEnv, floatValues[i]);
    }
...
/*********************************************/
void
CL_ReadNeededIntegers (Environment * theEnv)
{
  ...
  CL_GenReadBinary (theEnv, &SymbolData (theEnv)->NumberOfIntegers,
  ...
  integerValues =
    (long long *) CL_gm2 (theEnv,
			  (sizeof (long long) *
			   SymbolData (theEnv)->NumberOfIntegers));
  CL_GenReadBinary (theEnv, integerValues,
		    (sizeof (long long) *
		     SymbolData (theEnv)->NumberOfIntegers));
  ...
  for (i = 0; i < SymbolData (theEnv)->NumberOfIntegers; i++)
    {
      SymbolData (theEnv)->IntegerArray[i] =
	CL_CreateInteger (theEnv, integerValues[i]);
    }
  ...
  CL_rm (theEnv, integerValues,
	 (sizeof (long long) * SymbolData (theEnv)->NumberOfIntegers));
}
...
/*******************************************/
static void
ReadNeededBitMaps (Environment * theEnv)
{
  ...
  CL_GenReadBinary (theEnv, &SymbolData (theEnv)->NumberOfBitMaps,
		    sizeof (long));
  CL_GenReadBinary (theEnv, &space, sizeof (unsigned long));
  ...
  for (i = 0; i < SymbolData (theEnv)->NumberOfBitMaps; i++)
    {
      ...
      SymbolData (theEnv)->BitMapArray[i] =
	(CLIPSBitMap *) CL_AddBitMap (theEnv,
				      bitMapPtr + sizeof (unsigned short),
				      *tempSize);
...
/**********************************************************/
void
CL_FreeAtomicValueStorage (Environment * theEnv)
{
