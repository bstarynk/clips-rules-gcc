...
const char *CL_GetLogicalName(
  ...
  const char *defaultLogicalName)
  {
   ...
   if (CVIsType(&theArg,LEXEME_BITS) ||
     ...
     {
      logicalName = CL_CreateSymbol(theEnv,CL_FloatToString(theEnv,theArg.floatValue->contents))->contents;
     }
   else if (CVIsType(&theArg,INTEGER_BIT))
     {
      logicalName = CL_CreateSymbol(theEnv,CL_LongIntegerToString(theEnv,theArg.integerValue->contents))->contents;
     }
   else
     { logicalName = NULL; }
...
Defmodule *CL_GetModuleName(
   ...
   *error = false;
   ...
   if ((theModule = CL_FindDefmodule(theEnv,returnValue.lexemeValue->contents)) == NULL)
     {
      if (strcmp("*",returnValue.lexemeValue->contents) != 0)
        {
         CL_ExpectedTypeError1(theEnv,functionName,whichArgument,"'defmodule name'");
         *error = true;
        }
      return NULL;
     }
...
/****************************************************************/
const char *CL_GetConstructName(
  UDFContext *context,
  const char *functionName,
...
void CL_ExpectedCountError(
  ...
  unsigned int expectedNumber)
  {
   ...
   if (countRelation == EXACTLY)
     ...
     { CL_WriteString(theEnv,STDERR," generated an illegal argument check for "); }
...
/***************************************************/
void *CL_GetFactOrInstanceArgument(
  UDFContext *context,
  ...
  UDFValue *item)
  {
   ...
   if (CVIsType(item,FACT_ADDRESS_BIT))
   ...
   else if (CVIsType(item,INSTANCE_ADDRESS_BIT))
     {
      if (item->instanceValue->garbage)
        {
         CL_CantFindItemErrorMessage(theEnv,"instance",item->instanceValue->name->contents,false);
         return NULL;
        }
   ...
   /*==================================================*/
     ...
     {
      if ((ptr = (void *) CL_FindIndexedFact(theEnv,item->integerValue->contents)) == NULL)
        {
         char tempBuffer[20];
   ...
   /*================================================*/
     ...
     {
      if ((ptr = (void *) CL_FindInstanceBySymbol(theEnv,item->lexemeValue)) == NULL)
        {
         CL_CantFindItemErrorMessage(theEnv,"instance",item->lexemeValue->contents,false);
        }
      return ptr;
     }
#endif
   ...
   CL_ExpectedTypeError2(theEnv,CL_UDFContextFunctionName(context),thePosition);
   return NULL;
  }
...
/****************************************************/
void CL_IllegalLogicalNameMessage(
  Environment *theEnv,
