...
static struct expr *CL_AssertParse (Environment *, struct expr *,
				    const char *);
#if DEBUGGING_FUNCTIONS
static long long Get_FactsArgument (UDFContext *);
#endif
static struct expr *Standard_LoadFact (Environment *, const char *,
				       struct token *);
static Deftemplate **Get_Save_Facts_DeftemplateNames (Environment *,
						      struct expr *, int,
						      unsigned int *, bool *);
...
/***************************************/
void
CL_FactCommandDefinitions (Environment * theEnv)
{
#if ! RUN_TIME
#if DEBUGGING_FUNCTIONS
  CL_AddUDF (theEnv, "facts", "v", 0, 4, "l;*", CL_FactsCommand,
	     "CL_FactsCommand", NULL);
#endif

  CL_AddUDF (theEnv, "assert", "bf", 0, UNBOUNDED, NULL, CL_AssertCommand,
	     "CL_AssertCommand", NULL);
  CL_AddUDF (theEnv, "retract", "v", 1, UNBOUNDED, "fly", CL_RetractCommand,
	     "CL_RetractCommand", NULL);
  CL_AddUDF (theEnv, "assert-string", "bf", 1, 1, "s",
	     CL_AssertStringFunction, "CL_AssertStringFunction", NULL);
  CL_AddUDF (theEnv, "str-assert", "bf", 1, 1, "s", CL_AssertStringFunction,
	     "CL_AssertStringFunction", NULL);

  CL_AddUDF (theEnv, "get-fact-duplication", "b", 0, 0, NULL,
	     CL_GetFactDuplicationCommand, "CL_GetFactDuplicationCommand",
	     NULL);
  CL_AddUDF (theEnv, "set-fact-duplication", "b", 1, 1, NULL,
	     CL_SetFactDuplicationCommand, "CL_SetFactDuplicationCommand",
	     NULL);

  CL_AddUDF (theEnv, "save-facts", "b", 1, UNBOUNDED, "y;sy",
	     CL_Save_FactsCommand, "CL_Save_FactsCommand", NULL);
  CL_AddUDF (theEnv, "load-facts", "b", 1, 1, "sy", CL_Load_FactsCommand,
	     "CL_Load_FactsCommand", NULL);
  CL_AddUDF (theEnv, "fact-index", "l", 1, 1, "f", CL_FactIndexFunction,
	     "CL_FactIndexFunction", NULL);
...
/***************************************/
void
CL_AssertCommand (Environment * theEnv,
		  UDFContext * context, UDFValue * returnValue)
{
  ...
  if (theDeftemplate->implied == false)
	...
	{
	  newFact->theProposition.contents[0].multifieldValue =
  ...
  for (theExpression = theExpression->nextArg, i = 0;
       theExpression != NULL; theExpression = theExpression->nextArg, i++)
    {
      /*===================================================*/
      ...
      if ((slotPtr != NULL) ?
	  (slotPtr->multislot == false)
	  && (theValue.header->type == MULTIFIELD_TYPE) : false)
	{
...
/****************************************/
void
CL_RetractCommand (Environment * theEnv,
		   UDFContext * context, UDFValue * returnValue)
{
  long long factIndex;
  Fact *ptr;
  ...
  while (UDFHasNextArgument (context))
    {
	  ...
	  (context, INTEGER_BIT | FACT_ADDRESS_BIT | SYMBOL_BIT, &theArg))
	{
	  return;
	}
	  ...
	  if (factIndex < 0)
	    {
	      CL_UDFInvalidArgumentMessage (context,
					    "fact-address, fact-index, or the symbol *");
	      return;
	    }
...
/***************************************************/
void
CL_SetFactDuplicationCommand (Environment * theEnv,
			      UDFContext * context, UDFValue * returnValue)
{
  UDFValue theArg;
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_GetFactDuplication (theEnv));
...
/***************************************************/
void
CL_GetFactDuplicationCommand (Environment * theEnv,
			      UDFContext * context, UDFValue * returnValue)
{
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_GetFactDuplication (theEnv));
...
/*******************************************/
void
CL_FactIndexFunction (Environment * theEnv,
		      UDFContext * context, UDFValue * returnValue)
{
  UDFValue theArg;
  ...
  returnValue->integerValue =
    CL_CreateInteger (theEnv, CL_FactIndex (theArg.factValue));
}
...
/**************************************/
void
CL_FactsCommand (Environment * theEnv,
		 UDFContext * context, UDFValue * returnValue)
{
  ...
  if (!CL_UDFFirstArgument (context, SYMBOL_BIT | INTEGER_BIT, &theArg))
    return;
  ...
  if (CVIsType (&theArg, SYMBOL_BIT))
	  ...
	  && (strcmp (theArg.lexemeValue->contents, "*") != 0))
	{
      ...
      if ((start = Get_FactsArgument (context)) == INVALID)
	return;
    }
  ...
  /* check to see that a valid index was specified. */
   /*================================================*/
  ...
  else if (CVIsType (&theArg, INTEGER_BIT))
    {
      ...
      if (start < 0)
	{
	  CL_ExpectedTypeError1 (theEnv, "facts", 1,
				 "symbol or 'positive number'");
  ...
  if ((end = Get_FactsArgument (context)) == INVALID)
    return;
  if ((max = Get_FactsArgument (context)) == INVALID)
    return;
...
/**************************************************/
void
CL_Facts (Environment * theEnv,
	  const char *logicalName,
	  Defmodule * theModule,
	  long long start, long long end, long long max)
{
  Fact *factPtr;
  ...
  if (allModules)
    factPtr = CL_GetNextFact (theEnv, NULL);
  else
    factPtr = CL_GetNextFactInScope (theEnv, NULL);
  ...
  while (factPtr != NULL)
	...
	factPtr = CL_GetNextFactInScope (theEnv, factPtr);
    }
...
/****************************************************************/
static long long
Get_FactsArgument (UDFContext * context)
{
  long long factIndex;
  UDFValue theArg;
...
/**********************************************/
void
CL_AssertStringFunction (Environment * theEnv,
			 UDFContext * context, UDFValue * returnValue)
{
  UDFValue theArg;
  Fact *theFact;
...
/******************************************/
void
CL_Save_FactsCommand (Environment * theEnv,
		      UDFContext * context, UDFValue * returnValue)
{
  const char *fileName;
  ...
  if (numArgs > 1)
	...
	{
	  CL_ExpectedTypeError1 (theEnv, "save-facts", 2,
				 "symbol with value local or visible");
	  returnValue->lexemeValue = FalseSymbol (theEnv);
	  return;
	}
    }
  ...
  if (numArgs > 2)
    theList = GetFirstArgument ()->nextArg->nextArg;
...
/******************************************/
void
CL_Load_FactsCommand (Environment * theEnv,
		      UDFContext * context, UDFValue * returnValue)
{
  const char *fileName;
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_Load_Facts (theEnv, fileName));
}
...
/***********************************************************/
bool
CL_Save_Facts (Environment * theEnv,
	       const char *fileName, CL_SaveScope saveCode)
{
...
/*****************************************************************/
bool
CL_Save_FactsDriver (Environment * theEnv,
		     const char *fileName,
		     CL_SaveScope saveCode, struct expr *theList)
{
  ...
  deftemplateArray =
    Get_Save_Facts_DeftemplateNames (theEnv, theList, saveCode, &count,
				     &error);
  ...
  for (theFact = CL_GetNextFactInScope (theEnv, NULL);
       theFact != NULL; theFact = CL_GetNextFactInScope (theEnv, theFact))
    {
      /*===========================================================*/
	  ...
	  (theFact->whichDeftemplate->header.whichModule->theModule !=
	   theModule))
	{
	  printFact = false;
	}
      ...
      else
	{
      ...
      if (printFact)
	{
	  CL_PrintFact (theEnv, (char *) filePtr, theFact, false, false,
			NULL);
...
/*******************************************************************/
static Deftemplate **
Get_Save_Facts_DeftemplateNames (Environment * theEnv,
				 struct expr *theList,
				 int saveCode,
				 unsigned int *count, bool *error)
{
  ...
  *error = false;
  ...
  for (tempList = theList, *count = 0;
       tempList != NULL; tempList = tempList->nextArg, (*count)++)
    {				/* Do Nothing */
    }
  ...
  deftemplateArray =
    (Deftemplate **) CL_gm2 (theEnv, sizeof (Deftemplate *) * *count);
  ...
  for (tempList = theList, i = 0;
       i < *count; tempList = tempList->nextArg, i++)
    {
      /*========================*/
      /* CL_Evaluate the argument. */
      /*========================*/
	...
	{
	  theDeftemplate =
	    CL_FindDeftemplateInModule (theEnv,
					tempArg.lexemeValue->contents);
	  if (theDeftemplate == NULL)
	    {
	      *error = true;
	      CL_ExpectedTypeError1 (theEnv, "save-facts", 3 + i,
				     "'local deftemplate name'");
	      CL_rm (theEnv, deftemplateArray,
		     sizeof (Deftemplate *) * *count);
	      return NULL;
	    }
	}
...
/***********************************************************/
bool
CL_Load_Facts (Environment * theEnv, const char *fileName)
{
  FILE *filePtr;
  ...
  while (theToken.tknType != STOP_TOKEN)
	...
	CL_EvaluateExpression (theEnv, testPtr, &rv);
...
/******************************************/
bool
CL_Load_FactsFromString (Environment * theEnv,
...
/**************************************************************************/
static struct expr *
Standard_LoadFact (Environment * theEnv,
...
/****************************************************************/
static struct expr *
CL_AssertParse (Environment * theEnv,
		struct expr *top, const char *logicalName)
{
  ...
  rv =
    CL_BuildRHS_Assert (theEnv, logicalName, &theToken, &error, true, true,
			"assert command");
