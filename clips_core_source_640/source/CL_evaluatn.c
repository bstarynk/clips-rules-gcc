...
/**************************************************/
void
Initialize_EvaluationData (Environment * theEnv)
{
  struct externalAddressType cPointer =
    { "C", PrintCAddress, PrintCAddress, NULL, NewCAddress, NULL };

  CL_AllocateEnvironmentData (theEnv, EVALUATION_DATA,
			      sizeof (struct evaluationData),
			      Deallocate_EvaluationData);
...
/*****************************************************/
static void
Deallocate_EvaluationData (Environment * theEnv)
{
  int i;

  for (i = 0; i < CL_EvaluationData (theEnv)->numberOfAddressTypes; i++)
    {
      rtn_struct (theEnv, externalAddressType,
		  CL_EvaluationData (theEnv)->ExternalAddressTypes[i]);
    }
}
...
/**************************************************************/
bool
CL_EvaluateExpression (Environment * theEnv,
		       struct expr *problem, UDFValue * returnValue)
{
  ...
  switch (problem->type)
    ...
    case MF_VARIABLE:
    case SF_VARIABLE:
      if (CL_GetBoundVariable (theEnv, returnValue, problem->lexemeValue) ==
	  false)
	{
...
/******************************************/
void
CL_InstallPrimitive (Environment * theEnv,
		     struct entityRecord *thePrimitive, int whichPosition)
{
...
/******************************************************/
int
CL_InstallExternalAddressType (Environment * theEnv,
			       struct externalAddressType *theAddressType)
{
  ...
  if (CL_EvaluationData (theEnv)->numberOfAddressTypes ==
      MAXIMUM_EXTERNAL_ADDRESS_TYPES)
    {
  ...
  copyEAT =
    (struct externalAddressType *) CL_genalloc (theEnv,
						sizeof (struct
							externalAddressType));
...
/*******************/
void
CL_ResetErrorFlags (Environment * theEnv)
{
...
/******************************************************/
void
Set_EvaluationError (Environment * theEnv, bool value)
{
...
/*********************************************************/
bool
Get_EvaluationError (Environment * theEnv)
{
  return (CL_EvaluationData (theEnv)->CL_EvaluationError);
}
...
/**************************************************/
void
Set_HaltExecution (Environment * theEnv, bool value)
{
...
/*****************************************************/
bool
CL_Get_HaltExecution (Environment * theEnv)
{
...
/*****************************************************/
void
CL_ReturnValues (Environment * theEnv,
		 UDFValue * garbagePtr, bool decrementSupplementalInfo)
{
  UDFValue *nextPtr;
  ...
  while (garbagePtr != NULL)
      ...
      if ((garbagePtr->supplementalInfo != NULL) && decrementSupplementalInfo)
	{
	  CL_ReleaseLexeme (theEnv,
			    (CLIPSLexeme *) garbagePtr->supplementalInfo);
	}
...
/**************************************************/
void
CL_WriteCLIPSValue (Environment * theEnv,
		    const char *fileid, CLIPSValue * argPtr)
{
...
/**********************************************/
void
CL_WriteUDFValue (Environment * theEnv, const char *fileid, UDFValue * argPtr)
{
  switch (argPtr->header->type)
    {
    case VOID_TYPE:
...
/*************************************************/
void
CL_SetMultifieldErrorValue (Environment * theEnv, UDFValue * returnValue)
{
...
/***********************************************/
void
CL_RetainUDFV (Environment * theEnv, UDFValue * vPtr)
{
  if (vPtr->header->type == MULTIFIELD_TYPE)
    {
      CL_IncrementCLIPSValueMultifieldReferenceCount (theEnv,
						      vPtr->multifieldValue);
    }
  else
    {
...
/***********************************************/
void
CL_ReleaseUDFV (Environment * theEnv, UDFValue * vPtr)
{
  if (vPtr->header->type == MULTIFIELD_TYPE)
    {
      CL_DecrementCLIPSValueMultifieldReferenceCount (theEnv,
						      vPtr->multifieldValue);
    }
  else
    {
...
/*************************************************/
void
CL_RetainCV (Environment * theEnv, CLIPSValue * vPtr)
{
  if (vPtr->header->type == MULTIFIELD_TYPE)
    {
      CL_IncrementCLIPSValueMultifieldReferenceCount (theEnv,
...
/*************************************************/
void
CL_ReleaseCV (Environment * theEnv, CLIPSValue * vPtr)
{
  if (vPtr->header->type == MULTIFIELD_TYPE)
    {
      CL_DecrementCLIPSValueMultifieldReferenceCount (theEnv,
...
/******************************************/
void
CL_Retain (Environment * theEnv, TypeHeader * th)
{
  switch (th->type)
    {
    case SYMBOL_TYPE:
...
/*************************************/
void
CL_Release (Environment * theEnv, TypeHeader * th)
{
  switch (th->type)
    {
    case SYMBOL_TYPE:
...
/*****************************************/
void
CL_AtomInstall (Environment * theEnv, unsigned short type, void *vPtr)
{
  switch (type)
    {
    case SYMBOL_TYPE:
...
/*******************************************/
void
CL_AtomDeinstall (Environment * theEnv, unsigned short type, void *vPtr)
{
  switch (type)
    {
    case SYMBOL_TYPE:
...
/***************************************************/
void
CL_CopyDataObject (Environment * theEnv,
		   UDFValue * dst, UDFValue * src, int garbageMultifield)
{
...
/***********************************************/
void
CL_TransferDataObjectValues (UDFValue * dst, UDFValue * src)
{
  dst->value = src->value;
  dst->begin = src->begin;
  dst->range = src->range;
...
/************************************************************************/
struct expr *
CL_ConvertValueToExpression (Environment * theEnv, UDFValue * theValue)
{
  size_t i;
  ...
  for (i = theValue->begin; i < (theValue->begin + theValue->range); i++)
	...
	CL_GenConstant (theEnv,
			theValue->multifieldValue->contents[i].header->type,
			theValue->multifieldValue->contents[i].value);
      if (last == NULL)
	head = newItem;
      else
	last->nextArg = newItem;
      last = newItem;
    }

  if (head == NULL)
    return (CL_GenConstant
	    (theEnv, FCALL, CL_FindFunction (theEnv, "create$")));
...
/****************************************/
unsigned long
CL_GetAtomicHashValue (unsigned short type,
		       void *value, unsigned short position)
{
...
/***********************************************************/
struct expr *
CL_FunctionReferenceExpression (Environment * theEnv, const char *name)
{
#if DEFGENERIC_CONSTRUCT
  Defgeneric *gfunc;
#endif
...
/******************************************************************/
bool
CL_GetFunctionReference (Environment * theEnv,
			 const char *name, Expression * theReference)
{
#if DEFGENERIC_CONSTRUCT
  Defgeneric *gfunc;
#endif
...
/*******************************************************/
bool
CL_DOsEqual (UDFValue * dobj1, UDFValue * dobj2)
{
 ...
 ***********************************************************/
bool
CL_EvaluateAndStoreInDataObject (Environment * theEnv,
				 bool mfp,
				 Expression * theExp,
				 UDFValue * val, bool garbageSegment)
{
  val->begin = 0;
  val->range = 0;
  ...
  if (theExp == NULL)
    {
      if (garbageSegment)
	val->value = CL_CreateMultifield (theEnv, 0L);
      else
	val->value = CL_CreateUnmanagedMultifield (theEnv, 0L);
...
/******************/
static void
PrintCAddress (Environment * theEnv, const char *logicalName, void *theValue)
{
  char buffer[20];
...
/****************/
static void
NewCAddress (UDFContext * context, UDFValue * rv)
{
  ...
  if (numberOfArguments != 1)
    {
      ...
      CL_WriteString (theEnv, STDERR,
		      "Function new expected no additional arguments for the C external language type.\n");
      Set_EvaluationError (theEnv, true);
      return;
    }
...
/******************************/
FunctionCall_Builder *
CL_CreateFunctionCall_Builder (Environment * theEnv, size_t theSize)
{
  ...
  if (theSize == 0)
    ...
    {
      theFC->contents =
	(CLIPSValue *) CL_gm2 (theEnv, sizeof (CLIPSValue) * theSize);
    }
...
/**********************/
void
CL_FCBAppendUDFValue (FunctionCall_Builder * theFCB, UDFValue * theValue)
{
  ...
  if (neededSize > theFCB->bufferMaximum)
    {
      ...
      newArray =
	(CLIPSValue *) CL_gm2 (theEnv, sizeof (CLIPSValue) * newSize);
	...
	{
	  CL_rm (theFCB->fcbEnv, theFCB->contents,
		 sizeof (CLIPSValue) * theFCB->bufferMaximum);
	}
...
/**************/
void
CL_FCBAppend (FunctionCall_Builder * theFCB, CLIPSValue * theValue)
{
  Environment *theEnv = theFCB->fcbEnv;
  size_t i, neededSize, newSize;
  CLIPSValue *newArray;
...
/**************************/
void
CL_FCBAppendCLIPSInteger (FunctionCall_Builder * theFCB, CLIPSInteger * pv)
{
  CLIPSValue theValue;
...
/*********************/
void
CL_FCBAppendInteger (FunctionCall_Builder * theFCB, long long intValue)
{
  CLIPSValue theValue;
...
/************************/
void
CL_FCBAppendCLIPSFloat (FunctionCall_Builder * theFCB, CLIPSFloat * pv)
{
  CLIPSValue theValue;
...
/*******************/
void
CL_FCBAppendFloat (FunctionCall_Builder * theFCB, double floatValue)
{
  CLIPSValue theValue;
...
/*************************/
void
CL_FCBAppendCLIPSLexeme (FunctionCall_Builder * theFCB, CLIPSLexeme * pv)
{
  CLIPSValue theValue;
...
/********************/
void
CL_FCBAppendSymbol (FunctionCall_Builder * theFCB, const char *strValue)
{
  CLIPSValue theValue;
...
/********************/
void
CL_FCBAppendString (FunctionCall_Builder * theFCB, const char *strValue)
{
  CLIPSValue theValue;
...
/**************************/
void
CL_FCBAppend_InstanceName (FunctionCall_Builder * theFCB,
			   const char *strValue)
{
  CLIPSValue theValue;
...
/**********************************/
void
CL_FCBAppendCLIPSExternalAddress (FunctionCall_Builder * theFCB,
...
/******************/
void
CL_FCBAppendFact (FunctionCall_Builder * theFCB, Fact * pv)
{
  CLIPSValue theValue;
...
/**********************/
void
CL_FCBAppendInstance (FunctionCall_Builder * theFCB, Instance * pv)
{
  CLIPSValue theValue;
...
/************************/
void
CL_FCBAppendMultifield (FunctionCall_Builder * theFCB, Multifield * pv)
{
  CLIPSValue theValue;
...
CL_FCBCall (FunctionCall_Builder * theFCB,
	    const char *functionName, CLIPSValue * returnValue)
{
  Environment *theEnv;
  ...
  for (i = 0; i < theFCB->length; i++)
	...
	{
	  nextAdd =
	    CL_GenConstant (theEnv, theFCB->contents[i].header->type,
			    theFCB->contents[i].value);
  ...
  if (theReference.type == FCALL)
    {
      ...
      if ((theError =
	   CL_CheckExpressionAgainstRestrictions (theEnv, &theReference,
						  theFunction,
						  functionName)) !=
	  FAE_NO_ERROR)
	{
	  CL_ExpressionDeinstall (theEnv, &theReference);
	  CL_ReturnExpression (theEnv, theReference.argList);
	  if (theError == FAE_TYPE_ERROR)
	    return FCBE_ARGUMENT_TYPE_ERROR;
	  else if (theError == FAE_COUNT_ERROR)
	    return FCBE_ARGUMENT_COUNT_ERROR;
	  else
	    {
...
/*************/
void
CL_FCB_Reset (FunctionCall_Builder * theFCB)
{
  size_t i;
  ...
  if (theFCB->buffer_Reset != theFCB->bufferMaximum)
	...
	{
	  theFCB->contents =
	    (CLIPSValue *) CL_gm2 (theFCB->fcbEnv,
				   sizeof (CLIPSValue) *
				   theFCB->buffer_Reset);
	}
...
/***************/
void
CL_FCBDispose (FunctionCall_Builder * theFCB)
{
  Environment *theEnv = theFCB->fcbEnv;
  size_t i;
