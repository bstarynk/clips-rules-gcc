...
CL_GetConstraintRecord (Environment * theEnv)
{
...
/********************************************************/
void
CL_SetAnyAllowedFlags (CONSTRAINT_RECORD * theConstraint, bool justOne)
{
  bool flag1, flag2;
...
/*****************************************************/
struct constraintRecord *
CL_CopyConstraintRecord (Environment * theEnv,
			 CONSTRAINT_RECORD * sourceConstraint)
{
  ...
  theConstraint->instanceNamesAllowed =
    sourceConstraint->instanceNamesAllowed;
  theConstraint->instanceAddressesAllowed =
    sourceConstraint->instanceAddressesAllowed;
  theConstraint->externalAddressesAllowed =
    sourceConstraint->externalAddressesAllowed;
  ...
  theConstraint->factAddressesAllowed =
    sourceConstraint->factAddressesAllowed;
  ...
  theConstraint->instanceNameRestriction =
    sourceConstraint->instanceNameRestriction;
  theConstraint->classList =
    CL_CopyExpression (theEnv, sourceConstraint->classList);
  theConstraint->restrictionList =
    CL_CopyExpression (theEnv, sourceConstraint->restrictionList);
  theConstraint->minValue =
    CL_CopyExpression (theEnv, sourceConstraint->minValue);
  theConstraint->maxValue =
    CL_CopyExpression (theEnv, sourceConstraint->maxValue);
  theConstraint->minFields =
    CL_CopyExpression (theEnv, sourceConstraint->minFields);
  theConstraint->maxFields =
    CL_CopyExpression (theEnv, sourceConstraint->maxFields);
  theConstraint->bucket = 0;
  theConstraint->installed = false;
  theConstraint->count = 0;
  theConstraint->multifield =
    CL_CopyConstraintRecord (theEnv, sourceConstraint->multifield);
...
/**************************************************************/
void
CL_SetAnyRestrictionFlags (CONSTRAINT_RECORD * theConstraint, bool justOne)
{
  bool flag1, flag2;
...
/*****************************************************/
bool
CL_SetConstraintType (int theType, CONSTRAINT_RECORD * constraints)
{
  bool rv = true;
...
/*************************************************************/
int
CL_CompareNumbers (Environment * theEnv,
		   int type1, void *vptr1, int type2, void *vptr2)
{
   /*============================================*/
  ...
  if (vptr1 == SymbolData (theEnv)->PositiveInfinity)
    return (GREATER_THAN);

  if (vptr1 == SymbolData (theEnv)->NegativeInfinity)
    return (LESS_THAN);

  if (vptr2 == SymbolData (theEnv)->PositiveInfinity)
    return (LESS_THAN);

  if (vptr2 == SymbolData (theEnv)->NegativeInfinity)
    return (GREATER_THAN);
  ...
  if ((type1 == INTEGER_TYPE) && (type2 == INTEGER_TYPE))
	  ...
	  ((CLIPSInteger *) vptr2)->contents)
	{
      ...
      else if (((CLIPSInteger *) vptr1)->contents >
	       ((CLIPSInteger *) vptr2)->contents)
	{
	  return (GREATER_THAN);
	}
  ...
  if ((type1 == FLOAT_TYPE) && (type2 == FLOAT_TYPE))
      ...
      if (((CLIPSFloat *) vptr1)->contents < ((CLIPSFloat *) vptr2)->contents)
	{
      ...
      else if (((CLIPSFloat *) vptr1)->contents >
  ...
  if ((type1 == INTEGER_TYPE) && (type2 == FLOAT_TYPE))
	  ...
	  ((CLIPSFloat *) vptr2)->contents)
	{
      ...
      else if (((double) ((CLIPSInteger *) vptr1)->contents) >
	       ((CLIPSFloat *) vptr2)->contents)
	{
	  return (GREATER_THAN);
	}
...
/****************************************************************/
CONSTRAINT_RECORD *
CL_ExpressionToConstraintRecord (Environment * theEnv,
				 struct expr *theExpression)
{
  ...
  if ((theExpression->type == SF_VARIABLE) ||
    ...
    {
      return (CL_FunctionCallToConstraintRecord
	      (theEnv, theExpression->value));
    }
  ...
  if (rv->floatsAllowed || rv->integersAllowed || rv->symbolsAllowed ||
    ...
    {
      rv->restrictionList =
	CL_GenConstant (theEnv, theExpression->type, theExpression->value);
    }
...
{
  return CL_ArgumentTypeToConstraintRecord (theEnv,
					    UnknownFunctionType
					    (theFunction));
}
...
/*********************************************/
CONSTRAINT_RECORD *
CL_ArgumentTypeToConstraintRecord (Environment * theEnv, unsigned bitTypes)
{
  CONSTRAINT_RECORD *rv;
