...
struct factgenData
{
  ...
  struct entityRecord CL_FactSlotLengthInfo;
  struct entityRecord CL_FactPNConstant1Info;
  struct entityRecord CL_FactPNConstant2Info;
};
...
/*******************************************************************/
/* CL_InitializeFactReteFunctions: Installs the fact pattern matching */
/*   and value access routines as primitive operations.            */
/*******************************************************************/
void
CL_InitializeFactReteFunctions (Environment * theEnv)
{
  ...
  CL_AllocateEnvironmentData (theEnv, FACTGEN_DATA,
			      sizeof (struct factgenData), NULL);

  memcpy (&FactgenData (theEnv)->FactJNGV1Info, &factJNGV1Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->FactJNGV2Info, &factJNGV2Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->FactJNGV3Info, &factJNGV3Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->FactPNGV1Info, &factPNGV1Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->FactPNGV2Info, &factPNGV2Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->FactPNGV3Info, &factPNGV3Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->FactJNCV1Info, &factJNCV1Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->FactJNCV2Info, &factJNCV2Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->FactPNCV1Info, &factPNCV1Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->FactStoreMFInfo, &factStoreMFInfo,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->CL_FactSlotLengthInfo, &factSlotLengthInfo,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->CL_FactPNConstant1Info, &factPNConstant1Info,
	  sizeof (struct entityRecord));
  memcpy (&FactgenData (theEnv)->CL_FactPNConstant2Info, &factPNConstant2Info,
	  sizeof (struct entityRecord));

  CL_InstallPrimitive (theEnv, (EntityRecord *) & FactData (theEnv)->FactInfo,
		       FACT_ADDRESS_TYPE);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactJNGV1Info,
		       FACT_JN_VAR1);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactJNGV2Info,
		       FACT_JN_VAR2);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactJNGV3Info,
		       FACT_JN_VAR3);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactPNGV1Info,
		       FACT_PN_VAR1);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactPNGV2Info,
		       FACT_PN_VAR2);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactPNGV3Info,
		       FACT_PN_VAR3);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactJNCV1Info,
		       FACT_JN_CMP1);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactJNCV2Info,
		       FACT_JN_CMP2);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactPNCV1Info,
		       FACT_PN_CMP1);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->FactStoreMFInfo,
		       FACT_STORE_MULTIFIELD);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->CL_FactSlotLengthInfo,
		       FACT_SLOT_LENGTH);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->CL_FactPNConstant1Info,
		       FACT_PN_CONSTANT1);
  CL_InstallPrimitive (theEnv, &FactgenData (theEnv)->CL_FactPNConstant2Info,
		       FACT_PN_CONSTANT2);
#endif
}
...
/******************************************************************/
/* CL_FactGenPNConstant: Generates an expression for use in the fact */
/*   pattern network that compares a field from a single field or */
/*   multifield slot against a constant.                          */
/******************************************************************/
struct expr *
CL_FactGenPNConstant (Environment * theEnv, struct lhsParseNode *theField)
{
  struct expr *top;
  ...
  if (theField->withinMultifieldSlot == false)
    {
      CL_ClearBitString (&hack1, sizeof (struct factConstantPN1Call));

      if (theField->negated)
	hack1.testForEquality = false;
      else
	hack1.testForEquality = true;

      hack1.whichSlot = (theField->slotNumber - 1);

      top =
	CL_GenConstant (theEnv, FACT_PN_CONSTANT1,
			CL_AddBitMap (theEnv, &hack1,
				      sizeof (struct factConstantPN1Call)));

      top->argList =
	CL_GenConstant (theEnv, CL_NodeTypeToType (theField),
			theField->value);
  ...
  /* but include the offset from the beginning of the slot.          */
   /*=================================================================*/
	   ...
	   ((theField->multiFieldsBefore == 1)
	    && (theField->multiFieldsAfter == 0)))
    {
      CL_ClearBitString (&hack2, sizeof (struct factConstantPN2Call));

      if (theField->negated)
	hack2.testForEquality = false;
      else
	hack2.testForEquality = true;
      ...
      else
	{
      ...
      top =
	CL_GenConstant (theEnv, FACT_PN_CONSTANT2,
			CL_AddBitMap (theEnv, &hack2,
				      sizeof (struct factConstantPN2Call)));
  ...
  /* pattern network variable retrieval function call.             */
   /*===============================================================*/
	...
	{
	  top =
	    CL_GenConstant (theEnv, FCALL, ExpressionData (theEnv)->PTR_NEQ);
	}
      else
	{
	  top =
	    CL_GenConstant (theEnv, FCALL, ExpressionData (theEnv)->PTR_EQ);
	}

      tempValue = theField->pnType;
      theField->pnType = SF_VARIABLE_NODE;
      top->argList = CL_FactGenGetfield (theEnv, theField);
...
/*******************************************************/
/* CL_FactGenGetfield: Generates an expression for use in */
/*   the fact pattern network that retrieves a value   */
/*   from a single or multifield slot.                 */
/*******************************************************/
struct expr *
CL_FactGenGetfield (Environment * theEnv, struct lhsParseNode *theNode)
{
   /*===================================================*/
  ...
  if ((theNode->slotNumber > 0) && (theNode->slotNumber != UNSPECIFIED_SLOT)
      && (theNode->withinMultifieldSlot == false))
    {
      return (CL_GenConstant
	      (theEnv, FACT_PN_VAR2, FactGetVarPN2 (theEnv, theNode)));
    }
  ...
  if (((theNode->pnType == SF_WILDCARD_NODE)
       || (theNode->pnType == SF_VARIABLE_NODE) || CL_ConstantNode (theNode))
      && ((theNode->multiFieldsBefore == 0)
	  || ((theNode->multiFieldsBefore == 1)
	      && (theNode->multiFieldsAfter == 0))))
    {
      return (CL_GenConstant
	      (theEnv, FACT_PN_VAR3, FactGetVarPN3 (theEnv, theNode)));
    }

  if (((theNode->pnType == MF_WILDCARD_NODE)
  ...
  return (CL_GenConstant
	  (theEnv, FACT_PN_VAR1, FactGetVarPN1 (theEnv, theNode)));
}

/**************************************************/
/* CL_FactGenGetvar: Generates an expression for use */
/*   in the join network that retrieves a value   */
/*   from a single or multifield slot of a fact.  */
/**************************************************/
struct expr *
CL_FactGenGetvar (Environment * theEnv,
		  struct lhsParseNode *theNode, int side)
{
   /*====================================================*/
  /* Generate call to retrieve single field slot value. */
   /*====================================================*/
  ...
  if ((theNode->slotNumber > 0) && (theNode->slotNumber != UNSPECIFIED_SLOT)
    ...
    {
      return (CL_GenConstant
	      (theEnv, FACT_JN_VAR2, FactGetVarJN2 (theEnv, theNode, side)));
    }
  ...
  if (((theNode->pnType == SF_WILDCARD_NODE)
    ...
    {
      return (CL_GenConstant
	      (theEnv, FACT_JN_VAR3, FactGetVarJN3 (theEnv, theNode, side)));
    }
  ...
  return (CL_GenConstant
	  (theEnv, FACT_JN_VAR1, FactGetVarJN1 (theEnv, theNode, side)));
}
...
/**************************************************************/
struct expr *
CL_FactGenCheckLength (Environment * theEnv, struct lhsParseNode *theNode)
{
  ...
  CL_ClearBitString (&hack, sizeof (struct factCheckLengthPNCall));
  ...
  return (CL_GenConstant
	  (theEnv, FACT_SLOT_LENGTH,
	   CL_AddBitMap (theEnv, &hack,
			 sizeof (struct factCheckLengthPNCall))));
}
...
/**************************************************************/
struct expr *
CL_FactGenCheckZeroLength (Environment * theEnv, unsigned short theSlot)
{
  struct factCheckLengthPNCall hack;
...
/*********************************************************************/
/* CL_FactReplaceGetvar: Replaces a variable reference in an expression */
/*   with a function call to retrieve the variable using the join    */
/*   network variable access functions for facts.                    */
/*********************************************************************/
void
CL_FactReplaceGetvar (Environment * theEnv,
		      struct expr *theItem,
		      struct lhsParseNode *theNode, int side)
{
   /*====================================================*/
...
/***********************************************************************/
/* CL_FactReplaceGetfield: Replaces a variable reference in an expression */
/*   with a function call to retrieve the variable using the pattern   */
/*   network variable access functions for facts.                      */
/***********************************************************************/
void
CL_FactReplaceGetfield (Environment * theEnv,
			struct expr *theItem, struct lhsParseNode *theNode)
{
   /*====================================================*/
  /* Generate call to retrieve single field slot value. */
   /*====================================================*/
...
/*************************************************************/
static void *
FactGetVarJN1 (Environment * theEnv, struct lhsParseNode *theNode, int side)
{
  struct factGetVarJN1Call hack;

   /*===================================================*/
  /* CL_Clear the bitmap for storing the argument values. */
   /*===================================================*/

  CL_ClearBitString (&hack, sizeof (struct factGetVarJN1Call));
  ...
  return (CL_AddBitMap (theEnv, &hack, sizeof (struct factGetVarJN1Call)));
}
...
/**************************************************************/
static void *
FactGetVarJN2 (Environment * theEnv, struct lhsParseNode *theNode, int side)
{
  ...
  CL_ClearBitString (&hack, sizeof (struct factGetVarJN2Call));
  ...
  return CL_AddBitMap (theEnv, &hack, sizeof (struct factGetVarJN2Call));
}
...
/*************************************************************/
static void *
FactGetVarJN3 (Environment * theEnv, struct lhsParseNode *theNode, int side)
{
  ...
  CL_ClearBitString (&hack, sizeof (struct factGetVarJN3Call));
  ...
  if ((theNode->pnType == SF_WILDCARD_NODE)
      ...
      else
	{
      ...
      return (CL_AddBitMap
	      (theEnv, &hack, sizeof (struct factGetVarJN3Call)));
    }
  ...
  return CL_AddBitMap (theEnv, &hack, sizeof (struct factGetVarJN3Call));
}
...
/**************************************************************/
static void *
FactGetVarPN1 (Environment * theEnv, struct lhsParseNode *theNode)
{
  ...
  CL_ClearBitString (&hack, sizeof (struct factGetVarPN1Call));
  ...
  return CL_AddBitMap (theEnv, &hack, sizeof (struct factGetVarPN1Call));
}
...
/***************************************************************/
static void *
FactGetVarPN2 (Environment * theEnv, struct lhsParseNode *theNode)
{
  ...
  CL_ClearBitString (&hack, sizeof (struct factGetVarPN2Call));
  ...
  return CL_AddBitMap (theEnv, &hack, sizeof (struct factGetVarPN2Call));
}
...
/*************************************************************/
static void *
FactGetVarPN3 (Environment * theEnv, struct lhsParseNode *theNode)
{
  ...
  CL_ClearBitString (&hack, sizeof (struct factGetVarPN3Call));
  ...
  if ((theNode->pnType == SF_WILDCARD_NODE)
      ...
      else
	{
      ...
      return (CL_AddBitMap
	      (theEnv, &hack, sizeof (struct factGetVarPN3Call)));
    }
  ...
  return CL_AddBitMap (theEnv, &hack, sizeof (struct factGetVarPN3Call));
}

/*************************************************************/
/* CL_FactPNVariableComparison: Generates an expression for use */
/*   in the fact pattern network to compare two variables of */
/*   the same name found in the same pattern.                */
/*************************************************************/
struct expr *
CL_FactPNVariableComparison (Environment * theEnv,
			     ...
			     struct lhsParseNode *referringNode)
{
  ...
  CL_ClearBitString (&hack, sizeof (struct factCompVarsPN1Call));
  ...
  if ((selfNode->withinMultifieldSlot == false) &&
	...
	hack.pass = 1;
      ...
      top =
	CL_GenConstant (theEnv, FACT_PN_CMP1,
			CL_AddBitMap (theEnv, &hack,
				      sizeof (struct factCompVarsPN1Call)));
    }
  ...
  /* by the appropriate get variable value functions.               */
   /*================================================================*/
	...
	top = CL_GenConstant (theEnv, FCALL, ExpressionData (theEnv)->PTR_EQ);

      top->argList = CL_FactGenGetfield (theEnv, selfNode);
      top->argList->nextArg = CL_FactGenGetfield (theEnv, referringNode);
    }
...
/*********************************************************/
/* CL_FactJNVariableComparison: Generates an expression for */
/*   use in the join network to compare two variables of */
/*   the same name found in different patterns.          */
/*********************************************************/
struct expr *
CL_FactJNVariableComparison (Environment * theEnv,
			     struct lhsParseNode *selfNode,
			     struct lhsParseNode *referringNode,
			     bool nandJoin)
{
  ...
  if ((selfNode->withinMultifieldSlot == false) &&
      ...
      (referringNode->slotNumber != UNSPECIFIED_SLOT))
    {
      CL_ClearBitString (&hack1, sizeof (struct factCompVarsJN1Call));
      hack1.pass = 0;
      hack1.fail = 0;
      ...
      else
	{
      ...
      if (referringNode->index == NO_INDEX)
	hack1.slot2 = 0;
      else
	hack1.slot2 = referringNode->slotNumber - 1;
	...
	hack1.pass = 1;
      ...
      top =
	CL_GenConstant (theEnv, FACT_JN_CMP1,
			CL_AddBitMap (theEnv, &hack1,
				      sizeof (struct factCompVarsJN1Call)));
    }
  ...
  /* following specified variable comparison routine.              */
   /*===============================================================*/
    ...
    {
      CL_ClearBitString (&hack2, sizeof (struct factCompVarsJN2Call));
      hack2.pass = 0;
      hack2.fail = 0;
	...
	hack2.pass = 1;
      ...
      top =
	CL_GenConstant (theEnv, FACT_JN_CMP2,
			CL_AddBitMap (theEnv, &hack2,
				      sizeof (struct factCompVarsJN2Call)));
    }
  ...
  /* retrieval function call.                                      */
   /*===============================================================*/
      ...
      else
	{
      ...
      if (nandJoin)
	{
	  top->argList = CL_FactGenGetvar (theEnv, selfNode, NESTED_RHS);
	}
      else
	{
	  top->argList = CL_FactGenGetvar (theEnv, selfNode, RHS);
	}

      top->argList->nextArg = CL_FactGenGetvar (theEnv, referringNode, LHS);
    }
