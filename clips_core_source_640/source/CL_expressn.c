...
#if (! RUN_TIME)
static unsigned long ListToPacked (struct expr *, struct expr *,
				   unsigned long);
#endif
static EXPRESSION_HN *FindHashedExpression (Environment *, Expression *,
					    unsigned *, EXPRESSION_HN **);
...
/**************************************************/
void
CL_InitExpressionData (Environment * theEnv)
{
  unsigned i;

  CL_AllocateEnvironmentData (theEnv, EXPRESSION_DATA,
			      sizeof (struct expressionData),
			      DeallocateExpressionData);
...
/*****************************************/
static void
DeallocateExpressionData (Environment * theEnv)
{
  int i;
...
#if (BLOAD || BLOAD_ONLY || BLOAD_AND_BSAVE)
  if ((ExpressionData (theEnv)->NumberOfExpressions != 0)
      && CL_Bloaded (theEnv))
    {
      CL_genfree (theEnv, ExpressionData (theEnv)->ExpressionArray,
		  ExpressionData (theEnv)->NumberOfExpressions *
		  sizeof (struct expr));
    }
#endif
}
...
/****************************************************/
void
CL_InitExpressionPointers (Environment * theEnv)
{
  ...
  if ((ExpressionData (theEnv)->PTR_AND == NULL)
      || (ExpressionData (theEnv)->PTR_OR == NULL)
      || (ExpressionData (theEnv)->PTR_EQ == NULL)
      || (ExpressionData (theEnv)->PTR_NEQ == NULL)
...
/***************************************************/
void
CL_ExpressionInstall (Environment * theEnv, struct expr *expression)
{
  if (expression == NULL)
    return;
...
/*****************************************************/
void
CL_ExpressionDeinstall (Environment * theEnv, struct expr *expression)
{
  if (expression == NULL)
    return;
...
/***********************************************************************/
struct expr *
CL_PackExpression (Environment * theEnv, struct expr *original)
{
  struct expr *packPtr;
...
ListToPacked (struct expr *original,
	      struct expr *destination, unsigned long count)
{
  unsigned long i;
  ...
  while (original != NULL)
	...
	{
	  destination[i].argList = (struct expr *) &destination[count];
...
/***************************************************************/
void
CL_ReturnPackedExpression (Environment * theEnv, struct expr *packPtr)
{
  if (packPtr != NULL)
    {
      CL_rm (theEnv, packPtr,
	     sizeof (struct expr) * CL_ExpressionSize (packPtr));
    }
}
...
/***********************************************/
void
CL_ReturnExpression (Environment * theEnv, struct expr *waste)
{
  struct expr *tmp;
  ...
  while (waste != NULL)
	...
	CL_ReturnExpression (theEnv, waste->argList);
      tmp = waste;
      waste = waste->nextArg;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static EXPRESSION_HN *
FindHashedExpression (Environment * theEnv,
		      Expression * theExp,
		      unsigned *hashval, EXPRESSION_HN ** prv)
{
 ...
 ***************************************************/
static unsigned
HashExpression (Expression * theExp)
{
 ...
 ***************************************************/
void
CL_RemoveHashedExpression (Environment * theEnv, Expression * theExp)
{
 ...
 *****************************************************/
Expression *
CL_AddHashedExpression (Environment * theEnv, Expression * theExp)
{
  ...
  exphash->next =
    ExpressionData (theEnv)->ExpressionHashTable[exphash->hashval];
 ...
 ***************************************************/
unsigned long
CL_HashedExpressionIndex (Environment * theEnv, Expression * theExp)
{
  EXPRESSION_HN *exphash, *prv;
  unsigned hashval;
...
/********************************************************/
bool
CL_SetSequenceOperatorRecognition (Environment * theEnv, bool value)
{
  bool ov;
...
/********************************************************/
bool
CL_GetSequenceOperatorRecognition (Environment * theEnv)
{
