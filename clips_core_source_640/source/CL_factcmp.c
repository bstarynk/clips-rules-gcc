...
static bool PatternNetworkToCode (Environment *, const char *, const char *,
...
static void PatternNodeToCode (Environment *, FILE *,
			       struct factPatternNode *, unsigned int,
			       unsigned int);
...
/**************************************************************/
void
CL_FactPatternsCompilerSetup (Environment * theEnv)
{
  FactData (theEnv)->FactCodeItem =
    CL_AddCodeGeneratorItem (theEnv, "facts", 0, BeforePatternNetworkToCode,
...
/****************************************************************/
static void
BeforePatternNetworkToCode (Environment * theEnv)
{
...
GetNextPatternNode (struct factPatternNode *thePattern)
{
   /*=========================================*/
...
/********************************************************************/
static bool
PatternNetworkToCode (Environment * theEnv,
		      const char *fileName,
		      const char *pathName,
		      char *fileNameBuffer,
		      unsigned int fileID,
	   ...
	   theTemplate = CL_GetNextDeftemplate (theEnv, theTemplate))
	{
	 ...
	 /*======================================================*/
	  ...
	  for (thePatternNode = theTemplate->patternNetwork;
	    ...
	    {
	      networkFile =
		CL_OpenFileIfNeeded (theEnv, networkFile, fileName, pathName,
				     fileNameBuffer, fileID, imageID,
				     &fileCount, networkArrayVersion,
				     headerFP, "struct factPatternNode",
				     FactPrefix (), false, NULL);
	      if (networkFile == NULL)
		{
	      ...
	      PatternNodeToCode (theEnv, networkFile, thePatternNode, imageID,
				 maxIndices);
	      networkArrayCount++;
	      networkFile =
		CL_CloseFileIfNeeded (theEnv, networkFile, &networkArrayCount,
				      &networkArrayVersion, maxIndices, NULL,
				      NULL);
	    }
	}
    }
...
/****************************************************************/
static void
CloseNetworkFiles (Environment * theEnv,
		   FILE * networkFile, unsigned int maxIndices)
{
  ...
  if (networkFile != NULL)
    {
      CL_CloseFileIfNeeded (theEnv, networkFile, &count, &arrayVersion,
...
/************************************************************/
static void
PatternNodeToCode (Environment * theEnv,
		   FILE * theFile,
		   struct factPatternNode *thePatternNode,
		   unsigned int imageID, unsigned int maxIndices)
{
  ...
  CL_PatternNodeHeaderToCode (theEnv, theFile, &thePatternNode->header,
			      imageID, maxIndices);
  ...
  fprintf (theFile, ",0,%d,%d,%d,", thePatternNode->whichField,
	   thePatternNode->whichSlot, thePatternNode->leaveFields);

   /*===============*/
  /* Network Tests */
   /*===============*/

  CL_PrintHashedExpressionReference (theEnv, theFile,
				     thePatternNode->networkTest, imageID,
				     maxIndices);
...
/**********************************************************/
void
CL_FactPatternNodeReference (Environment * theEnv,
			     void *theVPattern,
			     FILE * theFile,
			     unsigned int imageID, unsigned int maxIndices)
{
