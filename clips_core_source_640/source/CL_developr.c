...
/**************************************************/
void
DeveloperCommands (Environment * theEnv)
{
#if ! RUN_TIME
  CL_AddUDF (theEnv, "primitives-info", "v", 0, 0, NULL,
	     PrimitiveTablesInfoCommand, "PrimitiveTablesInfoCommand", NULL);
  CL_AddUDF (theEnv, "primitives-usage", "v", 0, 0, NULL,
	     PrimitiveTablesUsageCommand, "PrimitiveTablesUsageCommand",
	     NULL);

#if DEFRULE_CONSTRUCT && DEFTEMPLATE_CONSTRUCT
  CL_AddUDF (theEnv, "validate-fact-integrity", "b", 0, 0, NULL,
	     ValidateFactIntegrityCommand, "ValidateFactIntegrityCommand",
	     NULL);

  CL_AddUDF (theEnv, "show-fpn", "v", 1, 1, "y",
	     ShowFactPatternNetworkCommand, "ShowFactPatternNetworkCommand",
	     NULL);
  CL_AddUDF (theEnv, "show-fht", "v", 0, 0, NULL, ShowFactHashTableCommand,
	     "ShowFactHashTableCommand", NULL);
#endif

#if DEFRULE_CONSTRUCT && OBJECT_SYSTEM
  CL_AddUDF (theEnv, "show-opn", "v", 0, 0, NULL,
	     PrintObjectPatternNetworkCommand,
	     "PrintObjectPatternNetworkCommand", NULL);
#endif

#if OBJECT_SYSTEM
  CL_AddUDF (theEnv, "instance-table-usage", "v", 0, 0, NULL,
	     InstanceTableUsageCommand, "InstanceTableUsageCommand", NULL);
#endif
...
/******************************************************/
void
PrimitiveTablesInfoCommand (Environment * theEnv,
			    UDFContext * context, UDFValue * returnValue)
{
  ...
  for (i = 0; i < SYMBOL_HASH_SIZE; i++)
    {
      for (symbolPtr = symbolArray[i]; symbolPtr != NULL;
  ...
  for (i = 0; i < INTEGER_HASH_SIZE; i++)
    {
      for (integerPtr = integerArray[i]; integerPtr != NULL;
  ...
  for (i = 0; i < FLOAT_HASH_SIZE; i++)
    {
      for (floatPtr = floatArray[i]; floatPtr != NULL;
  ...
  for (i = 0; i < BITMAP_HASH_SIZE; i++)
    {
      for (bitMapPtr = bitMapArray[i]; bitMapPtr != NULL;
...
/*********************************************************/
void
PrimitiveTablesUsageCommand (Environment * theEnv,
			     UDFContext * context, UDFValue * returnValue)
{
  unsigned long i;
...
/******************************************************/
void
ValidateFactIntegrityCommand (Environment * theEnv,
			      UDFContext * context, UDFValue * returnValue)
{
  Fact *theFact;
...
/*************************************************************/
void
ShowFactPatternNetworkCommand (Environment * theEnv,
			       UDFContext * context, UDFValue * returnValue)
{
  ...
  while (patternPtr != NULL)
    {
      for (i = 0; i < depth; i++)
	CL_WriteString (theEnv, STDOUT, " ");
      if (patternPtr->header.singlefieldNode)
	CL_WriteString (theEnv, STDOUT, "SF   ");
      else if (patternPtr->header.multifieldNode)
	{
      ...
      if (patternPtr->networkTest == NULL)
	CL_WriteString (theEnv, STDOUT, "None");
      else
	CL_PrintExpression (theEnv, STDOUT, patternPtr->networkTest);
      CL_WriteString (theEnv, STDOUT, " RightHash: ");
      if (patternPtr->header.rightHash == NULL)
	CL_WriteString (theEnv, STDOUT, "None");
      else
	CL_PrintExpression (theEnv, STDOUT, patternPtr->header.rightHash);
      CL_WriteString (theEnv, STDOUT, "\n");
 ...
 ***************************************************/
void
PrintObjectPatternNetworkCommand (Environment * theEnv,
				  UDFContext * context,
 ...
 **********************************************************/
static void
PrintOPNLevel (Environment * theEnv,
	       OBJECT_PATTERN_NODE * pptr, char *indentbuf, int ilen)
{
  CLASS_BITMAP *cbmp;
  SLOT_BITMAP *sbmp;
  unsigned i;
  ...
  while (pptr != NULL)
	...
	CL_WriteString (theEnv, STDOUT, "+");
      CL_WriteString (theEnv, STDOUT,
		      CL_FindIDSlotName (theEnv, pptr->slotNameID)->contents);
      CL_WriteString (theEnv, STDOUT, " (");
...
/*******************************************************/
void
InstanceTableUsageCommand (Environment * theEnv,
			   UDFContext * context, UDFValue * returnValue)
{
  unsigned long i;
  ...
  for (i = 0; i < INSTANCE_TABLE_HASH_SIZE; i++)
    {
      instanceCount = 0;
      for (ins = InstanceData (theEnv)->InstanceTable[i]; ins != NULL;
	   ins = ins->nxtHash)
	{
	  instanceCount++;
	  totalInstanceCount++;
...
/******************/
static void
ExamineMemory (Environment * theEnv,
	       struct joinNode *theJoin, struct betaMemory *theMemory)
{
#if MAC_XCD
...
/*************************/
static void
TraverseBetaMemories (Environment * theEnv, struct joinNode *theJoin)
{
  if (theJoin == NULL)
    {
      return;
...
/***********************************/
static void
ValidateRuleBetaMemoriesAction (Environment * theEnv,
...
#pragma unused(buffer)
  ...
  for (rulePtr = (Defrule *) theConstruct;
       rulePtr != NULL; rulePtr = rulePtr->disjunct)
    {
...
/*******************************/
void
ValidateBetaMemoriesCommand (Environment * theEnv,
			     UDFContext * context, UDFValue * returnValue)
{
  CL_WriteString (theEnv, STDOUT, "ValidateBetaMemories");
  CL_DoForAllConstructs (theEnv, ValidateRuleBetaMemoriesAction,
			 DefruleData (theEnv)->CL_DefruleModuleIndex, false,
			 NULL);
}
