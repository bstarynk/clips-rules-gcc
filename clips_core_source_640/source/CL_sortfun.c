...
static bool DefaultCompareSwapFunction (Environment *, UDFValue *,
					UDFValue *);
...
/****************************************/
void
CL_SortFunctionDefinitions (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, SORTFUN_DATA,
			      sizeof (struct sortFunctionData),
			      Deallocate_SortFunctionData);
#if ! RUN_TIME
  CL_AddUDF (theEnv, "sort", "bm", 1, UNBOUNDED, "*;y", CL_SortFunction,
	     "CL_SortFunction", NULL);
#endif
}
...
/*******************************************************/
static void
Deallocate_SortFunctionData (Environment * theEnv)
{
...
/********************************/
static bool
DefaultCompareSwapFunction (Environment * theEnv,
			    UDFValue * item1, UDFValue * item2)
{
  UDFValue returnValue;

  CL_SortFunctionData (theEnv)->SortComparisonFunction->argList =
    CL_GenConstant (theEnv, item1->header->type, item1->value);
  CL_SortFunctionData (theEnv)->SortComparisonFunction->argList->nextArg =
    CL_GenConstant (theEnv, item2->header->type, item2->value);
...
/************************************/
void
CL_SortFunction (Environment * theEnv,
		 UDFContext * context, UDFValue * returnValue)
{
  ...
  if (functionReference == NULL)
    {
      CL_ExpectedTypeError1 (theEnv, "sort", 1,
			     "function name, deffunction name, or defgeneric name");
      return;
    }
  ...
  if (functionReference->type == FCALL)
    {
	  ...
	  (CL_GetMaximumArgs (fptr) == 0) || (CL_GetMaximumArgs (fptr) == 1))
	{
	  CL_ExpectedTypeError1 (theEnv, "sort", 1,
				 "function name expecting two arguments");
  ...
  if (functionReference->type == PCALL)
	...
	{
	  CL_ExpectedTypeError1 (theEnv, "sort", 1,
				 "deffunction name expecting two arguments");
	  CL_ReturnExpression (theEnv, functionReference);
	  return;
	}
    }
  ...
  theArguments =
    (UDFValue *) CL_genalloc (theEnv,
			      (argumentCount - 1) * sizeof (UDFValue));
  ...
  theArguments2 =
    (UDFValue *) CL_genalloc (theEnv, argumentSize * sizeof (UDFValue));
  ...
  for (i = 2; i <= argumentCount; i++)
      ...
      if (theArguments[i - 2].header->type == MULTIFIELD_TYPE)
	{
	  ...
	  for (j = theArguments[i - 2].begin;
	       j < (theArguments[i - 2].begin + theArguments[i - 2].range);
	       j++, k++)
	    {
  ...
  CL_MergeSort (theEnv, argumentSize, theArguments2,
		DefaultCompareSwapFunction);
  ...
  CL_SortFunctionData (theEnv)->SortComparisonFunction =
    CL_SortFunctionData (theEnv)->SortComparisonFunction->nextArg;
...
/*******************************************/
void
CL_MergeSort (Environment * theEnv,
	      size_t listSize,
	      UDFValue * theList,
	      bool (*swapFunction) (Environment *, UDFValue *, UDFValue *))
{
  ...
  middle = (listSize + 1) / 2;
  Do_MergeSort (theEnv, theList, tempList, 0, middle - 1, middle,
		listSize - 1, swapFunction);
...
/******************************************************/
static void
Do_MergeSort (Environment * theEnv,
	      UDFValue * theList,
	      UDFValue * tempList,
	      size_t s1,
	      size_t e1,
	      ...
	      bool (*swapFunction) (Environment *, UDFValue *, UDFValue *))
{
  ...
  if (s1 == e1)
  ...
  else
    {
      size = ((e1 - s1) + 1);
      middle = s1 + ((size + 1) / 2);
      Do_MergeSort (theEnv, theList, tempList, s1, middle - 1, middle, e1,
		    swapFunction);
    }
  ...
  if (s2 == e2)
  ...
  else
    {
      size = ((e2 - s2) + 1);
      middle = s2 + ((size + 1) / 2);
      Do_MergeSort (theEnv, theList, tempList, s2, middle - 1, middle, e2,
		    swapFunction);
    }
