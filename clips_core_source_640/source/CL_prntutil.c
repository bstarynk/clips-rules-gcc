...
/*****************************************************/
void
CL_InitializePrintUtilityData (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, PRINT_UTILITY_DATA,
			      sizeof (struct printUtilityData), NULL);
}
...
/************************************************************/
void
CL_WriteFloat (Environment * theEnv, const char *fileid, double number)
{
...
/************************************************/
void
CL_WriteInteger (Environment * theEnv,
		 const char *logicalName, long long number)
{
  char printBuffer[32];
...
/*******************************************/
void
CL_PrintUnsignedInteger (Environment * theEnv,
			 const char *logicalName, unsigned long long number)
{
  char printBuffer[32];
...
/**************************************/
void
CL_PrintAtom (Environment * theEnv,
	      const char *logicalName, unsigned short type, void *value)
{
  ...
  switch (type)
      ...
      else
	{
	  ...
	  CL_gensprintf (buffer, "%p",
			 ((CLIPSExternalAddress *) value)->contents);
	  CL_WriteString (theEnv, logicalName, buffer);
	  CL_WriteString (theEnv, logicalName, ">");
	}
...
/**********************************************************/
void
CL_PrintTally (Environment * theEnv,
	       ...
	       const char *singular, const char *plural)
{
  ...
  if (count == 1)
    CL_WriteString (theEnv, logicalName, singular);
  else
    CL_WriteString (theEnv, logicalName, plural);
...
/********************************************/
void
CL_PrintErrorID (Environment * theEnv,
		 const char *module, int errorID, bool printCR)
{
...
/**********************************************/
void
CL_PrintWarningID (Environment * theEnv,
		   const char *module, int warningID, bool printCR)
{
#if (! RUN_TIME) && (! BLOAD_ONLY)
  CL_FlushParsingMessages (theEnv);
...
/***************************************************/
void
CL_CantFindItemErrorMessage (Environment * theEnv,
			     const char *itemType,
			     const char *itemName, bool useQuotes)
{
...
/*****************************************************/
void
CL_CantFindItemInFunctionErrorMessage (Environment * theEnv,
				       const char *itemType,
				       const char *itemName,
				       const char *func, bool useQuotes)
{
  CL_PrintErrorID (theEnv, "PRNTUTIL", 1, false);
  CL_WriteString (theEnv, STDERR, "Unable to find ");
  CL_WriteString (theEnv, STDERR, itemType);
...
/*****************************************************/
void
CL_CantDeleteItemErrorMessage (Environment * theEnv,
			       const char *itemType, const char *itemName)
{
...
/****************************************************/
void
CL_AlreadyParsedErrorMessage (Environment * theEnv,
			      const char *itemType, const char *itemName)
{
...
/*********************************************************/
void
CL_SyntaxErrorMessage (Environment * theEnv, const char *location)
{
...
/****************************************************/
void
CL_LocalVariableErrorMessage (Environment * theEnv, const char *byWhat)
{
...
/******************************************/
void
CL_SystemError (Environment * theEnv, const char *module, int errorID)
{
  ...
  CL_WriteString (theEnv, STDERR, APPLICATION_NAME);
  CL_WriteString (theEnv, STDERR,
		  " data structures are in an inconsistent or corrupted state.\n");
  CL_WriteString (theEnv, STDERR,
...
/*******************************************************/
void
CL_DivideByZeroErrorMessage (Environment * theEnv, const char *functionName)
{
...
/********************************************************/
void
CL_ArgumentOverUnderflowErrorMessage (Environment * theEnv,
				      const char *functionName, bool error)
{
  if (error)
    {
      CL_PrintErrorID (theEnv, "PRNTUTIL", 17, false);
      CL_WriteString (theEnv, STDERR,
		      "Over or underflow of long long integer in '");
      CL_WriteString (theEnv, STDERR, functionName);
      CL_WriteString (theEnv, STDERR, "' function.\n");
    }
  else
...
/*******************************************************/
const char *
CL_FloatToString (Environment * theEnv, double number)
{
  char floatString[40];
  int i;
  char x;
...
/*******************************************************************/
const char *
CL_LongIntegerToString (Environment * theEnv, long long number)
{
  char buffer[50];
  CLIPSLexeme *thePtr;
...
/******************************************************************/
const char *
CL_DataObjectToString (Environment * theEnv, UDFValue * theDO)
{
  CLIPSLexeme *thePtr;
  const char *theString;
  char *newString;
  ...
  switch (theDO->header->type)
      ...
      else
	{
	  prefix = "<Instance-";
	  theString =
	    CL_GetFull_InstanceName (theEnv, theDO->instanceValue)->contents;
	  postfix = ">";
	}
...
/************************************************************/
void
SalienceInfo_rmationError (Environment * theEnv,
...
/**********************************************************/
void
CL_SalienceRangeError (Environment * theEnv, int min, int max)
{
...
/***************************************************************/
void
CL_SalienceNonIntegerError (Environment * theEnv)
{
  CL_PrintErrorID (theEnv, "PRNTUTIL", 10, true);
  CL_WriteString (theEnv, STDERR,
		  "Salience value must be an integer value.\n");
}
...
/****************************************************/
void
CL_Fact_RetractedErrorMessage (Environment * theEnv, Fact * theFact)
{
  char tempBuffer[20];
...
/****************************************************/
void
CL_FactVarSlotErrorMessage1 (Environment * theEnv,
			     Fact * theFact, const char *varSlot)
{
  char tempBuffer[20];
  ...
  CL_WriteString (theEnv, STDERR,
		  " cannot be resolved because the referenced fact ");
  CL_gensprintf (tempBuffer, "f-%lld", theFact->factIndex);
  CL_WriteString (theEnv, STDERR, tempBuffer);
  CL_WriteString (theEnv, STDERR, " has been retracted.\n");
}
...
/****************************************************/
void
CL_FactVarSlotErrorMessage2 (Environment * theEnv,
			     Fact * theFact, const char *varSlot)
{
  char tempBuffer[20];
...
/******************************************************/
void
CL_InvalidVarSlotErrorMessage (Environment * theEnv, const char *varSlot)
{
  ...
  CL_WriteString (theEnv, STDERR, varSlot);
  CL_WriteString (theEnv, STDERR,
		  " is invalid because slot names must be symbols.\n");
}
...
/*******************************************************/
void
CL_InstanceVarSlotErrorMessage1 (Environment * theEnv,
				 Instance * theInstance, const char *varSlot)
{
  ...
  CL_WriteString (theEnv, STDERR, varSlot);
  CL_WriteString (theEnv, STDERR,
		  " cannot be resolved because the referenced instance [");
...
/************************************************/
void
CL_InstanceVarSlotErrorMessage2 (Environment * theEnv,
				 Instance * theInstance, const char *varSlot)
{
  ...
  CL_WriteString (theEnv, STDERR, varSlot);
  CL_WriteString (theEnv, STDERR,
		  " is invalid because the referenced instance [");
  CL_WriteString (theEnv, STDERR, theInstance->name->contents);
  CL_WriteString (theEnv, STDERR, "] does not contain the specified slot.\n");
}
...
/***************************************************/
void
CL_SlotExistError (Environment * theEnv, const char *sname, const char *func)
{
