...
static CLIPSLexeme *CheckDeftemplateAndSlotArguments (UDFContext *,
						      Deftemplate **);
static void FreeTemplateValueArray (Environment *, CLIPSValue *,
				    Deftemplate *);
static struct expr *ModAndDupParse (Environment *, struct expr *,
				    const char *, const char *);
#if (! RUN_TIME) && (! BLOAD_ONLY)
static CLIPSLexeme *FindTemplateForFactAddress (CLIPSLexeme *,
						struct lhsParseNode *);
#endif
...
/****************************************************************/
void
CL_DeftemplateFunctions (Environment * theEnv)
{
#if ! RUN_TIME
  CL_AddUDF (theEnv, "modify", "bf", 0, UNBOUNDED, "*;lf", CL_ModifyCommand,
	     "CL_ModifyCommand", NULL);
  CL_AddUDF (theEnv, "duplicate", "bf", 0, UNBOUNDED, "*;lf",
	     CL_DuplicateCommand, "CL_DuplicateCommand", NULL);

  CL_AddUDF (theEnv, "deftemplate-slot-names", "bm", 1, 1, "y",
	     CL_DeftemplateSlotNamesFunction,
	     "CL_DeftemplateSlotNamesFunction", NULL);
  CL_AddUDF (theEnv, "deftemplate-slot-default-value", "*", 2, 2, "y",
	     CL_Deftemplate_SlotDefaultValueFunction,
	     "CL_Deftemplate_SlotDefaultValueFunction", NULL);
  CL_AddUDF (theEnv, "deftemplate-slot-cardinality", "*", 2, 2, "y",
	     CL_Deftemplate_SlotCardinalityFunction,
	     "CL_Deftemplate_SlotCardinalityFunction", NULL);
  CL_AddUDF (theEnv, "deftemplate-slot-allowed-values", "*", 2, 2, "y",
	     CL_Deftemplate_SlotAllowedValuesFunction,
	     "CL_Deftemplate_SlotAllowedValuesFunction", NULL);
  CL_AddUDF (theEnv, "deftemplate-slot-range", "*", 2, 2, "y",
	     CL_Deftemplate_SlotRangeFunction,
	     "CL_Deftemplate_SlotRangeFunction", NULL);
  CL_AddUDF (theEnv, "deftemplate-slot-types", "*", 2, 2, "y",
	     CL_Deftemplate_SlotTypesFunction,
	     "CL_Deftemplate_SlotTypesFunction", NULL);

  CL_AddUDF (theEnv, "deftemplate-slot-multip", "b", 2, 2, "y",
	     CL_DeftemplateSlotMultiPFunction,
	     "CL_DeftemplateSlotMultiPFunction", NULL);
  CL_AddUDF (theEnv, "deftemplate-slot-singlep", "b", 2, 2, "y",
	     CL_DeftemplateSlotSinglePFunction,
	     "CL_DeftemplateSlotSinglePFunction", NULL);
  CL_AddUDF (theEnv, "deftemplate-slot-existp", "b", 2, 2, "y",
	     CL_Deftemplate_SlotExistPFunction,
	     "CL_Deftemplate_SlotExistPFunction", NULL);
  CL_AddUDF (theEnv, "deftemplate-slot-defaultp", "y", 2, 2, "y",
	     CL_Deftemplate_SlotDefaultPFunction,
	     "CL_Deftemplate_SlotDefaultPFunction", NULL);

  CL_AddUDF (theEnv, "deftemplate-slot-facet-existp", "b", 3, 3, "y",
	     CL_DeftemplateSlotFacetExistPFunction,
	     "CL_DeftemplateSlotFacetExistPFunction", NULL);

  CL_AddUDF (theEnv, "deftemplate-slot-facet-value", "*", 3, 3, "y",
	     CL_DeftemplateSlotFacetValueFunction,
	     "CL_DeftemplateSlotFacetValueFunction", NULL);
...
/***************************/
static void
FreeTemplateValueArray (Environment * theEnv,
			CLIPSValue * theValueArray, Deftemplate * templatePtr)
{
  unsigned short i;
  ...
  CL_rm (theEnv, theValueArray,
	 sizeof (CLIPSValue) * templatePtr->numberOfSlots);
}
...
/*************************************************************/
void
CL_ModifyCommand (Environment * theEnv,
		  UDFContext * context, UDFValue * returnValue)
{
  ...
  if (computeResult.header->type == CL_INTEGER_TYPE)
    {
  ...
  if (templatePtr->numberOfSlots == 0)
	...
	(CLIPSValue *) CL_gm2 (theEnv,
			       sizeof (void *) * templatePtr->numberOfSlots);
      changeMap =
	(char *) CL_gm2 (theEnv,
			 CountToBitMapSize (templatePtr->numberOfSlots));
      CL_ClearBitString ((void *) changeMap,
			 CountToBitMapSize (templatePtr->numberOfSlots));
    }
  ...
  while (testPtr != NULL)
    {
      ...
      if (testPtr->type == CL_INTEGER_TYPE)
	{
      ...
      else
	{
      ...
      if (oldFact->theProposition.contents[position].header->type !=
	  MULTIFIELD_TYPE)
	{
	 /*======================================================*/
	 ...
	 /*======================================================*/
	  ...
	  if ((testPtr->argList ==
	       NULL) ? true : (testPtr->argList->nextArg != NULL))
	    {
	      CL_MultiIntoSingleFieldSlotError (theEnv,
						CL_GetNthSlot (templatePtr,
							       position),
						templatePtr);
	      FreeTemplateValueArray (theEnv, theValueArray, templatePtr);
	      if (changeMap != NULL)
	 ...
	 /*=============================*/
	  ...
	  if (oldFact->theProposition.contents[position].value !=
	      computeResult.value)
	    {
	      replacementCount++;
      ...
      else
	{
	 ...
	 /*======================================*/
	  ...
	  Increment_ClearReadyLocks (theEnv);
	  CL_StoreInMultifield (theEnv, &computeResult, testPtr->argList,
				false);
	  Set_EvaluationError (theEnv, false);
	  Decrement_ClearReadyLocks (theEnv);
  ...
  if ((oldFact =
       CL_ReplaceFact (theEnv, oldFact, theValueArray, changeMap)) != NULL)
    {
...
/****************/
Fact *
CL_ReplaceFact (Environment * theEnv,
		Fact * oldFact, CLIPSValue * theValueArray, char *changeMap)
{
  size_t i;
  Fact *theFact;
  ...
  if (FactData (theEnv)->ListOfModifyFunctions != NULL)
	...
	{
	  (*theModifyFunction->func) (theEnv, oldFact, NULL,
				      theModifyFunction->context);
	}
    }
  ...
  for (i = 0; i < oldFact->theProposition.length; i++)
	...
	{
	  CL_AtomDeinstall (theEnv,
			    oldFact->theProposition.contents[i].header->type,
			    oldFact->theProposition.contents[i].value);

	  if (oldFact->theProposition.contents[i].header->type ==
	      MULTIFIELD_TYPE)
	    {
	      Multifield *theSegment =
		oldFact->theProposition.contents[i].multifieldValue;
  ...
  theFact =
    CL_AssertDriver (oldFact, oldFact->factIndex, factListPosition,
		     templatePosition, changeMap);
  ...
  if (FactData (theEnv)->ListOfModifyFunctions != NULL)
    {
      ...
      for (theModifyFunction = FactData (theEnv)->ListOfModifyFunctions;
	   ...
	   theModifyFunction = theModifyFunction->next)
	{
	  (*theModifyFunction->func) (theEnv, NULL, theFact,
				      theModifyFunction->context);
	}
    }
...
/*******************************************************************/
void
CL_DuplicateCommand (Environment * theEnv,
		     UDFContext * context, UDFValue * returnValue)
{
  long long factNum;
  ...
  while (testPtr != NULL)
      ...
      if (newFact->theProposition.contents[position].value != NULL)
	{
	 ...
	 /*=============================*/
	  ...
	  newFact->theProposition.contents[position].value =
	    computeResult.value;
	}
...
/****************************************************/
void
CL_DeftemplateSlotNamesFunction (Environment * theEnv,
				 UDFContext * context, UDFValue * returnValue)
{
  ...
  deftemplateName =
    CL_GetConstructName (context, "deftemplate-slot-names",
			 "deftemplate name");
  ...
  if (theDeftemplate == NULL)
    {
      CL_CantFindItemErrorMessage (theEnv, "deftemplate", deftemplateName,
				   true);
      return;
    }
...
/**********************************************/
void
CL_DeftemplateSlotNames (Deftemplate * theDeftemplate,
...
/*******************************************************/
void
CL_Deftemplate_SlotDefaultPFunction (Environment * theEnv,
				     UDFContext * context,
				     UDFValue * returnValue)
{
  ...
  defaultType =
    CL_Deftemplate_SlotDefaultP (theDeftemplate, slotName->contents);
...
/*************************************************/
DefaultType
CL_Deftemplate_SlotDefaultP (Deftemplate * theDeftemplate,
			     const char *slotName)
{
  struct templateSlot *theSlot;
  Environment *theEnv = theDeftemplate->header.env;
...
/*************************************************************/
void
CL_Deftemplate_SlotDefaultValueFunction (Environment * theEnv,
					 UDFContext * context,
					 UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
...
/******************************************************/
bool
CL_Deftemplate_SlotDefaultValue (Deftemplate * theDeftemplate,
				 const char *slotName, CLIPSValue * theValue)
{
  ...
  if (theSlot->noDefault)
	...
	(theEnv, theDeftemplate, theSlot, &tempDO, true))
    {
...
/**********************************************************/
void
CL_Deftemplate_SlotCardinalityFunction (Environment * theEnv,
					UDFContext * context,
					UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
...
/****************************************************/
bool
CL_Deftemplate_SlotCardinality (Deftemplate * theDeftemplate,
				const char *slotName,
				CLIPSValue * returnValue)
{
  struct templateSlot *theSlot;
  ...
  if (theSlot->constraints != NULL)
	...
	theSlot->constraints->minFields->value;
      returnValue->multifieldValue->contents[1].value =
	theSlot->constraints->maxFields->value;
    }
  else
    {
      returnValue->multifieldValue->contents[0].integerValue =
...
/************************************************************/
void
CL_Deftemplate_SlotAllowedValuesFunction (Environment * theEnv,
					  UDFContext * context,
					  UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
  ...
  CL_Deftemplate_SlotAllowedValues (theDeftemplate, slotName->contents,
				    &result);
...
/*******************************************************/
bool
CL_Deftemplate_SlotAllowedValues (Deftemplate * theDeftemplate,
				  const char *slotName,
				  CLIPSValue * returnValue)
{
  struct templateSlot *theSlot;
  ...
  if ((theSlot->constraints !=
       NULL) ? (theSlot->constraints->restrictionList == NULL) : true)
    {
      returnValue->value = FalseSymbol (theEnv);
      return true;
    }
...
/****************************************************/
void
CL_Deftemplate_SlotRangeFunction (Environment * theEnv,
				  UDFContext * context,
				  UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
...
/**********************************************/
bool
CL_Deftemplate_SlotRange (Deftemplate * theDeftemplate,
			  const char *slotName, CLIPSValue * returnValue)
{
  struct templateSlot *theSlot;
  Environment *theEnv = theDeftemplate->header.env;
  ...
  if ((theSlot->constraints == NULL) ? false :
       ...
       || theSlot->constraints->integersAllowed))
    {
      returnValue->value = CL_CreateMultifield (theEnv, 2L);
      returnValue->multifieldValue->contents[0].value =
	theSlot->constraints->minValue->value;
      returnValue->multifieldValue->contents[1].value =
	theSlot->constraints->maxValue->value;
    }
  else
    {
      returnValue->value = FalseSymbol (theEnv);
...
/****************************************************/
void
CL_Deftemplate_SlotTypesFunction (Environment * theEnv,
				  UDFContext * context,
				  UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
...
/**********************************************/
bool
CL_Deftemplate_SlotTypes (Deftemplate * theDeftemplate,
			  const char *slotName, CLIPSValue * returnValue)
{
  ...
  if ((theDeftemplate->implied) ||
      ((theSlot->constraints !=
	NULL) ? theSlot->constraints->anyAllowed : true))
    {
#if OBJECT_SYSTEM
      numTypes = 8;
#else
  ...
  if (allTypes || theSlot->constraints->instanceAddressesAllowed)
    {
      returnValue->multifieldValue->contents[i++].lexemeValue =
	CL_CreateSymbol (theEnv, "INSTANCE-ADDRESS");
    }

  if (allTypes || theSlot->constraints->instanceNamesAllowed)
    {
      returnValue->multifieldValue->contents[i++].lexemeValue =
	CL_CreateSymbol (theEnv, "INSTANCE-NAME");
    }
#endif
...
/*****************************************************/
void
CL_DeftemplateSlotMultiPFunction (Environment * theEnv,
				  UDFContext * context,
				  UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv,
		      CL_DeftemplateSlotMultiP (theDeftemplate,
						slotName->contents));
}
...
/***********************************************/
bool
CL_DeftemplateSlotMultiP (Deftemplate * theDeftemplate, const char *slotName)
{
  struct templateSlot *theSlot;
  Environment *theEnv = theDeftemplate->header.env;
...
/******************************************************/
void
CL_DeftemplateSlotSinglePFunction (Environment * theEnv,
				   UDFContext * context,
				   UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv,
		      CL_DeftemplateSlotSingleP (theDeftemplate,
						 slotName->contents));
}
...
/************************************************/
bool
CL_DeftemplateSlotSingleP (Deftemplate * theDeftemplate, const char *slotName)
{
  struct templateSlot *theSlot;
  Environment *theEnv = theDeftemplate->header.env;
...
/*****************************************************/
void
CL_Deftemplate_SlotExistPFunction (Environment * theEnv,
				   UDFContext * context,
				   UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv,
		      CL_Deftemplate_SlotExistP (theDeftemplate,
						 slotName->contents));
}
...
/***********************************************/
bool
CL_Deftemplate_SlotExistP (Deftemplate * theDeftemplate, const char *slotName)
{
  Environment *theEnv = theDeftemplate->header.env;
  ...
  /* list of slots defined for the deftemplate. */
   /*============================================*/
    ...
    if (CL_FindSlot (theDeftemplate, CL_CreateSymbol (theEnv, slotName), NULL)
	== NULL)
    {
      return false;
    }
...
/**********************************************************/
void
CL_DeftemplateSlotFacetExistPFunction (Environment * theEnv,
				       UDFContext * context,
				       UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
...
/****************************************************/
bool
CL_DeftemplateSlotFacetExistP (Environment * theEnv,
			       Deftemplate * theDeftemplate,
			       const char *slotName, const char *facetName)
{
  struct templateSlot *theSlot;
  CLIPSLexeme *facetHN;
...
/*********************************************************/
void
CL_DeftemplateSlotFacetValueFunction (Environment * theEnv,
				      UDFContext * context,
				      UDFValue * returnValue)
{
  Deftemplate *theDeftemplate;
  ...
  CL_DeftemplateSlotFacetValue (theEnv, theDeftemplate, slotName->contents,
				facetName.lexemeValue->contents, returnValue);
}
...
/****************************************************/
bool
CL_DeftemplateSlotFacetValue (Environment * theEnv,
			      Deftemplate * theDeftemplate,
			      const char *slotName,
			      const char *facetName, UDFValue * rv)
{
  struct templateSlot *theSlot;
  CLIPSLexeme *facetHN;
  struct expr *tempFacet;
...
/************************************************************/
static CLIPSLexeme *
CheckDeftemplateAndSlotArguments (UDFContext * context,
				  Deftemplate ** theDeftemplate)
{
  UDFValue theArg;
  const char *deftemplateName;
...
/***************************************************************/
bool
CL_UpdateModifyDuplicate (Environment * theEnv,
			  struct expr *top, const char *name, void *vTheLHS)
{
  ...
  while (tempArg != NULL)
    {
      ...
      if ((slotPtr =
	   CL_FindSlot (theDeftemplate, tempArg->lexemeValue, NULL)) == NULL)
	{
	  CL_InvalidDeftemplateSlotMessage (theEnv,
      ...
      if (slotPtr->multislot == false)
	{
	  if (tempArg->argList == NULL)
	    ...
	    {
	      if ((ExpressionUnknownFunctionType (tempArg->argList) &
		   SINGLEFIELD_BITS) == 0)
		{
		  CL_SingleField_SlotCardinalityError (theEnv,
						       slotPtr->slotName->
      ...
      if (CL_CheckRHS_SlotTypes (theEnv, tempArg->argList, slotPtr, name) ==
	  0)
	return false;
      ...
      tempArg->type = CL_INTEGER_TYPE;
      tempArg->value =
	CL_CreateInteger (theEnv,
			  (long
			   long) (CL_FindSlotPosition (theDeftemplate,
						       tempArg->lexemeValue) -
				  1));
...
/**************************************************/
static CLIPSLexeme *
FindTemplateForFactAddress (CLIPSLexeme * factAddress,
			    struct lhsParseNode *theLHS)
{
  ...
  if ((thePattern->pnType != SF_WILDCARD_NODE)
  ...
  if ((thePattern->pnType != SYMBOL_NODE) ||
      (thePattern->right != NULL) || (thePattern->bottom != NULL))
    {
      return NULL;
    }
...
/*******************************************/
struct expr *
CL_ModifyParse (Environment * theEnv,
...
/*************************************************/
struct expr *
CL_DuplicateParse (Environment * theEnv,
		   struct expr *top, const char *logicalName)
{
...
/*************************************************************/
static struct expr *
ModAndDupParse (Environment * theEnv,
		struct expr *top, const char *logicalName, const char *name)
{
  bool error = false;
  ...
  if ((theToken.tknType == SF_VARIABLE_TOKEN)
    ...
    {
      nextOne = CL_GenConstant (theEnv, CL_INTEGER_TYPE, theToken.value);
    }
  ...
  while (theToken.tknType != RIGHT_PARENTHESIS_TOKEN)
      ...
      while (!done)
	{
	  CL_SavePPBuffer (theEnv, " ");
	  newField =
	    Get_AssertArgument (theEnv, logicalName, &theToken, &error,
