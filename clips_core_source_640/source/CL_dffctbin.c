...
/********************************************/
void
CL_DeffactsBinarySetup (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, DFFCTBIN_DATA,
			      sizeof (struct deffactsBinaryData),
			      DeallocateDeffacts_BloadData);
#if BLOAD_AND_BSAVE
...
/********************************************************/
static void
DeallocateDeffacts_BloadData (Environment * theEnv)
{
  size_t space;

  space = DeffactsBinaryData (theEnv)->NumberOfDeffacts * sizeof (Deffacts);
  if (space != 0)
    CL_genfree (theEnv, DeffactsBinaryData (theEnv)->DeffactsArray, space);

  space =
    DeffactsBinaryData (theEnv)->NumberOf_DeffactsModules *
    sizeof (struct deffactsModule);
  if (space != 0)
    CL_genfree (theEnv, DeffactsBinaryData (theEnv)->ModuleArray, space);
}
	   ...
	   theDeffacts = CL_GetNextDeffacts (theEnv, theDeffacts))
	{
	 ...
	 /*============================================================*/
	  ...
	  ExpressionData (theEnv)->ExpressionCount +=
	    CL_ExpressionSize (theDeffacts->assertList);
...
CL_BsaveStorage (Environment * theEnv, FILE * fp)
{
  ...
  CL_Gen_Write (&DeffactsBinaryData (theEnv)->NumberOfDeffacts, sizeof (long),
		fp);
  CL_Gen_Write (&DeffactsBinaryData (theEnv)->NumberOf_DeffactsModules,
		sizeof (long), fp);
}
...
CL_BsaveBinaryItem (Environment * theEnv, FILE * fp)
{
  ...
  space =
    ...
    (DeffactsBinaryData (theEnv)->NumberOf_DeffactsModules *
     sizeof (struct bsave_DeffactsModule));
  CL_Gen_Write (&space, sizeof (size_t), fp);
  ...
  for (theModule = CL_GetNextDefmodule (theEnv, NULL);
	...
	(struct deffactsModule *) CL_GetModuleItem (theEnv, NULL,
      ...
      CL_Gen_Write (&temp_DeffactsModule,
		    sizeof (struct bsave_DeffactsModule), fp);
    }
  ...
  for (theModule = CL_GetNextDefmodule (theEnv, NULL);
       theModule != NULL; theModule = CL_GetNextDefmodule (theEnv, theModule))
    {
      ...
      for (theDeffacts = CL_GetNextDeffacts (theEnv, NULL);
	   ...
	   theDeffacts = CL_GetNextDeffacts (theEnv, theDeffacts))
	{
	  CL_Assign_BsaveConstructHeaderVals (&newDeffacts.header,
					      &theDeffacts->header);
	  if (theDeffacts->assertList != NULL)
	    {
	      newDeffacts.assertList =
		ExpressionData (theEnv)->ExpressionCount;
	      ExpressionData (theEnv)->ExpressionCount +=
		CL_ExpressionSize (theDeffacts->assertList);
	    }
...
CL_BloadStorage (Environment * theEnv)
{
  ...
  CL_GenReadBinary (theEnv, &DeffactsBinaryData (theEnv)->NumberOfDeffacts,
		    sizeof (long));
  ...
  DeffactsBinaryData (theEnv)->ModuleArray =
    (struct deffactsModule *) CL_genalloc (theEnv, space);
  ...
  DeffactsBinaryData (theEnv)->DeffactsArray =
    (Deffacts *) CL_genalloc (theEnv, space);
}
...
/***************************************************/
static void
Update_DeffactsModule (Environment * theEnv, void *buf, unsigned long obji)
{
...
/*********************************************/
static void
UpdateDeffacts (Environment * theEnv, void *buf, unsigned long obji)
{
  ...
  CL_UpdateConstructHeader (theEnv, &bdp->header,
			    ...
			    sizeof (Deffacts),
			    DeffactsBinaryData (theEnv)->DeffactsArray);
  DeffactsBinaryData (theEnv)->DeffactsArray[obji].assertList =
    ExpressionPointer (bdp->assertList);
}
...
/******************************************************/
void *
CL_Bload_DeffactsModuleReference (Environment * theEnv,
