...
static bool Output_ProfileInfo (Environment *, const char *,
				struct construct_ProfileInfo *, const char *,
...
/******************************************************/
void
CL_ConstructProfilingFunctionDefinitions (Environment * theEnv)
{
  struct userDataRecord profileDataInfo =
    { 0, CL_Create_ProfileData, CL_Delete_ProfileData };

  CL_AllocateEnvironmentData (theEnv, PROFLFUN_DATA,
			      sizeof (struct profileFunctionData), NULL);

  memcpy (&CL_ProfileFunctionData (theEnv)->CL_ProfileDataInfo,
	  &profileDataInfo, sizeof (struct userDataRecord));
...
#if ! RUN_TIME
  CL_AddUDF (theEnv, "profile", "v", 1, 1, "y", CL_ProfileCommand,
	     "CL_ProfileCommand", NULL);
  CL_AddUDF (theEnv, "profile-info", "v", 0, 0, NULL, CL_ProfileInfoCommand,
	     "CL_ProfileInfoCommand", NULL);
  CL_AddUDF (theEnv, "profile-reset", "v", 0, 0, NULL,
	     CL_Profile_ResetCommand, "CL_Profile_ResetCommand", NULL);

  CL_AddUDF (theEnv, "set-profile-percent-threshold", "d", 1, 1, "ld",
	     Set_ProfilePercentThresholdCommand,
	     "Set_ProfilePercentThresholdCommand", NULL);
  CL_AddUDF (theEnv, "get-profile-percent-threshold", "d", 0, 0, NULL,
	     CL_Get_ProfilePercentThresholdCommand,
	     "CL_Get_ProfilePercentThresholdCommand", NULL);
...
/**********************************/
void *
CL_Create_ProfileData (Environment * theEnv)
{
...
/**************************************/
void
CL_Delete_ProfileData (Environment * theEnv, void *theData)
{
...
/**************************************/
void
CL_ProfileCommand (Environment * theEnv,
  ...
  if (!CL_Profile (theEnv, argument))
    {
      CL_UDFInvalidArgumentMessage (context,
				    "symbol with value constructs, user-functions, or off");
      return;
    }
...
/******************************/
bool
CL_Profile (Environment * theEnv, const char *argument)
{
   /*======================================================*/
  ...
  /* don't profile constructs and user-defined functions. */
   /*======================================================*/
	...
	(CL_ProfileFunctionData (theEnv)->CL_ProfileEndTime -
	 CL_ProfileFunctionData (theEnv)->CL_ProfileStartTime);
      CL_ProfileFunctionData (theEnv)->CL_Profile_UserFunctions = false;
      CL_ProfileFunctionData (theEnv)->CL_ProfileConstructs = false;
    }
...
/******************************************/
void
CL_ProfileInfoCommand (Environment * theEnv,
		       UDFContext * context, UDFValue * returnValue)
{
  char buffer[512];
  ...
  if (CL_ProfileFunctionData (theEnv)->CL_Profile_UserFunctions
  ...
  if (CL_ProfileFunctionData (theEnv)->Last_ProfileInfo != NO_PROFILE)
      ...
      if (CL_ProfileFunctionData (theEnv)->Last_ProfileInfo == USER_FUNCTIONS)
	{
      ...
      else if (CL_ProfileFunctionData (theEnv)->Last_ProfileInfo ==
	       CONSTRUCTS_CODE)
	{
	  CL_WriteString (theEnv, STDOUT,
			  "Construct Name                           ");
  ...
  if (CL_ProfileFunctionData (theEnv)->Last_ProfileInfo == USER_FUNCTIONS)
    Output_UserFunctionsInfo (theEnv);
  if (CL_ProfileFunctionData (theEnv)->Last_ProfileInfo == CONSTRUCTS_CODE)
    OutputConstructsCodeInfo (theEnv);
}
...
/**********************************************/
void
Start_Profile (Environment * theEnv,
	       struct profileFrameInfo *theFrame,
	       struct userData **theList, bool checkFlag)
{
  ...
  if (CL_ProfileFunctionData (theEnv)->Active_ProfileFrame != NULL)
    {
      addTime =
	startTime -
	CL_ProfileFunctionData (theEnv)->Active_ProfileFrame->startTime;
      CL_ProfileFunctionData (theEnv)->Active_ProfileFrame->totalSelfTime +=
	addTime;
    }
...
/*******************************************/
void
CL_End_Profile (Environment * theEnv, struct profileFrameInfo *theFrame)
{
...
/******************************************/
static bool
Output_ProfileInfo (Environment * theEnv,
		    const char *itemName,
		    struct construct_ProfileInfo *profileInfo,
		    const char *printPrefixBefore,
		    const char *printPrefix,
		    const char *printPrefixAfter, const char **banner)
{
  ...
  if (CL_ProfileFunctionData (theEnv)->CL_ProfileTotalTime != 0.0)
    {
      percent =
	(profileInfo->totalSelfTime * 100.0) /
	CL_ProfileFunctionData (theEnv)->CL_ProfileTotalTime;
      if (percent < 0.005)
	percent = 0.0;
      percentWithKids =
	(profileInfo->totalWithChildrenTime * 100.0) /
	CL_ProfileFunctionData (theEnv)->CL_ProfileTotalTime;
      if (percentWithKids < 0.005)
	percentWithKids = 0.0;
    }

  if (percent < CL_ProfileFunctionData (theEnv)->PercentThreshold)
    return false;
...
/*******************************************/
void
CL_Profile_ResetCommand (Environment * theEnv,
			 UDFContext * context, UDFValue * returnValue)
{
  ...
  for (theDefclass = CL_GetNextDefclass (theEnv, NULL);
       ...
       theDefclass = CL_GetNextDefclass (theEnv, theDefclass))
    {
      ...
      for (handlerIndex = CL_GetNextDefmessageHandler (theDefclass, 0);
	   handlerIndex != 0;
	   handlerIndex =
	   CL_GetNextDefmessageHandler (theDefclass, handlerIndex))
	{
	  theHandler =
	    CL_GetDefmessageHandlerPointer (theDefclass, handlerIndex);
	  CL_Reset_ProfileInfo ((struct construct_ProfileInfo *)
				CL_TestUserData (CL_ProfileFunctionData
						 (theEnv)->CL_ProfileDataID,
...
/*************************************************/
void
CL_Reset_ProfileInfo (struct construct_ProfileInfo *profileInfo)
{
  if (profileInfo == NULL)
    return;
...
/****************************/
static void
Output_UserFunctionsInfo (Environment * theEnv)
{
  struct functionDefinition *theFunction;
  int i;
...
/*****************************/
static void
OutputConstructsCodeInfo (Environment * theEnv)
{
...
/*********************************************************/
void
Set_ProfilePercentThresholdCommand (Environment * theEnv,
				    UDFContext * context,
				    UDFValue * returnValue)
{
  ...
  if ((newThreshold < 0.0) || (newThreshold > 100.0))
	...
	CL_CreateFloat (theEnv,
			Set_ProfilePercentThreshold (theEnv, newThreshold));
    }
}
...
/****************************************************/
double
Set_ProfilePercentThreshold (Environment * theEnv, double value)
{
...
/*********************************************************/
void
CL_Get_ProfilePercentThresholdCommand (Environment * theEnv,
				       UDFContext * context,
				       UDFValue * returnValue)
{
  returnValue->floatValue =
    CL_CreateFloat (theEnv,
		    CL_ProfileFunctionData (theEnv)->PercentThreshold);
}
...
/****************************************************/
double
CL_Get_ProfilePercentThreshold (Environment * theEnv)
{
...
/**********************************************************/
const char *
Set_ProfileOutputString (Environment * theEnv, const char *value)
{
...
/******************************************************************/
static void
CL_Profile_ClearFunction (Environment * theEnv, void *context)
{
  struct functionDefinition *theFunction;
  int i;
