...
static const char *PrintFo_rmatFlag (UDFContext *, const char *, unsigned int,
				     int);
...
/**************************************/
void
CL_IOFunctionDefinitions (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, IO_FUNCTION_DATA,
			      sizeof (struct IOFunctionData), NULL);

#if IO_FUNCTIONS
  IOFunctionData (theEnv)->useFullCRLF = false;
  IOFunctionData (theEnv)->locale =
    CL_CreateSymbol (theEnv, setlocale (LC_ALL, NULL));
  IncrementLexemeCount (IOFunctionData (theEnv)->locale);
#endif

#if ! RUN_TIME
#if IO_FUNCTIONS
  CL_AddUDF (theEnv, "printout", "v", 1, UNBOUNDED, "*;ldsyn",
	     CL_PrintoutFunction, "CL_PrintoutFunction", NULL);
  CL_AddUDF (theEnv, "print", "v", 0, UNBOUNDED, NULL, CL_PrintFunction,
	     "CL_PrintFunction", NULL);
  CL_AddUDF (theEnv, "println", "v", 0, UNBOUNDED, NULL, CL_PrintlnFunction,
	     "CL_PrintlnFunction", NULL);
  CL_AddUDF (theEnv, "read", "synldfie", 0, 1, ";ldsyn", CL_ReadFunction,
	     "CL_ReadFunction", NULL);
  CL_AddUDF (theEnv, "open", "b", 2, 3, "*;sy;ldsyn;s", CL_OpenFunction,
	     "CL_OpenFunction", NULL);
  CL_AddUDF (theEnv, "close", "b", 0, 1, "ldsyn", CL_CloseFunction,
	     "CL_CloseFunction", NULL);
  CL_AddUDF (theEnv, "flush", "b", 0, 1, "ldsyn", CL_FlushFunction,
	     "CL_FlushFunction", NULL);
  CL_AddUDF (theEnv, "rewind", "b", 1, 1, ";ldsyn", CL_RewindFunction,
	     "CL_RewindFunction", NULL);
  CL_AddUDF (theEnv, "tell", "lb", 1, 1, ";ldsyn", CL_TellFunction,
	     "CL_TellFunction", NULL);
  CL_AddUDF (theEnv, "seek", "b", 3, 3, ";ldsyn;l;y", CL_SeekFunction,
	     "CL_SeekFunction", NULL);
  CL_AddUDF (theEnv, "get-char", "l", 0, 1, ";ldsyn", CL_GetCharFunction,
	     "CL_GetCharFunction", NULL);
  CL_AddUDF (theEnv, "unget-char", "l", 1, 2, ";ldsyn;l",
	     CL_UngetCharFunction, "CL_UngetCharFunction", NULL);
  CL_AddUDF (theEnv, "put-char", "v", 1, 2, ";ldsyn;l", CL_PutCharFunction,
	     "CL_PutCharFunction", NULL);
  CL_AddUDF (theEnv, "remove", "b", 1, 1, "sy", CL_RemoveFunction,
	     "CL_RemoveFunction", NULL);
  CL_AddUDF (theEnv, "rename", "b", 2, 2, "sy", CL_RenameFunction,
	     "CL_RenameFunction", NULL);
  CL_AddUDF (theEnv, "fo_rmat", "s", 2, UNBOUNDED, "*;ldsyn;s",
	     CL_Fo_rmatFunction, "CL_Fo_rmatFunction", NULL);
  CL_AddUDF (theEnv, "readline", "sy", 0, 1, ";ldsyn", CL_ReadlineFunction,
	     "CL_ReadlineFunction", NULL);
  CL_AddUDF (theEnv, "set-locale", "sy", 0, 1, ";s", CL_SetLocaleFunction,
	     "CL_SetLocaleFunction", NULL);
  CL_AddUDF (theEnv, "read-number", "syld", 0, 1, ";ldsyn",
	     CL_ReadNumberFunction, "CL_ReadNumberFunction", NULL);
  CL_AddUDF (theEnv, "chdir", "b", 0, 1, "sy", CL_ChdirFunction,
	     "CL_ChdirFunction", NULL);
#endif
#else
#if MAC_XCD
...
/******************************************/
void
CL_PrintoutFunction (Environment * theEnv,
...
/*************************************/
void
CL_PrintFunction (Environment * theEnv,
		  UDFContext * context, UDFValue * returnValue)
{
...
/*************************************/
void
CL_PrintlnFunction (Environment * theEnv,
		    UDFContext * context, UDFValue * returnValue)
{
...
/*************************************************/
static void
PrintDriver (UDFContext * context, const char *logicalName, bool endCRLF)
{
  UDFValue theArg;
...
/*****************************************************/
bool
CL_SetFullCRLF (Environment * theEnv, bool value)
{
...
/*************************************************************/
void
CL_ReadFunction (Environment * theEnv,
		 UDFContext * context, UDFValue * returnValue)
{
  ...
  if ((theToken.tknType == FLOAT_TOKEN) || (theToken.tknType == STRING_TOKEN)
    ...
    {
      CL_SetErrorValue (theEnv,
			&CL_CreateSymbol (theEnv, "READ_ERROR")->header);
      returnValue->lexemeValue = FalseSymbol (theEnv);
    }
  else
    {
...
/********************************************************/
static void
ReadTokenFromStdin (Environment * theEnv, struct token *theToken)
{
  char *inputString;
  ...
  if (inputStringSize > 0)
    CL_rm (theEnv, inputString, inputStringSize);
...
/*************************************************************/
void
CL_OpenFunction (Environment * theEnv,
		 UDFContext * context, UDFValue * returnValue)
{
  ...
  if ((strcmp (accessMode, "r") != 0) &&
      ...
      (strcmp (accessMode, "a+b") != 0) && (strcmp (accessMode, "ab+")))
    {
      Set_HaltExecution (theEnv, true);
      Set_EvaluationError (theEnv, true);
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv,
		      CL_OpenAFile (theEnv, fileName, accessMode,
				    logicalName));
}
...
/*************************************************************/
void
CL_CloseFunction (Environment * theEnv,
		  UDFContext * context, UDFValue * returnValue)
{
  const char *logicalName;
  ...
  if (!UDFHasNextArgument (context))
    {
      returnValue->lexemeValue =
	CL_CreateBoolean (theEnv, CL_CloseAllFiles (theEnv));
      return;
    }
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_CloseFile (theEnv, logicalName));
}
...
/*************************************************************/
void
CL_FlushFunction (Environment * theEnv,
		  UDFContext * context, UDFValue * returnValue)
{
  const char *logicalName;
  ...
  if (!UDFHasNextArgument (context))
    {
      returnValue->lexemeValue =
	CL_CreateBoolean (theEnv, CL_FlushAllFiles (theEnv));
      return;
    }
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_FlushFile (theEnv, logicalName));
}
...
/***************************************************************/
void
CL_RewindFunction (Environment * theEnv,
		   UDFContext * context, UDFValue * returnValue)
{
  const char *logicalName;
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_RewindFile (theEnv, logicalName));
}
...
/***********************************************************/
void
CL_TellFunction (Environment * theEnv,
		 UDFContext * context, UDFValue * returnValue)
{
  const char *logicalName;
  long long rv;
...
/***********************************************************/
void
CL_SeekFunction (Environment * theEnv,
		 UDFContext * context, UDFValue * returnValue)
{
  const char *logicalName;
  UDFValue theArg;
  ...
  if (strcmp (seekCode, "seek-set") == 0)
    {
      returnValue->lexemeValue =
	CL_CreateBoolean (theEnv,
			  CL_SeekFile (theEnv, logicalName, offset,
				       SEEK_SET));
    }
...
/***************************************/
void
CL_GetCharFunction (Environment * theEnv,
		    UDFContext * context, UDFValue * returnValue)
{
  const char *logicalName;
  ...
  if (strcmp (logicalName, STDIN) == 0)
      ...
      if (RouterData (theEnv)->InputUngets > 0)
	{
	  returnValue->integerValue =
	    CL_CreateInteger (theEnv, CL_ReadRouter (theEnv, logicalName));
...
/*****************************************/
void
CL_UngetCharFunction (Environment * theEnv,
		      UDFContext * context, UDFValue * returnValue)
{
  ...
  returnValue->integerValue =
    CL_CreateInteger (theEnv,
		      CL_UnreadRouter (theEnv, logicalName, (int) theChar));
}
...
/***************************************/
void
CL_PutCharFunction (Environment * theEnv,
		    UDFContext * context, UDFValue * returnValue)
{
  unsigned int numberOfArguments;
  const char *logicalName;
...
/****************************************/
void
CL_RemoveFunction (Environment * theEnv,
		   UDFContext * context, UDFValue * returnValue)
{
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_genremove (theEnv, theFileName));
}
...
/****************************************/
void
CL_RenameFunction (Environment * theEnv,
		   UDFContext * context, UDFValue * returnValue)
{
  ...
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv,
		      CL_genrename (theEnv, oldFileName, newFileName));
}
...
/*************************************/
void
CL_ChdirFunction (Environment * theEnv,
		  UDFContext * context, UDFValue * returnValue)
{
  const char *theFileName;
  int success;
  ...
  switch (success)
    ...
    default:
      CL_WriteString (theEnv, STDERR,
		      "The chdir function is not supported on this system.\n");
      Set_HaltExecution (theEnv, true);
      Set_EvaluationError (theEnv, true);
      returnValue->lexemeValue = FalseSymbol (theEnv);
      break;
...
/****************************************/
void
CL_Fo_rmatFunction (Environment * theEnv,
		    UDFContext * context, UDFValue * returnValue)
{
  ...
  while (fo_rmatString[fo_rm_pos] != '\0')
      ...
      if (fo_rmatString[fo_rm_pos] != '%')
	{
	  ...
	  fstr =
	    CL_AppendNToString (theEnv, &fo_rmatString[start_pos], fstr,
				fo_rm_pos - start_pos, &fpos, &fmaxm);
	}
      else
	{
	  fo_rm_pos++;
	  fo_rmatFlagType =
	    FindFo_rmatFlag (fo_rmatString, &fo_rm_pos, percentBuffer,
			     FLAG_MAX);
	  if (fo_rmatFlagType != ' ')
	    {
	      if ((theString =
		   PrintFo_rmatFlag (context, percentBuffer, f_cur_arg,
				     fo_rmatFlagType)) == NULL)
		{
		  if (fstr != NULL)
	      ...
	      fstr =
		CL_AppendToString (theEnv, theString, fstr, &fpos, &fmaxm);
	      if (fstr == NULL)
		{
		  returnValue->value = hptr;
		  return;
	    ...
	    {
	      fstr =
		CL_AppendToString (theEnv, percentBuffer, fstr, &fpos,
				   &fmaxm);
	      if (fstr == NULL)
		{
		  returnValue->value = hptr;
  ...
  if (fstr != NULL)
    {
      ...
      if (strcmp (logicalName, "nil") != 0)
	CL_WriteString (theEnv, logicalName, fstr);
      CL_rm (theEnv, fstr, fmaxm);
    }
  else
    {
...
/*********************************************************************/
static const char *
ControlStringCheck (UDFContext * context, unsigned int argCount)
{
  UDFValue t_ptr;
...
/***********************************************/
static char
FindFo_rmatFlag (const char *fo_rmatString,
		 size_t *a, char *fo_rmatBuffer, size_t bufferMax)
{
  ...
  while ((fo_rmatString[*a] != '%') &&
	 (fo_rmatString[*a] != '\0') && (copy_pos < (bufferMax - 5)))
    {
      inchar = fo_rmatString[*a];
      (*a)++;

      if ((inchar == 'd') ||
	  (inchar == 'o') || (inchar == 'x') || (inchar == 'u'))
	{
...
/**********************************************************************/
static const char *
PrintFo_rmatFlag (UDFContext * context,
		  const char *fo_rmatString,
		  unsigned int whichArg, int fo_rmatType)
{
  UDFValue theResult;
  const char *theString;
  char *printBuffer;
  ...
  switch (fo_rmatType)
      ...
      else if (theResult.header->type == INTEGER_TYPE)
	{
	  ...
	  printBuffer = (char *) CL_gm2 (theEnv, (sizeof (char) * theLength));
	  CL_gensprintf (printBuffer, fo_rmatString,
			 (char) theResult.integerValue->contents);
	}
      else
	{
	  CL_ExpectedTypeError1 (theEnv, "fo_rmat", whichArg,
	...
	{
	  CL_gensprintf (printBuffer, fo_rmatString,
			 (long long) theResult.floatValue->contents);
	}
      else
	{
	  CL_gensprintf (printBuffer, fo_rmatString,
			 theResult.integerValue->contents);
	}
    ...
    default:
      CL_WriteString (theEnv, STDERR,
		      " Error in fo_rmat, the conversion character");
      CL_WriteString (theEnv, STDERR,
		      " for fo_rmatted output is not valid\n");
      return NULL;
    }
...
/******************************************/
void
CL_ReadlineFunction (Environment * theEnv,
		     UDFContext * context, UDFValue * returnValue)
{
  char *buffer;
  size_t line_max = 0;
  ...
  if (strcmp (logicalName, STDIN) == 0)
    {
      ...
      buffer =
	FillBuffer (theEnv, logicalName,
		    &RouterData (theEnv)->CommandBufferInputCount, &line_max);
...
/*************************************************************/
static char *
FillBuffer (Environment * theEnv,
	    const char *logicalName,
	    size_t *currentPosition, size_t *maximumSize)
{
  int c;
  char *buf = NULL;
  ...
  while ((c != '\n') && (c != '\r') && (c != EOF) &&
    ...
    {
      buf =
	CL_ExpandStringWithChar (theEnv, c, buf, currentPosition, maximumSize,
				 *maximumSize + 80);
      c = CL_ReadRouter (theEnv, logicalName);
    }
  ...
  buf =
    CL_ExpandStringWithChar (theEnv, EOS, buf, currentPosition, maximumSize,
			     *maximumSize + 80);
  return (buf);
}
...
/*****************************************/
void
CL_SetLocaleFunction (Environment * theEnv,
		      UDFContext * context, UDFValue * returnValue)
{
  UDFValue theArg;
...
/******************************************/
void
CL_ReadNumberFunction (Environment * theEnv,
		       UDFContext * context, UDFValue * returnValue)
{
  struct token theToken;
  const char *logicalName = NULL;
...
/********************************************/
static void
ReadNumber (Environment * theEnv,
	    const char *logicalName, struct token *theToken, bool isStdin)
{
  char *inputString;
  char *charPtr = NULL;
  size_t inputStringSize;
  ...
  while ((((!isStdin) && (!isspace (inchar))) ||
	  (isStdin && (inchar != '\n') && (inchar != '\r'))) &&
	 (inchar != EOF) && (!CL_Get_HaltExecution (theEnv)))
    {
      inputString =
	CL_ExpandStringWithChar (theEnv, inchar, inputString,
				 &RouterData
  ...
  if ((charPtr != inputString) && (isspace (*charPtr) || (*charPtr == '\0')))
    {
      theToken->tknType = INTEGER_TOKEN;
