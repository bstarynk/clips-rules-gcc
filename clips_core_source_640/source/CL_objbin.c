...
typedef struct bsaveSlotDescriptor
{
  ...
  unsigned long cls;
  unsigned long slotName, defaultValue, constraint, overrideMessage;
} BSAVE_SLOT_DESC;
...
static void CL_BsaveDefaultSlotExpressions (Environment *, ConstructHeader *,
					    void *);
static void CL_BsaveHandlerActionExpressions (Environment *,
					      ConstructHeader *, void *);
 ...
 ***********************************************************/
void
SetupObjects_Bload (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, OBJECTBIN_DATA,
			      sizeof (struct objectBinaryData),
			      DeallocateObjectBinaryData);

  CL_AddAbort_BloadFunction (theEnv, "defclass", CL_CreateSystemClasses, 0,
			     NULL);

#if BLOAD_AND_BSAVE
  CL_AddBinaryItem (theEnv, "defclass", 0, CL_BsaveObjectsFind,
		    CL_BsaveObjectsExpressions, CL_BsaveStorageObjects,
		    CL_BsaveObjects, CL_BloadStorageObjects, CL_BloadObjects,
...
/*******************************************************/
static void
DeallocateObjectBinaryData (Environment * theEnv)
{
  size_t space;
  unsigned long i;
  ...
  if (ObjectBinaryData (theEnv)->ClassCount != 0)
	...
	{
	  CL_rm (theEnv, DefclassData (theEnv)->ClassIDMap,
		 (sizeof (Defclass *) * DefclassData (theEnv)->AvailClassID));
	}
      ...
      for (i = 0L; i < ObjectBinaryData (theEnv)->SlotCount; i++)
	{
      ...
      if (space != 0L)
	{
	  CL_genfree (theEnv, ObjectBinaryData (theEnv)->DefclassArray,
		      space);
	}
      ...
      if (space != 0L)
	{
      ...
      space =
	(sizeof (SlotDescriptor) * ObjectBinaryData (theEnv)->SlotCount);
      if (space != 0L)
	{
      ...
      if (space != 0L)
	{
	  CL_genfree (theEnv, ObjectBinaryData (theEnv)->SlotNameArray,
		      space);
	}

      space =
	(sizeof (SlotDescriptor *) *
	 ObjectBinaryData (theEnv)->TemplateSlotCount);
      if (space != 0L)
	{
      ...
      space =
	(sizeof (unsigned) * ObjectBinaryData (theEnv)->SlotNameMapCount);
      if (space != 0L)
	{
  ...
  if (ObjectBinaryData (theEnv)->HandlerCount != 0L)
	...
	(sizeof (DefmessageHandler) *
	 ObjectBinaryData (theEnv)->HandlerCount);
      if (space != 0L)
	{
	  CL_genfree (theEnv, ObjectBinaryData (theEnv)->HandlerArray, space);
	  space =
	    (sizeof (unsigned) * ObjectBinaryData (theEnv)->HandlerCount);
	  CL_genfree (theEnv, ObjectBinaryData (theEnv)->MaphandlerArray,
		      space);
	}
    }
#endif
 ...
 ***************************************************/
void *
CL_Bload_DefclassModuleReference (Environment * theEnv,
 ...
 ***************************************************************************/
static void
CL_BsaveObjectsFind (Environment * theEnv)
{
  unsigned i;
  SLOT_NAME *snp;
...
MarkDefclassItems (Environment * theEnv,
		   ConstructHeader * theDefclass, void *buf)
{
#if MAC_XCD
#pragma unused(buf)
#endif
  ...
  CL_MarkConstructHeaderNeededItems (&cls->header,
				     ObjectBinaryData (theEnv)->ClassCount++);
  ObjectBinaryData (theEnv)->LinkCount +=
    cls->directSuperclasses.classCount + cls->directSubclasses.classCount +
      ...
      if (cls->slots[i].defaultValue != NULL)
	{
	  if (cls->slots[i].dynamicDefault)
		...
		CL_ExpressionSize ((Expression *) cls->slots[i].defaultValue);
	      CL_MarkNeededItems (theEnv,
				  (Expression *) cls->slots[i].defaultValue);
	    }
	  else
	    {
	      /* =================================================
  ...
  for (i = 0; i < cls->handlerCount; i++)
    {
      ...
      ExpressionData (theEnv)->ExpressionCount +=
	CL_ExpressionSize (cls->handlers[i].actions);
 ...
 ***************************************************/
static void
CL_BsaveObjectsExpressions (Environment * theEnv, FILE * fp)
{
  ...
  /* ================================================
     ...
     ================================================ */
  CL_DoForAllConstructs (theEnv, CL_BsaveDefaultSlotExpressions,
  ...
  /* ==============================================
     ...
     ============================================== */
  CL_DoForAllConstructs (theEnv, CL_BsaveHandlerActionExpressions,
			 DefclassData (theEnv)->CL_DefclassModuleIndex, false,
			 fp);
}
 ...
 ***************************************************/
static void
CL_BsaveDefaultSlotExpressions (Environment * theEnv,
				ConstructHeader * theDefclass, void *buf)
{
  Defclass *cls = (Defclass *) theDefclass;
  long i;
 ...
 ***************************************************/
static void
CL_BsaveHandlerActionExpressions (Environment * theEnv,
				  ConstructHeader * theDefclass, void *buf)
{
  Defclass *cls = (Defclass *) theDefclass;
  long i;
 ...
 *************************************************************************************/
static void
CL_BsaveStorageObjects (Environment * theEnv, FILE * fp)
{
  size_t space;
  ...
  CL_Gen_Write (&ObjectBinaryData (theEnv)->TemplateSlotCount, sizeof (long),
		fp);
  CL_Gen_Write (&ObjectBinaryData (theEnv)->SlotNameMapCount, sizeof (long),
		fp);
 ...
 *************************************************************************************/
static void
CL_BsaveObjects (Environment * theEnv, FILE * fp)
{
  size_t space;
  Defmodule *theModule;
  ...
  /* =====================
     ...
     ===================== */
  CL_DoForAllConstructs (theEnv, CL_BsaveDefclass,
			 DefclassData (theEnv)->CL_DefclassModuleIndex, false,
			 fp);
  ...
  /* =========================
     ...
     ========================= */
  ObjectBinaryData (theEnv)->LinkCount = 0L;
  CL_DoForAllConstructs (theEnv, CL_BsaveClassLinks,
			 DefclassData (theEnv)->CL_DefclassModuleIndex, false,
			 fp);
  ...
  /* ===================
     ...
     =================== */
  CL_DoForAllConstructs (theEnv, CL_BsaveSlots,
			 DefclassData (theEnv)->CL_DefclassModuleIndex, false,
			 fp);
  ...
  /* =====================================
     ...
     ===================================== */
  CL_DoForAllConstructs (theEnv, CL_BsaveTemplateSlots,
			 DefclassData (theEnv)->CL_DefclassModuleIndex, false,
			 fp);
  ...
  /* =============================================
     ...
     ============================================= */
  CL_DoForAllConstructs (theEnv, CL_BsaveSlotMap,
			 DefclassData (theEnv)->CL_DefclassModuleIndex, false,
			 fp);
  ...
  /* ==============================
     ...
     ============================== */
  CL_DoForAllConstructs (theEnv, CL_BsaveHandlers,
			 DefclassData (theEnv)->CL_DefclassModuleIndex, false,
			 fp);
  ...
  /* ==========================================
     ...
     ========================================== */
  CL_DoForAllConstructs (theEnv, CL_BsaveHandlerMap,
			 DefclassData (theEnv)->CL_DefclassModuleIndex, false,
			 fp);
 ...
 ***************************************************/
static void
CL_BsaveDefclass (Environment * theEnv,
		  ConstructHeader * theDefclass, void *buf)
{
  Defclass *cls = (Defclass *) theDefclass;
  ...
  dummy_class.directSuperclasses.classCount =
    cls->directSuperclasses.classCount;
  ...
  if (cls->directSuperclasses.classCount != 0)
    {
      dummy_class.directSuperclasses.classArray =
	ObjectBinaryData (theEnv)->LinkCount;
      ObjectBinaryData (theEnv)->LinkCount +=
	cls->directSuperclasses.classCount;
    }
  ...
  if (cls->directSubclasses.classCount != 0)
    {
      dummy_class.directSubclasses.classArray =
	ObjectBinaryData (theEnv)->LinkCount;
      ObjectBinaryData (theEnv)->LinkCount +=
  ...
  if (cls->allSuperclasses.classCount != 0)
    {
      dummy_class.allSuperclasses.classArray =
	ObjectBinaryData (theEnv)->LinkCount;
  ...
  if (cls->relevant_te_rminal_alpha_nodes != NULL)
    {
      dummy_class.relevant_te_rminal_alpha_nodes =
	cls->relevant_te_rminal_alpha_nodes->bsaveID;
    }
  else
    {
 ...
 ***************************************************/
static void
CL_BsaveClassLinks (Environment * theEnv,
		    ConstructHeader * theDefclass, void *buf)
{
  Defclass *cls = (Defclass *) theDefclass;
  unsigned long i;
  ...
  for (i = 0; i < cls->directSuperclasses.classCount; i++)
    {
      dummy_class_index =
	DefclassIndex (cls->directSuperclasses.classArray[i]);
 ...
 ***************************************************/
static void
CL_BsaveSlots (Environment * theEnv, ConstructHeader * theDefclass, void *buf)
{
  Defclass *cls = (Defclass *) theDefclass;
  long i;
  ...
  for (i = 0; i < cls->slotCount; i++)
      ...
      if (sp->defaultValue != NULL)
	{
	  ...
	  if (sp->dynamicDefault)
	    ExpressionData (theEnv)->ExpressionCount +=
	      CL_ExpressionSize ((Expression *) sp->defaultValue);
	  else
	    {
	      tmpexp =
		CL_ConvertValueToExpression (theEnv,
 ...
 **************************************************************/
static void
CL_BsaveTemplateSlots (Environment * theEnv,
		       ConstructHeader * theDefclass, void *buf)
{
  Defclass *cls = (Defclass *) theDefclass;
  unsigned long i;
 ...
 ***************************************************************/
static void
CL_BsaveSlotMap (Environment * theEnv,
		 ConstructHeader * theDefclass, void *buf)
{
  Defclass *cls = (Defclass *) theDefclass;
#if MAC_XCD
 ...
 ************************************************************/
static void
CL_BsaveHandlers (Environment * theEnv,
		  ConstructHeader * theDefclass, void *buf)
{
  Defclass *cls = (Defclass *) theDefclass;
  unsigned long i;
  ...
  for (i = 0; i < cls->handlerCount; i++)
    {
      ...
      CL_Assign_BsaveConstructHeaderVals (&dummy_handler.header,
					  &hnd->header);
 ...
 ****************************************************************/
static void
CL_BsaveHandlerMap (Environment * theEnv,
		    ConstructHeader * theDefclass, void *buf)
{
  Defclass *cls = (Defclass *) theDefclass;
#if MAC_XCD
 ...
 ***********************************************************************/
static void
CL_BloadStorageObjects (Environment * theEnv)
{
  size_t space;
  ...
  if (ObjectBinaryData (theEnv)->ModuleCount != 0L)
    {
      space =
	(sizeof (DEFCLASS_MODULE) * ObjectBinaryData (theEnv)->ModuleCount);
      ObjectBinaryData (theEnv)->ModuleArray =
	(DEFCLASS_MODULE *) CL_genalloc (theEnv, space);
    }
  if (ObjectBinaryData (theEnv)->ClassCount != 0L)
    {
      space = (sizeof (Defclass) * ObjectBinaryData (theEnv)->ClassCount);
      ObjectBinaryData (theEnv)->DefclassArray =
	(Defclass *) CL_genalloc (theEnv, space);
  ...
  if (ObjectBinaryData (theEnv)->LinkCount != 0L)
    {
      space = (sizeof (Defclass *) * ObjectBinaryData (theEnv)->LinkCount);
      ObjectBinaryData (theEnv)->LinkArray =
	(Defclass **) CL_genalloc (theEnv, space);
    }
  if (ObjectBinaryData (theEnv)->SlotCount != 0L)
    {
      space =
	(sizeof (SlotDescriptor) * ObjectBinaryData (theEnv)->SlotCount);
      ObjectBinaryData (theEnv)->SlotArray =
	(SlotDescriptor *) CL_genalloc (theEnv, space);
    }
  if (ObjectBinaryData (theEnv)->SlotNameCount != 0L)
    {
      space = (sizeof (SLOT_NAME) * ObjectBinaryData (theEnv)->SlotNameCount);
      ObjectBinaryData (theEnv)->SlotNameArray =
	(SLOT_NAME *) CL_genalloc (theEnv, space);
    }
  if (ObjectBinaryData (theEnv)->TemplateSlotCount != 0L)
	...
	(sizeof (SlotDescriptor *) *
	 ObjectBinaryData (theEnv)->TemplateSlotCount);
      ObjectBinaryData (theEnv)->TmpslotArray =
	(SlotDescriptor **) CL_genalloc (theEnv, space);
    }
  if (ObjectBinaryData (theEnv)->SlotNameMapCount != 0L)
    {
      space =
	(sizeof (unsigned) * ObjectBinaryData (theEnv)->SlotNameMapCount);
      ObjectBinaryData (theEnv)->MapslotArray =
	(unsigned *) CL_genalloc (theEnv, space);
    }
  if (ObjectBinaryData (theEnv)->HandlerCount != 0L)
    {
      space =
	(sizeof (DefmessageHandler) *
	 ObjectBinaryData (theEnv)->HandlerCount);
      ObjectBinaryData (theEnv)->HandlerArray =
	(DefmessageHandler *) CL_genalloc (theEnv, space);
      space = (sizeof (unsigned) * ObjectBinaryData (theEnv)->HandlerCount);
      ObjectBinaryData (theEnv)->MaphandlerArray =
	(unsigned *) CL_genalloc (theEnv, space);
    }
}
 ...
 **************************************************************/
static void
CL_BloadObjects (Environment * theEnv)
{
  size_t space;
  ...
  if (space == 0L)
    return;
  if (ObjectBinaryData (theEnv)->ModuleCount != 0L)
    CL_Bloadand_Refresh (theEnv, ObjectBinaryData (theEnv)->ModuleCount,
			 sizeof (BSAVE_DEFCLASS_MODULE),
			 Update_DefclassModule);
  if (ObjectBinaryData (theEnv)->ClassCount != 0L)
    {
      CL_Bloadand_Refresh (theEnv, ObjectBinaryData (theEnv)->ClassCount,
			   sizeof (BSAVE_DEFCLASS), UpdateDefclass);
      ...
      CL_Bloadand_Refresh (theEnv, ObjectBinaryData (theEnv)->SlotNameCount,
			   sizeof (BSAVE_SLOT_NAME), UpdateSlotName);
      CL_Bloadand_Refresh (theEnv, ObjectBinaryData (theEnv)->SlotCount,
			   sizeof (BSAVE_SLOT_DESC), UpdateSlot);
      if (ObjectBinaryData (theEnv)->TemplateSlotCount != 0L)
	CL_Bloadand_Refresh (theEnv,
			     ObjectBinaryData (theEnv)->TemplateSlotCount,
			     sizeof (long), UpdateTemplateSlot);
      if (ObjectBinaryData (theEnv)->SlotNameMapCount != 0L)
	{
	  space =
	    (sizeof (unsigned) * ObjectBinaryData (theEnv)->SlotNameMapCount);
	  CL_GenReadBinary (theEnv, ObjectBinaryData (theEnv)->MapslotArray,
			    space);
	}
      if (ObjectBinaryData (theEnv)->HandlerCount != 0L)
	{
	  CL_Bloadand_Refresh (theEnv,
			       ObjectBinaryData (theEnv)->HandlerCount,
			       sizeof (BSAVE_HANDLER), UpdateHandler);
	  space =
	    (sizeof (unsigned) * ObjectBinaryData (theEnv)->HandlerCount);
 ...
 ***************************************************/
static void
UpdatePrimitiveClassesMap (Environment * theEnv)
{
  unsigned i;

  for (i = 0; i < OBJECT_TYPE_CODE; i++)
    DefclassData (theEnv)->PrimitiveClassMap[i] =
      (Defclass *) & ObjectBinaryData (theEnv)->DefclassArray[i];
}
 ...
 *********************************************************/
static void
Update_DefclassModule (Environment * theEnv, void *buf, unsigned long obji)
{
...
static void
UpdateDefclass (Environment * theEnv, void *buf, unsigned long obji)
{
  BSAVE_DEFCLASS *bcls;
  Defclass *cls;
  ...
  cls->directSuperclasses.classArray =
    LinkPointer (bcls->directSuperclasses.classArray);
  cls->directSubclasses.classCount = bcls->directSubclasses.classCount;
  cls->directSubclasses.classArray =
    LinkPointer (bcls->directSubclasses.classArray);
  cls->allSuperclasses.classCount = bcls->allSuperclasses.classCount;
  cls->allSuperclasses.classArray =
    LinkPointer (bcls->allSuperclasses.classArray);
  ...
  cls->relevant_te_rminal_alpha_nodes =
    ClassAlphaPointer (bcls->relevant_te_rminal_alpha_nodes);
#if DEFMODULE_CONSTRUCT
...
static void
UpdateSlot (Environment * theEnv, void *buf, unsigned long obji)
{
  SlotDescriptor *sp;
  BSAVE_SLOT_DESC *bsp;
...
static void
UpdateSlotName (Environment * theEnv, void *buf, unsigned long obji)
{
  SLOT_NAME *snp;
...
static void
UpdateTemplateSlot (Environment * theEnv, void *buf, unsigned long obji)
{
  ObjectBinaryData (theEnv)->TmpslotArray[obji] = SlotPointer (*(long *) buf);
}

static void
UpdateHandler (Environment * theEnv, void *buf, unsigned long obji)
{
  DefmessageHandler *hnd;
  BSAVE_HANDLER *bhnd;
  ...
  CL_UpdateConstructHeader (theEnv, &bhnd->header, &hnd->header,
			    DEFMESSAGE_HANDLER, sizeof (DEFCLASS_MODULE),
			    ObjectBinaryData (theEnv)->ModuleArray,
			    sizeof (DefmessageHandler),
			    ObjectBinaryData (theEnv)->HandlerArray);
 ...
 ***************************************************************/
static void
CL_Clear_BloadObjects (Environment * theEnv)
{
  unsigned long i;
  size_t space;
