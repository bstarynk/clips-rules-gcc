...
static bool GetVariables (Environment *, struct lhsParseNode *, ParseNodeType,
			  struct nandFrame *);
static bool UnboundVariablesInPattern (Environment *, struct lhsParseNode *,
				       unsigned short);
static bool PropagateVariableToNodes (Environment *, struct lhsParseNode *,
				      ParseNodeType, CLIPSLexeme *,
				      struct lhsParseNode *, int, bool, bool);
static struct lhsParseNode *CheckExpression (Environment *,
					     struct lhsParseNode *,
					     struct lhsParseNode *,
					     unsigned short, CLIPSLexeme *,
					     unsigned short);
static void VariableReferenceErrorMessage (Environment *, CLIPSLexeme *,
					   struct lhsParseNode *, int,
					   CLIPSLexeme *, int);
static bool ProcessField (Environment *, struct lhsParseNode *,
			  struct lhsParseNode *, struct lhsParseNode *,
			  ParseNodeType, struct nandFrame *);
static bool ProcessVariable (Environment *, struct lhsParseNode *,
			     struct lhsParseNode *, struct lhsParseNode *,
			     ParseNodeType, struct nandFrame *);
...
static bool PropagateVariableDriver (Environment *,
				     ...
				     struct lhsParseNode *,
				     bool, ParseNodeType);
static bool TestCEAnalysis (Environment *, struct lhsParseNode *,
			    struct lhsParseNode *, bool, bool *,
			    struct nandFrame *);
...
/******************************************************************/
bool
CL_VariableAnalysis (Environment * theEnv, struct lhsParseNode *patternPtr)
{
  ...
  while (patternPtr != NULL)
      ...
      if (patternPtr->pnType == PATTERN_CE_NODE)
	{
	  ...
	  if ((patternPtr->value != NULL) &&
	      ...
	      if (patternPtr->referringNode->index == NO_INDEX)
		{
		  ...
		  CL_WriteString (theEnv, STDERR,
				  "Duplicate pattern-address ?");
		  CL_WriteString (theEnv, STDERR,
				  patternPtr->lexemeValue->contents);
	      ...
	      else
		{
		  ...
		  CL_WriteInteger (theEnv, STDERR, patternPtr->whichCE);
		  CL_WriteString (theEnv, STDERR,
				  " was previously bound within a pattern CE.\n");
		}
	    }
	 ...
	 /*====================================================*/
	  ...
	  if (GetVariables
	      (theEnv, patternPtr, PATTERN_CE_NODE, theNandFrames))
	    {
	 ...
	 /*==========================================================*/
	  ...
	  if (TestCEAnalysis
	      (theEnv, patternPtr, patternPtr->expression, false, &errorFlag,
	       theNandFrames) == true)
	    {
	      CL_ReleaseNandFrames (theEnv, theNandFrames);
	      return true;
	  ...
	  if (TestCEAnalysis
	      (theEnv, patternPtr, patternPtr->secondaryExpression, true,
	       &errorFlag, theNandFrames) == true)
	    {
	      CL_ReleaseNandFrames (theEnv, theNandFrames);
	      return true;
	    }
...
/******************************************************/
static void
CL_ReleaseNandFrames (Environment * theEnv, struct nandFrame *theFrames)
{
...
/*******************************************************************/
static bool
TestCEAnalysis (Environment * theEnv,
		struct lhsParseNode *patternPtr,
		struct lhsParseNode *theExpression,
		bool secondary,
		bool *errorFlag, struct nandFrame *theNandFrames)
{
  ...
  rv =
    CheckExpression (theEnv, theExpression, NULL, patternPtr->whichCE, NULL,
		     0);
  ...
  for (tempList = theList; tempList != NULL; tempList = tempList->right)
    {
      if (PropagateVariableDriver
	  (theEnv, patternPtr, patternPtr, NULL, SF_VARIABLE_NODE,
  ...
  if (rv != NULL)
    ...
    {
      patternPtr->secondaryNetworkTest =
	CL_CombineExpressions (theEnv, patternPtr->secondaryNetworkTest,
			       CL_GetvarReplace (theEnv, theExpression, false,
						 theNandFrames));
    }
  else
    {
      patternPtr->networkTest =
	CL_CombineExpressions (theEnv, patternPtr->networkTest,
			       CL_GetvarReplace (theEnv, theExpression, false,
						 theNandFrames));
    }
...
/****************************************************************/
static bool
GetVariables (Environment * theEnv,
	      struct lhsParseNode *thePattern,
	      ParseNodeType patternHeadType, struct nandFrame *theNandFrames)
{
  ...
  while (thePattern != NULL)
      ...
      if (thePattern != NULL)
	{
	  if ((thePattern->pnType == SF_VARIABLE_NODE) ||
	      ...
	      ((thePattern->pnType == PATTERN_CE_NODE)
	       && (thePattern->value != NULL)))
	    {
	      if (ProcessVariable
		  (theEnv, thePattern, multifieldHeader, patternHead,
		   patternHeadType, theNandFrames))
		{
		  return true;
		}
	    }
...
/******************************************************/
static bool
ProcessVariable (Environment * theEnv,
		 struct lhsParseNode *thePattern,
		 ...
		 struct nandFrame *theNandFrames)
{
  ...
  if (thePattern->pnType == PATTERN_CE_NODE)
    {
      ...
      if (thePattern->derivedConstraints)
	CL_RemoveConstraint (theEnv, thePattern->constraints);
  ...
  if (thePattern->pnType != PATTERN_CE_NODE)
    {
      PropagateVariableToNodes (theEnv, thePattern->bottom, theType,
				theVariable, thePattern,
				patternHead->beginNandDepth, true, false);

      if (ProcessField
	  (theEnv, thePattern, multifieldHeader, patternHead, patternHeadType,
	   theNandFrames))
	{
	  return true;
	}
  ...
  return (PropagateVariableDriver
	  (theEnv, patternHead, thePattern, multifieldHeader, theType,
...
/*******************************************/
static bool
PropagateVariableDriver (Environment * theEnv,
			 struct lhsParseNode *patternHead,
			 ...
			 bool assignReference, ParseNodeType patternHeadType)
{
   /*===================================================*/
  /* Propagate the variable location to any additional */
  ...
  if (multifieldHeader != NULL)
    {
      if (PropagateVariableToNodes
	  (theEnv, multifieldHeader->right, theType, variableName,
  ...
  if (PropagateVariableToNodes
      (theEnv, theNode->right, theType, variableName, theReference,
  ...
  if (PropagateVariableToNodes
      (theEnv, patternHead->expression, theType, variableName, theReference,
  ...
  if (PropagateVariableToNodes
      (theEnv, patternHead->secondaryExpression, theType, variableName,
       theReference, patternHead->beginNandDepth, assignReference, true))
    {
      return true;
    }
  ...
  if (((patternHead->pnType == PATTERN_CE_NODE)
       || (patternHead->pnType == TEST_CE_NODE))
      && (patternHead->negated == false) && (patternHead->exists == false)
      && (patternHead->beginNandDepth <= patternHead->endNandDepth))
    {
      ...
      if (patternHeadType == TEST_CE_NODE)
	ignoreVariableMixing = true;
      else
	ignoreVariableMixing = false;
      ...
      if (PropagateVariableToNodes
	  (theEnv, patternHead->bottom, theType, variableName, theReference,
...
/********************************************************/
static bool
ProcessField (Environment * theEnv,
	      struct lhsParseNode *thePattern,
	      struct lhsParseNode *multifieldHeader,
	      struct lhsParseNode *patternHead,
	      ParseNodeType patternHeadType, struct nandFrame *theNandFrames)
{
  ...
  for (tempList = theList; tempList != NULL; tempList = tempList->right)
    {
      if (PropagateVariableDriver
	  (theEnv, patternHead, thePattern, multifieldHeader,
	   tempList->pnType, tempList->lexemeValue, tempList, false,
	   patternHeadType))
	{
	  CL_ReturnLHSParseNodes (theEnv, theList);
	  return true;
  ...
  if (CL_ProcessConnectedConstraints
      (theEnv, thePattern, multifieldHeader, patternHead))
    {
      return true;
    }
...
/*************************************************************/
static bool
PropagateVariableToNodes (Environment * theEnv,
			  struct lhsParseNode *theNode,
			  ParseNodeType theType,
			  CLIPSLexeme * variableName,
			  struct lhsParseNode *theReference,
			  int startDepth,
			  bool assignReference, bool ignoreVariableTypes)
{
  ...
  while (theNode != NULL)
	...
	{
	  PropagateVariableToNodes (theEnv, theNode->expression, theType,
				    variableName, theReference, startDepth,
				    assignReference, true);
	}

      if (theNode->secondaryExpression != NULL)
	{
	  PropagateVariableToNodes (theEnv, theNode->secondaryExpression,
				    theType, variableName, theReference,
				    startDepth, assignReference, true);
	}
	    ...
	    && (theNode->value == (void *) variableName))
	{
	 ...
	 /*======================================================*/
	  ...
	  if (ignoreVariableTypes == false)
	    {
	      if (((theType == SF_VARIABLE_NODE)
		   && (theNode->pnType == MF_VARIABLE_NODE))
		  || ((theType == MF_VARIABLE_NODE)
	  ...
	  if ((theReference->constraints != NULL) && (!theNode->negated))
	    {
	      ...
	      theNode->constraints =
		CL_IntersectConstraints (theEnv, theReference->constraints,
					 tempConstraints);
      ...
      if (theNode->right != NULL)
	{
	  if (PropagateVariableToNodes
	      (theEnv, theNode->right, theType, variableName, theReference,
	       startDepth, assignReference, ignoreVariableTypes))
	    {
	      return true;
	    }
	}
      ...
      if ((theNode->pnType == PATTERN_CE_NODE)
	  || (theNode->pnType == TEST_CE_NODE))
	{
	  if (theNode->endNandDepth < startDepth)
	    theNode = NULL;
	  else
...
/*************************************************************/
static bool
UnboundVariablesInPattern (Environment * theEnv,
			   ...
			   unsigned short pattern)
{
  ...
  for (orField = theSlot->bottom; orField != NULL; orField = orField->bottom)
    {
      /*===========================================*/
      ...
      for (andField = orField; andField != NULL; andField = andField->right)
	{
	 /*=======================================================*/
	 ...
	 /*=======================================================*/
	  ...
	  if (((andField->pnType == SF_VARIABLE_NODE)
	       || (andField->pnType == MF_VARIABLE_NODE))
	      && (andField->referringNode == NULL))
	    {
	      VariableReferenceErrorMessage (theEnv, andField->lexemeValue,
					     NULL, pattern, slotName,
					     theField);
	      return true;
	    }
	 ...
	 /*==============================================*/
	    ...
	    {
	      rv =
		CheckExpression (theEnv, andField->expression, NULL, pattern,
				 slotName, theField);
	      if (rv != NULL)
		return true;
	    }
	 ...
	 /*========================================================*/
	    ...
	    if (((andField->pnType == INTEGER_NODE)
		 || (andField->pnType == FLOAT_NODE)
		 || (andField->pnType == SYMBOL_NODE)
		 || (andField->pnType == STRING_NODE)
		 || (andField->pnType == INSTANCE_NAME_NODE)))
	    {
	      result =
		CL_ConstraintCheckValue (theEnv, CL_NodeTypeToType (andField),
					 andField->value, theConstraints);
	      if (result != NO_VIOLATION)
		{
		  CL_ConstraintViolationErrorMessage (theEnv,
						      "A literal restriction value",
						      NULL, false, pattern,
						      slotName, theField,
						      result, theConstraints,
						      true);
		  return true;
		}
	    }
...
CheckExpression (Environment * theEnv,
		 struct lhsParseNode *exprPtr,
		 struct lhsParseNode *lastOne,
		 unsigned short whichCE,
		 CLIPSLexeme * slotName, unsigned short theField)
{
  struct lhsParseNode *rv;
  unsigned short i = 1;
  ...
  while (exprPtr != NULL)
      ...
      if (exprPtr->pnType == SF_VARIABLE_NODE)
	{
	  if (exprPtr->referringNode == NULL)
	    {
	      VariableReferenceErrorMessage (theEnv, exprPtr->lexemeValue,
					     lastOne, whichCE, slotName,
					     theField);
	      return exprPtr;
	    }
	  else if (CL_UnmatchableConstraint (exprPtr->constraints))
	    {
	      CL_ConstraintReferenceErrorMessage (theEnv,
						  exprPtr->lexemeValue,
						  lastOne, i, whichCE,
						  slotName, theField);
	      return exprPtr;
      ...
      else if ((exprPtr->pnType == MF_VARIABLE_NODE)
	       && (exprPtr->referringNode == NULL))
	{
	  VariableReferenceErrorMessage (theEnv, exprPtr->lexemeValue,
					 lastOne, whichCE, slotName,
					 theField);
      ...
      else if (exprPtr->pnType == GBL_VARIABLE_NODE)
	{
	  ...
	  if (CL_FindImportedConstruct
	      (theEnv, "defglobal", NULL, exprPtr->lexemeValue->contents,
	...
	{
	  if ((rv =
	       CheckExpression (theEnv, exprPtr->bottom, exprPtr, whichCE,
				slotName, theField)) != NULL)
	    {
	      return rv;
	    }
	}
...
/********************************************************/
static void
VariableReferenceErrorMessage (Environment * theEnv,
			       CLIPSLexeme * theVariable,
			       struct lhsParseNode *theExpression,
			       int whichCE,
...
/************************************************************/
static void
VariableMixingErrorMessage (Environment * theEnv, CLIPSLexeme * theVariable)
{
  CL_PrintErrorID (theEnv, "ANALYSIS", 3, true);
  CL_WriteString (theEnv, STDERR, "Variable ?");
  CL_WriteString (theEnv, STDERR, theVariable->contents);
  CL_WriteString (theEnv, STDERR,
		  " is used as both a single and multifield variable in the LHS.\n");
}
