...
#if (! BLOAD_ONLY) && (! RUN_TIME)
static bool Wild_DeleteHandler (Environment *, Defclass *, CLIPSLexeme *,
				const char *);
#endif

#if DEBUGGING_FUNCTIONS
static bool DefmessageHandler_WatchAccess (Environment *, int, bool,
					   Expression *);
static bool DefmessageHandler_WatchPrint (Environment *, const char *, int,
					  Expression *);
static bool DefmessageHandler_WatchSupport (Environment *, const char *,
					    const char *, bool,
					    void (*)(Environment *,
						     const char *, Defclass *,
						     unsigned),
					    void (*)(Defclass *, unsigned,
						     bool), Expression *);
static bool CL_WatchClassHandlers (Environment *, Defclass *, const char *,
				   int, const char *, bool, bool,
				   void (*)(Environment *, const char *,
					    Defclass *, unsigned),
				   void (*)(Defclass *, unsigned, bool));
static void CL_PrintHandler_WatchFlag (Environment *, const char *,
				       Defclass *, unsigned);
#endif
 ...
 ***************************************************/
void
CL_SetupMessageHandlers (Environment * theEnv)
{
  ...
  CL_AllocateEnvironmentData (theEnv, MESSAGE_HANDLER_DATA,
			      sizeof (struct messageHandlerData),
			      DeallocateMessageHandlerData);
  memcpy (&MessageHandlerData (theEnv)->HandlerGetInfo, &handlerGetInfo,
	  sizeof (struct entityRecord));
  memcpy (&MessageHandlerData (theEnv)->HandlerPutInfo, &handlerPutInfo,
	  sizeof (struct entityRecord));
  ...
  CL_InstallPrimitive (theEnv, &MessageHandlerData (theEnv)->HandlerGetInfo,
		       HANDLER_GET);
  CL_InstallPrimitive (theEnv, &MessageHandlerData (theEnv)->HandlerPutInfo,
		       HANDLER_PUT);

#if ! RUN_TIME
  MessageHandlerData (theEnv)->INIT_SYMBOL =
    CL_CreateSymbol (theEnv, INIT_STRING);
  IncrementLexemeCount (MessageHandlerData (theEnv)->INIT_SYMBOL);

  MessageHandlerData (theEnv)->DELETE_SYMBOL =
    CL_CreateSymbol (theEnv, DELETE_STRING);
  IncrementLexemeCount (MessageHandlerData (theEnv)->DELETE_SYMBOL);

  MessageHandlerData (theEnv)->CREATE_SYMBOL =
    CL_CreateSymbol (theEnv, CREATE_STRING);
  ...
  MessageHandlerData (theEnv)->SELF_SYMBOL =
    CL_CreateSymbol (theEnv, SELF_STRING);
  IncrementLexemeCount (MessageHandlerData (theEnv)->SELF_SYMBOL);

  CL_AddConstruct (theEnv, "defmessage-handler", "defmessage-handlers",
		   CL_ParseDefmessageHandler, NULL, NULL, NULL, NULL, NULL,
		   NULL, NULL, NULL, NULL);
  CL_AddUDF (theEnv, "undefmessage-handler", "v", 2, 3, "y",
	     CL_UndefmessageHandlerCommand, "CL_UndefmessageHandlerCommand",
	     NULL);

#endif

  CL_AddUDF (theEnv, "send", "*", 2, UNBOUNDED, "*;*;y", CL_SendCommand,
	     "CL_SendCommand", NULL);

#if DEBUGGING_FUNCTIONS
  CL_AddUDF (theEnv, "preview-send", "v", 2, 2, "y", CL_Preview_SendCommand,
	     "CL_Preview_SendCommand", NULL);

  CL_AddUDF (theEnv, "ppdefmessage-handler", "v", 2, 4, "y",
	     CL_PPDefmessageHandlerCommand, "CL_PPDefmessageHandlerCommand",
	     NULL);
  CL_AddUDF (theEnv, "list-defmessage-handlers", "v", 0, 2, "y",
	     CL_ListDefmessageHandlersCommand,
	     "CL_ListDefmessageHandlersCommand", NULL);
#endif

  CL_AddUDF (theEnv, "next-handlerp", "b", 0, 0, NULL,
	     CL_NextHandlerAvailableFunction,
	     "CL_NextHandlerAvailableFunction", NULL);
  CL_FuncSeqOvlFlags (theEnv, "next-handlerp", true, false);
  CL_AddUDF (theEnv, "call-next-handler", "*", 0, 0, NULL, CL_CallNextHandler,
	     "CL_CallNextHandler", NULL);
  CL_FuncSeqOvlFlags (theEnv, "call-next-handler", true, false);
  CL_AddUDF (theEnv, "override-next-handler", "*", 0, UNBOUNDED, NULL,
	     CL_CallNextHandler, "CL_CallNextHandler", NULL);
  CL_FuncSeqOvlFlags (theEnv, "override-next-handler", true, false);

  CL_AddUDF (theEnv, "dynamic-get", "*", 1, 1, "y", CL_DynamicHandlerGetSlot,
	     "CL_DynamicHandlerGetSlot", NULL);
  CL_AddUDF (theEnv, "dynamic-put", "*", 1, UNBOUNDED, "*;y",
	     CL_DynamicHandlerPutSlot, "CL_DynamicHandlerPutSlot", NULL);
  CL_AddUDF (theEnv, "get", "*", 1, 1, "y", CL_DynamicHandlerGetSlot,
	     "CL_DynamicHandlerGetSlot", NULL);
  CL_AddUDF (theEnv, "put", "*", 1, UNBOUNDED, "*;y",
	     CL_DynamicHandlerPutSlot, "CL_DynamicHandlerPutSlot", NULL);
#endif

#if DEBUGGING_FUNCTIONS
  CL_Add_WatchItem (theEnv, "messages", 0,
		    &MessageHandlerData (theEnv)->CL_WatchMessages, 36, NULL,
		    NULL);
  CL_Add_WatchItem (theEnv, "message-handlers", 0,
		    &MessageHandlerData (theEnv)->CL_WatchHandlers, 35,
		    DefmessageHandler_WatchAccess,
		    DefmessageHandler_WatchPrint);
#endif
}
...
/******************************************************/
static void
DeallocateMessageHandlerData (Environment * theEnv)
{
 ...
 *****************************************************/
const char *
CL_DefmessageHandlerName (Defclass * theDefclass, unsigned theIndex)
{
 ...
 *****************************************************/
const char *
CL_Defmessage_HandlerType (Defclass * theDefclass, unsigned theIndex)
{
 ...
 **************************************************************/
unsigned
CL_GetNextDefmessageHandler (Defclass * theDefclass, unsigned theIndex)
{
  if (theIndex == 0)
    {
 ...
 *****************************************************/
DefmessageHandler *
CL_GetDefmessageHandlerPointer (Defclass * theDefclass, unsigned int theIndex)
{
 ...
 *********************************************************/
bool
CL_DefmessageHandlerGet_Watch (Defclass * theDefclass, unsigned theIndex)
{
 ...
 *********************************************************/
void
CL_DefmessageHandlerSet_Watch (Defclass * theClass,
			       unsigned theIndex, bool newState)
{
 ...
 ***************************************************/
unsigned
CL_FindDefmessageHandler (Defclass * theDefclass,
			  const char *hname, const char *htypestr)
{
  unsigned htype;
  CLIPSLexeme *hsym;
  int theIndex;
 ...
 ***************************************************/
bool
CL_DefmessageHandlerIsDeletable (Defclass * theDefclass, unsigned theIndex)
{
  Environment *theEnv = theDefclass->header.env;
 ...
 ******************************************************************************/
void
CL_UndefmessageHandlerCommand (Environment * theEnv,
			       UDFContext * context, UDFValue * returnValue)
{
  ...
  if ((cls == NULL) ? (strcmp (theArg.lexemeValue->contents, "*") !=
		       0) : false)
    {
      CL_ClassExistError (theEnv, "undefmessage-handler",
 ...
 ***********************************************************/
bool
CL_UndefmessageHandler (Defclass * theDefclass,
			unsigned mhi, Environment * allEnv)
{
  Environment *theEnv;
#if (! RUN_TIME) &&  (! BLOAD_ONLY)
  bool success;
  ...
  if (theDefclass == NULL)
      ...
      if (mhi != 0)
	{
	  ...
	  CL_WriteString (theEnv, STDERR,
			  "Incomplete message-handler specification for deletion.\n");
 ...
 *******************************************************************************/
void
CL_PPDefmessageHandlerCommand (Environment * theEnv,
			       UDFContext * context, UDFValue * returnValue)
{
  UDFValue theArg;
  ...
  if (((cls == NULL) || (msym == NULL)) ? true :
      ((hnd = CL_FindHandlerByAddress (cls, msym, mtype)) == NULL))
    {
      ...
      CL_WriteString (theEnv, STDERR,
		      "' in function 'ppdefmessage-handler'.\n");
      Set_EvaluationError (theEnv, true);
      return;
    }
  ...
  if (strcmp (logicalName, "nil") == 0)
      ...
      if (hnd->header.ppFo_rm != NULL)
	{
	  returnValue->lexemeValue =
	    CL_CreateString (theEnv, hnd->header.ppFo_rm);
	}
      else
	{
 ...
 *****************************************************************************/
void
CL_ListDefmessageHandlersCommand (Environment * theEnv,
				  UDFContext * context,
				  UDFValue * returnValue)
{
  ...
  if (CL_UDFArgumentCount (context) == 0)
    ...
    {
      clsptr =
	CL_ClassInfoFnxArgs (context, "list-defmessage-handlers", &inhp);
      if (clsptr == NULL)
	return;
 ...
 ********************************************************************/
void
CL_Preview_SendCommand (Environment * theEnv,
			UDFContext * context, UDFValue * returnValue)
{
  Defclass *cls;
  UDFValue theArg;
  ...
  if (cls == NULL)
    {
      CL_ClassExistError (theEnv, "preview-send",
			  theArg.lexemeValue->contents);
      return;
    }
 ...
 ********************************************************/
const char *
CL_DefmessageHandlerPPFo_rm (Defclass * theDefclass, unsigned theIndex)
{
 ...
 *******************************************************************/
void
CL_ListDefmessageHandlers (Environment * theEnv,
			   Defclass * theDefclass,
			   const char *logName, bool inhp)
{
  unsigned long cnt;
  ...
  if (theDefclass != NULL)
	...
	{
	  cnt =
	    CL_DisplayHandlersInLinks (theEnv, logName,
 ...
 ********************************************************************/
void
CL_Preview_Send (Defclass * theDefclass,
		 const char *logicalName, const char *msgname)
{
  HANDLER_LINK *core;
  CLIPSLexeme *msym;
  Environment *theEnv = theDefclass->header.env;
 ...
 ****************************************************/
unsigned long
CL_DisplayHandlersInLinks (Environment * theEnv,
			   const char *logName,
			   PACKED_CLASS_LINKS * plinks, unsigned int theIndex)
{
  unsigned long i;
  unsigned long cnt;
  ...
  for (i = 0; i < plinks->classArray[theIndex]->handlerCount; i++)
    CL_PrintHandler (theEnv, logName,
		     &plinks->classArray[theIndex]->handlers[i], false, true);
  return cnt;
}
 ...
 **********************************************************/
static void
CreateSystemHandlers (Environment * theEnv, void *context)
{
  CL_NewSystemHandler (theEnv, USER_TYPE_NAME, INIT_STRING, "init-slots", 0);
  CL_NewSystemHandler (theEnv, USER_TYPE_NAME, DELETE_STRING,
		       "delete-instance", 0);
  CL_NewSystemHandler (theEnv, USER_TYPE_NAME, CREATE_STRING,
		       "(create-instance)", 0);
  ...
  CL_NewSystemHandler (theEnv, USER_TYPE_NAME, DIRECT_MODIFY_STRING,
		       "(direct-modify)", 1);
  CL_NewSystemHandler (theEnv, USER_TYPE_NAME, MSG_MODIFY_STRING,
		       "(message-modify)", 1);
  CL_NewSystemHandler (theEnv, USER_TYPE_NAME, DIRECT_DUPLICATE_STRING,
		       "(direct-duplicate)", 2);
  CL_NewSystemHandler (theEnv, USER_TYPE_NAME, MSG_DUPLICATE_STRING,
		       "(message-duplicate)", 2);
}
 ...
 ************************************************************/
static bool
Wild_DeleteHandler (Environment * theEnv,
		    Defclass * cls, CLIPSLexeme * msym, const char *tname)
{
  int mtype;
  ...
  if (tname != NULL)
    {
      mtype =
	(int) CL_HandlerType (theEnv, "undefmessage-handler", true, tname);
      if (mtype == MERROR)
	return false;
    }
  else
 ...
 ******************************************************************/
static bool
DefmessageHandler_WatchAccess (Environment * theEnv,
 ...
 ***********************************************************************/
static bool
DefmessageHandler_WatchPrint (Environment * theEnv,
			      const char *logName,
...
#pragma unused(code)
#endif
  return DefmessageHandler_WatchSupport (theEnv, "list-watch-items", logName,
					 false, CL_PrintHandler_WatchFlag,
					 NULL, argExprs);
}
 ...
 *******************************************************/
static bool
DefmessageHandler_WatchSupport (Environment * theEnv,
				const char *funcName,
				const char *logName,
				bool newState,
				void (*printFunc) (Environment *,
						   const char *, Defclass *,
						   unsigned),
				void (*traceFunc) (Defclass *, unsigned,
						   bool),
				Expression * argExprs)
{
  Defmodule *theModule;
  ...
  if (argExprs == NULL)
      ...
      while (theModule != NULL)
	{
	  ...
	  while (theClass != NULL)
	    {
	      if (CL_WatchClassHandlers
		  (theEnv, theClass, NULL, -1, logName, newState, true,
		   printFunc, traceFunc) == false)
		return false;
  ...
  while (argExprs != NULL)
      ...
      if (tmpData.header->type != SYMBOL_TYPE)
	{
      ...
      theClass =
	CL_LookupDefclassByMdlOrScope (theEnv, tmpData.lexemeValue->contents);
      if (theClass == NULL)
	{
	  CL_ExpectedTypeError1 (theEnv, funcName, argIndex, "'class name'");
	  return false;
      ...
      if (GetNextArgument (argExprs) != NULL)
	{
	  ...
	  if (GetNextArgument (argExprs) != NULL)
	      ...
	      if (tmpData.header->type != SYMBOL_TYPE)
		{
	      ...
	      if ((theType =
		   (int) CL_HandlerType (theEnv, funcName, true,
					 tmpData.lexemeValue->contents)) ==
		  MERROR)
		return false;
	    }
	  else
      ...
      else
	{
      ...
      if (CL_WatchClassHandlers
	  (theEnv, theClass, theHandlerStr, theType, logName, newState, false,
	   printFunc, traceFunc) == false)
	{
 ...
 *******************************************************/
static bool
CL_WatchClassHandlers (Environment * theEnv,
		       Defclass * theClass,
		       const char *theHandlerStr,
		       int theType,
		       const char *logName,
  ...
  while (theHandler != 0)
	  ...
	  (theType == (int) theClass->handlers[theHandler - 1].type))
	{
	  if ((theHandlerStr == NULL) ? true :
	      (strcmp
	       (theHandlerStr,
		CL_DefmessageHandlerName (theClass, theHandler)) == 0))
	    {
	      if (traceFunc != NULL)
 ...
 ***************************************************/
static void
CL_PrintHandler_WatchFlag (Environment * theEnv,
			   const char *logName,
			   Defclass * theClass, unsigned theHandler)
{
  CL_WriteString (theEnv, logName, CL_DefclassName (theClass));
  CL_WriteString (theEnv, logName, " ");
  CL_WriteString (theEnv, logName,
		  CL_DefmessageHandlerName (theClass, theHandler));
  CL_WriteString (theEnv, logName, " ");
  CL_WriteString (theEnv, logName,
		  CL_Defmessage_HandlerType (theClass, theHandler));
