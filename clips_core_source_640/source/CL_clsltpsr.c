...
static TEMP_SLOT_LINK *InsertSlot (Environment *, const char *,
				   TEMP_SLOT_LINK *, SlotDescriptor *);
static int ParseSimpleFacet (Environment *, const char *, SlotDescriptor *,
			     char *, const char *, int, const char *,
			     const char *, const char *, const char *,
			     CLIPSLexeme **);
static bool CL_ParseDefaultFacet (Environment *, const char *, char *,
				  SlotDescriptor *);
static void CL_BuildCompositeFacets (Environment *, SlotDescriptor *,
				     PACKED_CLASS_LINKS *, const char *,
				     CONSTRAINT_PARSE_RECORD *);
static bool CheckForFacetConflicts (Environment *, SlotDescriptor *,
				    CONSTRAINT_PARSE_RECORD *);
static bool CL_Evaluate_SlotDefaultValue (Environment *, SlotDescriptor *,
					  const char *);
...
CL_ParseSlot (Environment * theEnv,
	      ...
	      PACKED_CLASS_LINKS * preclist, bool multiSlot)
{
  SlotDescriptor *slot;
  ...
  if ((DefclassData (theEnv)->ObjectParseToken.value ==
       (void *) DefclassData (theEnv)->ISA_SYMBOL)
      || (DefclassData (theEnv)->ObjectParseToken.value ==
	  (void *) DefclassData (theEnv)->NAME_SYMBOL))
    {
      CL_DeleteSlots (theEnv, slist);
      CL_SyntaxErrorMessage (theEnv, "defclass slot");
      return NULL;
  ...
  slot =
    NewSlot (theEnv, DefclassData (theEnv)->ObjectParseToken.lexemeValue);
  ...
  while (DefclassData (theEnv)->ObjectParseToken.tknType ==
	...
	if (strcmp
	    (DefclassData (theEnv)->ObjectParseToken.lexemeValue->contents,
	     DEFAULT_FACET) == 0)
	{
	  if (CL_ParseDefaultFacet (theEnv, readSource, specbits, slot) ==
	      false)
	    goto CL_ParseSlotError;
	}
      else
	...
	{
	  rtnCode =
	    ParseSimpleFacet (theEnv, readSource, slot, specbits,
			      ACCESS_FACET, ACCESS_BIT, SLOT_RDWRT_RLN,
			      SLOT_RDONLY_RLN, SLOT_INIT_RLN, NULL, NULL);
	  if (rtnCode == -1)
	    goto CL_ParseSlotError;
	  else if (rtnCode == 1)
	    slot->no_Write = 1;
	     ...
	     STORAGE_FACET) == 0)
	{
	  rtnCode =
	    ParseSimpleFacet (theEnv, readSource, slot, specbits,
			      STORAGE_FACET, STORAGE_BIT, SLOT_LOCAL_RLN,
			      SLOT_SHARE_RLN, NULL, NULL, NULL);
	  if (rtnCode == -1)
	    goto CL_ParseSlotError;
	     ...
	     PROPAGATION_FACET) == 0)
	{
	  rtnCode =
	    ParseSimpleFacet (theEnv, readSource, slot, specbits,
			      PROPAGATION_FACET, PROPAGATION_BIT,
			      SLOT_INH_RLN, SLOT_NO_INH_RLN, NULL, NULL,
			      NULL);
	  if (rtnCode == -1)
	    goto CL_ParseSlotError;
	     ...
	     SOURCE_FACET) == 0)
	{
	  rtnCode =
	    ParseSimpleFacet (theEnv, readSource, slot, specbits,
			      SOURCE_FACET, SOURCE_BIT, SLOT_EXCLUSIVE_RLN,
			      SLOT_COMPOSITE_RLN, NULL, NULL, NULL);
	  if (rtnCode == -1)
	    goto CL_ParseSlotError;
	...
	{
	  rtnCode =
	    ParseSimpleFacet (theEnv, readSource, slot, specbits, MATCH_FACET,
			      MATCH_BIT, SLOT_NONREACTIVE_RLN,
			      SLOT_REACTIVE_RLN, NULL, NULL, NULL);
	  if (rtnCode == -1)
	    goto CL_ParseSlotError;
	     ...
	     VISIBILITY_FACET) == 0)
	{
	  rtnCode =
	    ParseSimpleFacet (theEnv, readSource, slot, specbits,
			      VISIBILITY_FACET, VISIBILITY_BIT,
			      SLOT_PRIVATE_RLN, SLOT_PUBLIC_RLN, NULL, NULL,
			      NULL);
	  if (rtnCode == -1)
	    goto CL_ParseSlotError;
	...
	if (strcmp
	    (DefclassData (theEnv)->ObjectParseToken.lexemeValue->contents,
	     CREATE_ACCESSOR_FACET) == 0)
	{
	  rtnCode =
	    ParseSimpleFacet (theEnv, readSource, slot, specbits,
			      CREATE_ACCESSOR_FACET, CREATE_ACCESSOR_BIT,
	...
	if (strcmp
	    (DefclassData (theEnv)->ObjectParseToken.lexemeValue->contents,
	     OVERRIDE_MSG_FACET) == 0)
	{
	  rtnCode =
	    ParseSimpleFacet (theEnv, readSource, slot, specbits,
			      OVERRIDE_MSG_FACET, OVERRIDE_MSG_BIT, NULL,
			      NULL, NULL, SLOT_DEFAULT_RLN, &newOverrideMsg);
	  if (rtnCode == -1)
	    goto CL_ParseSlotError;
	  if (rtnCode == 4)
	    {
  ...
  if (slot->composite)
    CL_BuildCompositeFacets (theEnv, slot, preclist, specbits,
			     &parsedConstraint);
 ...
 ***************************************************/
void
CL_DeleteSlots (Environment * theEnv, TEMP_SLOT_LINK * slots)
{
  TEMP_SLOT_LINK *stmp;
  ...
  while (slots != NULL)
      ...
      if (stmp->desc->dynamicDefault == 1)
	{
	  CL_ExpressionDeinstall (theEnv,
				  (Expression *) stmp->desc->defaultValue);
...
NewSlot (Environment * theEnv, CLIPSLexeme * name)
{
  SlotDescriptor *slot;
...
InsertSlot (Environment * theEnv,
	    const char *className,
	    TEMP_SLOT_LINK * slist, SlotDescriptor * slot)
{
 ...
 *****************************************************************/
static int
ParseSimpleFacet (Environment * theEnv,
		  const char *readSource,
		  SlotDescriptor * slot,
		  char *specbits,
		  ...
		  const char *varRelation, CLIPSLexeme ** facetSymbolicValue)
{
  int rtnCode;
 ...
 *************************************************************/
static bool
CL_ParseDefaultFacet (Environment * theEnv,
		      const char *readSource,
		      char *specbits, SlotDescriptor * slot)
{
  Expression *tmp;
  ...
  error = false;
  tmp =
    CL_ParseDefault (theEnv, readSource, true,
		     TestBitMap (specbits, DEFAULT_DYNAMIC_BIT), false,
 ...
 *************************************************************************/
static void
CL_BuildCompositeFacets (Environment * theEnv,
			 SlotDescriptor * sd,
			 ...
			 CONSTRAINT_PARSE_RECORD * parsedConstraint)
{
  ...
  for (i = 1; i < preclist->classCount; i++)
    {
      compslot =
	CL_FindClassSlot (preclist->classArray[i], sd->slotName->name);
  ...
  if (compslot != NULL)
      ...
      if ((sd->defaultSpecified == 0) && (compslot->defaultSpecified == 1))
	{
	  ...
	  if (compslot->defaultValue != NULL)
		...
		{
		  sd->defaultValue =
		    CL_PackExpression (theEnv,
				       (Expression *) compslot->defaultValue);
		  CL_ExpressionInstall (theEnv,
					(Expression *) sd->defaultValue);
		}
	      else
		{
		  sd->defaultValue = get_struct (theEnv, udfValue);
		  GenCopyMemory (UDFValue, 1, sd->defaultValue,
				 compslot->defaultValue);
	  ...
	  compslot->overrideMessageSpecified)
	{
      ...
      CL_OverlayConstraint (theEnv, parsedConstraint, sd->constraint,
			    compslot->constraint);
    }
}
 ...
 ***************************************************/
static bool
CheckForFacetConflicts (Environment * theEnv,
			SlotDescriptor * sd,
			CONSTRAINT_PARSE_RECORD * parsedConstraint)
{
  if (sd->multiple == 0)
      ...
      if (parsedConstraint->cardinality)
	{
	  ...
	  CL_WriteString (theEnv, STDERR,
			  "The 'cardinality' facet can only be used with multifield slots.\n");
	  return false;
	}
      else
	{
  ...
  if (sd->noDefault && sd->no_Write)
    {
      ...
      CL_WriteString (theEnv, STDERR,
		      "Slots with an 'access' facet value of 'read-only' must have a default value.\n");
      return false;
    }
  if (sd->no_Write
      && (sd->create_WriteAccessor || sd->overrideMessageSpecified))
    {
      ...
      CL_WriteString (theEnv, STDERR,
		      "Slots with an 'access' facet value of 'read-only' cannot have a write accessor.\n");
      return false;
    }
 ...
 ********************************************************************/
static bool
CL_Evaluate_SlotDefaultValue (Environment * theEnv,
			      SlotDescriptor * sd, const char *specbits)
{
  UDFValue temp;
  ...
  if (sd->dynamicDefault == 0)
      ...
      if (TestBitMap (specbits, DEFAULT_BIT))
	{
	  ...
	  if (vPass != false)
	    vPass =
	      (CL_ValidSlotValue
	       (theEnv, &temp, sd, NULL,
		"the 'default' facet") == PSE_NO_ERROR);
    ...
    {
      vCode =
	CL_ConstraintCheckExpressionChain (theEnv,
					   (Expression *) sd->defaultValue,
					   sd->constraint);
