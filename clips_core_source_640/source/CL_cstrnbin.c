...
#if BLOAD_AND_BSAVE
static void CopyTo_BsaveConstraintRecord (Environment *, CONSTRAINT_RECORD *,
					  BSAVE_CONSTRAINT_RECORD *);
#endif
static void CopyFrom_BsaveConstraintRecord (Environment *, void *,
					    unsigned long);
...
/**************************************************/
void
CL_WriteNeededConstraints (Environment * theEnv, FILE * fp)
{
  int i;
  ...
  for (i = 0; i < SIZE_CONSTRAINT_HASH; i++)
	   ...
	   tmpPtr != NULL; tmpPtr = tmpPtr->next)
	{
	  ...
	  CL_Gen_Write (&bsaveConstraints, sizeof (BSAVE_CONSTRAINT_RECORD),
			fp);
	}
    }
}
...
/****************************************************/
static void
CopyTo_BsaveConstraintRecord (Environment * theEnv,
			      ...
			      BSAVE_CONSTRAINT_RECORD * bsaveConstraints)
{
  ...
  bsaveConstraints->instanceAddressesAllowed =
    constraints->instanceAddressesAllowed;
  bsaveConstraints->externalAddressesAllowed =
    constraints->externalAddressesAllowed;
  ...
  bsaveConstraints->instanceNameRestriction =
    constraints->instanceNameRestriction;

  bsaveConstraints->restrictionList =
    CL_HashedExpressionIndex (theEnv, constraints->restrictionList);
  bsaveConstraints->classList =
    CL_HashedExpressionIndex (theEnv, constraints->classList);
  bsaveConstraints->minValue =
    CL_HashedExpressionIndex (theEnv, constraints->minValue);
  bsaveConstraints->maxValue =
    CL_HashedExpressionIndex (theEnv, constraints->maxValue);
  bsaveConstraints->minFields =
    CL_HashedExpressionIndex (theEnv, constraints->minFields);
  bsaveConstraints->maxFields =
    CL_HashedExpressionIndex (theEnv, constraints->maxFields);
}
...
/********************************************************/
void
CL_ReadNeededConstraints (Environment * theEnv)
{
  CL_GenReadBinary (theEnv, &ConstraintData (theEnv)->NumberOfConstraints,
  ...
  CL_Bloadand_Refresh (theEnv, ConstraintData (theEnv)->NumberOfConstraints,
		       sizeof (BSAVE_CONSTRAINT_RECORD),
...
/*****************************************************/
static void
CopyFrom_BsaveConstraintRecord (Environment * theEnv,
				void *buf, unsigned long theIndex)
{
  BSAVE_CONSTRAINT_RECORD *bsaveConstraints;
  CONSTRAINT_RECORD *constraints;
  ...
  constraints->restrictionList =
    HashedExpressionPointer (bsaveConstraints->restrictionList);
  constraints->classList =
    HashedExpressionPointer (bsaveConstraints->classList);
  constraints->minValue =
    HashedExpressionPointer (bsaveConstraints->minValue);
  constraints->maxValue =
    HashedExpressionPointer (bsaveConstraints->maxValue);
  constraints->minFields =
    HashedExpressionPointer (bsaveConstraints->minFields);
  constraints->maxFields =
    HashedExpressionPointer (bsaveConstraints->maxFields);
...
/********************************************************/
void
CL_Clear_BloadedConstraints (Environment * theEnv)
{
  if (ConstraintData (theEnv)->NumberOfConstraints != 0)
    {
