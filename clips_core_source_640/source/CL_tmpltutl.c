...
static void PrintTemplateSlot (Environment *, const char *,
			       struct templateSlot *, CLIPSValue *);
static struct templateSlot *GetNextTemplateSlotToPrint (Environment *,
							struct fact *,
							struct templateSlot *,
							int *, int,
							const char *);
...
/********************************************************/
void
CL_InvalidDeftemplateSlotMessage (Environment * theEnv,
				  const char *slotName,
				  const char *deftemplateName, bool printCR)
{
  ...
  CL_WriteString (theEnv, STDERR,
		  "' not defined in corresponding deftemplate '");
...
/**********************************************************/
void
CL_SingleField_SlotCardinalityError (Environment * theEnv,
				     const char *slotName)
{
  ...
  CL_WriteString (theEnv, STDERR, slotName);
  CL_WriteString (theEnv, STDERR,
		  "' can only contain a single field value.\n");
}
...
/**********************************************************************/
void
CL_MultiIntoSingleFieldSlotError (Environment * theEnv,
...
/**************************************************************/
void
CL_CheckTemplateFact (Environment * theEnv, Fact * theFact)
{
  CLIPSValue *sublist;
  int i;
  Deftemplate *theDeftemplate;
  ...
  for (slotPtr = theDeftemplate->slotList;
      ...
      else
	{
      ...
      rv =
	CL_ConstraintCheckDataObject (theEnv, &theData, slotPtr->constraints);
      if (rv != NO_VIOLATION)
	{
	  ...
	  CL_ConstraintViolationErrorMessage (theEnv, NULL, thePlace, false,
					      0, slotPtr->slotName, 0, rv,
					      slotPtr->constraints, true);
...
/***********************************************************************/
bool
CL_CheckRHS_SlotTypes (Environment * theEnv,
		       struct expr *rhsSlots,
		       struct templateSlot *slotPtr, const char *thePlace)
{
  ConstraintViolationType rv;
  const char *theName;

  rv =
    CL_ConstraintCheckExpressionChain (theEnv, rhsSlots,
				       slotPtr->constraints);
  if (rv != NO_VIOLATION)
    {
      if (rv != CARDINALITY_VIOLATION)
	theName = "A literal slot value";
      else
	theName = "Literal slot values";
      CL_ConstraintViolationErrorMessage (theEnv, theName, thePlace, true, 0,
					  slotPtr->slotName, 0, rv,
					  slotPtr->constraints, true);
      return false;
    }
...
/*********************************************************/
struct templateSlot *
CL_GetNthSlot (Deftemplate * theDeftemplate, long long position)
{
  struct templateSlot *slotPtr;
  long long i = 0;
...
/*******************************************************/
int
CL_FindSlotPosition (Deftemplate * theDeftemplate, CLIPSLexeme * name)
{
  struct templateSlot *slotPtr;
  int position;

  for (slotPtr = theDeftemplate->slotList, position = 1;
       slotPtr != NULL; slotPtr = slotPtr->next, position++)
    {
...
/**********************/
static void
PrintTemplateSlot (Environment * theEnv,
		   const char *logicalName,
		   struct templateSlot *slotPtr, CLIPSValue * slotValue)
{
  ...
  if (slotPtr->multislot == false)
    {
      CL_WriteString (theEnv, logicalName, " ");
      CL_PrintAtom (theEnv, logicalName,
		    ((TypeHeader *) slotValue->value)->type,
		    slotValue->value);
    }
  ...
  /* Else print the value of the slot for a multi field slot. */
   /*==========================================================*/
	...
	{
	  CL_WriteString (theEnv, logicalName, " ");
	  CL_PrintMultifieldDriver (theEnv, logicalName, theSegment, 0,
				    theSegment->length, false);
	}
    }
...
GetNextTemplateSlotToPrint (Environment * theEnv,
			    struct fact *theFact,
			    struct templateSlot *slotPtr,
			    int *position,
			    int ignoreDefaults, const char *changeMap)
{
  UDFValue tempDO;
  CLIPSValue *sublist;
  ...
  while (slotPtr != NULL)
	...
	{
	  CL_DeftemplateSlotDefault (theEnv, theFact->whichDeftemplate,
				     slotPtr, &tempDO, true);
...
/**********************************************************/
void
CL_PrintTemplateFact (Environment * theEnv,
		      const char *logicalName,
		      Fact * theFact,
		      bool separateLines,
		      bool ignoreDefaults, const char *changeMap)
{
  CLIPSValue *sublist;
  int i;
  Deftemplate *theDeftemplate;
  ...
  if ((changeMap != NULL) && (theFact->whichDeftemplate->slotList != slotPtr))
    {
      CL_WriteString (theEnv, logicalName, " ...");
    }
...
/***************************************************************************/
void
CL_UpdateDeftemplateScope (Environment * theEnv)
{
  Deftemplate *theDeftemplate;
...
/****************************************************************/
struct templateSlot *
CL_FindSlot (Deftemplate * theDeftemplate,
	     CLIPSLexeme * name, unsigned short *whichOne)
{
  struct templateSlot *slotPtr;
...
/************************************************************/
Deftemplate *
CL_CreateImpliedDeftemplate (Environment * theEnv,
			     CLIPSLexeme * deftemplateName, bool setFlag)
{
