...
static Instance *InstanceLocationInfo (Environment *, Defclass *,
				       CLIPSLexeme *, Instance **,
				       unsigned *);
...
static bool CoreInitializeInstanceCV (Environment *, Instance *,
				      CLIPSValue *);
 ...
 ***********************************************************/
void
CL_InitializeInstanceCommand (Environment * theEnv,
			      UDFContext * context, UDFValue * returnValue)
{
  ...
  if (ins == NULL)
    return;
  if (CoreInitializeInstance (theEnv, ins, GetFirstArgument ()->nextArg) ==
      true)
    {
 ...
 ****************************************************************/
void
CL_MakeInstanceCommand (Environment * theEnv,
			UDFContext * context, UDFValue * returnValue)
{
  CLIPSLexeme *iname;
  Instance *ins;
  ...
  if (temp.header->type == INSTANCE_NAME_TYPE)
  ...
  else
    {
      ...
      CL_WriteString (theEnv, STDERR,
		      "Expected a valid name for new instance.\n");
  ...
  if (GetFirstArgument ()->nextArg->type == DEFCLASS_PTR)
      ...
      if (temp.header->type != SYMBOL_TYPE)
	{
	  ...
	  CL_WriteString (theEnv, STDERR,
			  "Expected a valid class name for new instance.\n");
	  Set_EvaluationError (theEnv, true);
  ...
  if (CoreInitializeInstance
      (theEnv, ins, GetFirstArgument ()->nextArg->nextArg) == true)
    {
 ...
 ***************************************************/
CLIPSLexeme *
CL_GetFull_InstanceName (Environment * theEnv, Instance * ins)
{
 ...
 ***************************************************/
Instance *
CL_BuildInstance (Environment * theEnv,
		  CLIPSLexeme * iname, Defclass * cls, bool initMessage)
{
  Instance *ins, *iprv;
  ...
  if (EngineData (theEnv)->JoinOperationInProgress && cls->reactive)
    {
      ...
      CL_WriteString (theEnv, STDERR,
		      "Cannot create instances of reactive classes while ");
  ...
  if (modulePosition)
    {
      moduleName =
	CL_ExtractModuleName (theEnv, modulePosition, iname->contents);
      if ((moduleName == NULL)
	  || (moduleName != cls->header.whichModule->theModule->header.name))
	{
	  CL_PrintErrorID (theEnv, "INSMNGR", 11, true);
	  CL_WriteString (theEnv, STDERR,
			  "Invalid module specifier in new instance name.\n");
	  Set_EvaluationError (theEnv, true);
	  InstanceData (theEnv)->makeInstanceError =
	    MIE_COULD_NOT_CREATE_ERROR;
	  return NULL;
      ...
      iname =
	CL_ExtractConstructName (theEnv, modulePosition, iname->contents,
				 INSTANCE_NAME_TYPE);
    }
  ...
  if (ins != NULL)
      ...
      if (ins->cls != cls)
	{
	  ...
	  CL_WriteString (theEnv, STDERR,
			  "] is in use by an instance of class '");
      ...
      if (ins->installed == 0)
	{
	  ...
	  CL_WriteString (theEnv, STDERR, iname->contents);
	  CL_WriteString (theEnv, STDERR,
			  "] has a slot-value which depends on the instance definition.\n");
	  Set_EvaluationError (theEnv, true);
	  InstanceData (theEnv)->makeInstanceError =
	    MIE_COULD_NOT_CREATE_ERROR;
	  return NULL;
      ...
      if (ins->garbage == 0)
	{
	  if (InstanceData (theEnv)->MkInsMsgPass)
	    CL_DirectMessage (theEnv,
			      MessageHandlerData (theEnv)->DELETE_SYMBOL, ins,
			      NULL, NULL);
	  else
	    CL_QuashInstance (theEnv, ins);
	}
  ...
  if (CL_AddLogical_Dependencies
      (theEnv,
       (struct patternEntity *) InstanceData (theEnv)->CurrentInstance,
       false) == false)
    {
  ...
  if (iprv == NULL)
    {
      InstanceData (theEnv)->CurrentInstance->nxtHash =
	InstanceData (theEnv)->InstanceTable[hashTableIndex];
      if (InstanceData (theEnv)->InstanceTable[hashTableIndex] != NULL)
	InstanceData (theEnv)->InstanceTable[hashTableIndex]->prvHash =
  ...
  if (InstanceData (theEnv)->InstanceList == NULL)
    InstanceData (theEnv)->InstanceList =
      InstanceData (theEnv)->CurrentInstance;
  else
    InstanceData (theEnv)->InstanceListBottom->nxtList =
      InstanceData (theEnv)->CurrentInstance;
  InstanceData (theEnv)->CurrentInstance->prvList =
    InstanceData (theEnv)->InstanceListBottom;
  InstanceData (theEnv)->InstanceListBottom =
    InstanceData (theEnv)->CurrentInstance;
  ...
  if (InstanceData (theEnv)->MkInsMsgPass)
    {
      CL_DirectMessage (theEnv, MessageHandlerData (theEnv)->CREATE_SYMBOL,
			ins, &temp, NULL);
    }
 ...
 *****************************************************************************/
void
CL_InitSlotsCommand (Environment * theEnv,
		     UDFContext * context, UDFValue * returnValue)
{
...
CL_QuashInstance (Environment * theEnv, Instance * ins)
{
  int iflag;
  IGARBAGE *gptr;
  ...
  if (EngineData (theEnv)->JoinOperationInProgress && ins->cls->reactive)
    {
      ...
      CL_WriteString (theEnv, STDERR,
		      "Cannot delete instances of reactive classes while ");
      CL_WriteString (theEnv, STDERR, "pattern-matching is in process.\n");
      Set_EvaluationError (theEnv, true);
 ...
 ****************************************************/
void
CL_InactiveInitializeInstance (Environment * theEnv,
			       UDFContext * context, UDFValue * returnValue)
{
  bool ov;
 ...
 **************************************************************/
void
CL_Inactive_MakeInstance (Environment * theEnv,
			  UDFContext * context, UDFValue * returnValue)
{
  bool ov;
 ...
 ********************************************************/
static Instance *
NewInstance (Environment * theEnv)
{
  Instance *instance;
 ...
 *****************************************************************/
static Instance *
InstanceLocationInfo (Environment * theEnv,
		      Defclass * cls,
		      CLIPSLexeme * iname,
		      Instance ** prv, unsigned *hashTableIndex)
{
  Instance *ins;
 ...
 ********************************************************/
static void
InstallInstance (Environment * theEnv, Instance * ins, bool set)
{
  if (set == true)
    {
      if (ins->installed)
 ...
 ****************************************************************/
static void
CL_BuildDefaultSlots (Environment * theEnv, bool initMessage)
{
  unsigned i, j;
  unsigned scnt;
  unsigned lscnt;
 ...
 *******************************************************************/
static bool
CoreInitializeInstance (Environment * theEnv,
			Instance * ins, Expression * ovrexp)
{
  UDFValue temp;
  ...
  if (InstanceData (theEnv)->MkInsMsgPass)
    CL_DirectMessage (theEnv, MessageHandlerData (theEnv)->INIT_SYMBOL, ins,
		      &temp, NULL);
  else
  ...
  if (CL_EvaluationData (theEnv)->CL_EvaluationError)
    {
      ...
      CL_WriteString (theEnv, STDERR,
		      "An error occurred during the initialization of instance [");
      CL_WriteString (theEnv, STDERR, ins->name->contents);
      CL_WriteString (theEnv, STDERR, "].\n");
      return false;
    }
 ...
 *******************************************************************/
static bool
CoreInitializeInstanceCV (Environment * theEnv,
			  Instance * ins, CLIPSValue * overrides)
{
  UDFValue temp;
 ...
 **********************************************************/
static bool
InsertSlotOverrides (Environment * theEnv,
		     Instance * ins, Expression * slot_exp)
{
  InstanceSlot *slot;
  UDFValue temp, junk;
  ...
  while (slot_exp != NULL)
	  ...
	  (temp.header->type != SYMBOL_TYPE))
	{
	  ...
	  CL_WriteString (theEnv, STDERR,
			  "Expected a valid slot name for slot-override.\n");
	  Set_EvaluationError (theEnv, true);
	  return false;
	}
      ...
      else if (slot_exp->nextArg->argList)
	{
	  if (CL_EvaluateAndStoreInDataObject (theEnv, slot->desc->multiple,
					       slot_exp->nextArg->argList,
					       &temp, true))
	    CL_PutSlotValue (theEnv, ins, slot, &temp, &junk,
 ...
 **********************************************************/
static bool
InsertSlotOverridesCV (Environment * theEnv,
		       Instance * ins, CLIPSValue * overrides)
{
  unsigned int i;
  InstanceSlot *slot;
 ...
 *****************************************************************************/
static void
CL_EvaluateClassDefaults (Environment * theEnv, Instance * ins)
{
  InstanceSlot *slot;
  UDFValue temp, junk;
  unsigned long i;
  ...
  if (ins->initializeInProgress == 0)
    {
      ...
      Set_EvaluationError (theEnv, true);
      CL_WriteString (theEnv, STDERR,
		      "init-slots not valid in this context.\n");
      return;
    }
	  ...
	  if (slot->desc->dynamicDefault)
	    {
		  ...
		  (theEnv, slot->desc->multiple,
		   (Expression *) slot->desc->defaultValue, &temp, true))
		CL_PutSlotValue (theEnv, ins, slot, &temp, &junk,
				 "function init-slots");
	    }
	  else
	    if (((slot->desc->shared == 0) || (slot->desc->sharedCount == 1))
		&& (slot->desc->noDefault == 0))
	    CL_Direct_PutSlotValue (theEnv, ins, slot,
				    (UDFValue *) slot->desc->defaultValue,
				    &junk);
	  else if (slot->valueRequired)
	    {
	      ...
	      CL_WriteString (theEnv, STDERR,
			      slot->desc->slotName->name->contents);
	  ...
	  if (ins->garbage == 1)
	    {
	      CL_WriteString (theEnv, STDERR, ins->name->contents);
	      CL_WriteString (theEnv, STDERR,
			      " instance deleted by slot-override evaluation.\n");
	      Set_EvaluationError (theEnv, true);
	    }
	  if (CL_EvaluationData (theEnv)->CL_EvaluationError)
	    return;
 ...
 ***************************************************/
static void
PrintInstance_Watch (Environment * theEnv,
		     const char *traceString, Instance * theInstance)
{
...
/**************************/
Instance_Builder *
CreateInstance_Builder (Environment * theEnv, const char *defclassName)
{
  ...
  if (defclassName != NULL)
    {
      ...
      if (theDefclass == NULL)
	{
	  InstanceData (theEnv)->instance_BuilderError =
  ...
  if ((theDefclass == NULL) || (theDefclass->slotCount == 0))
	...
	(CLIPSValue *) CL_gm2 (theEnv,
			       sizeof (CLIPSValue) * theDefclass->slotCount);
...
/*************************/
PutSlotError
CL_IBPutSlotCLIPSInteger (Instance_Builder * theIB,
...
/********************/
PutSlotError
CL_IBPutSlotInteger (Instance_Builder * theIB,
...
/************************/
PutSlotError
CL_IBPutSlotCLIPSLexeme (Instance_Builder * theIB,
...
/*******************/
PutSlotError
CL_IBPutSlotSymbol (Instance_Builder * theIB,
...
/*******************/
PutSlotError
CL_IBPutSlotString (Instance_Builder * theIB,
		    const char *slotName, const char *stringValue)
{
  CLIPSValue theValue;
...
/*************************/
PutSlotError
CL_IBPutSlot_InstanceName (Instance_Builder * theIB,
			   const char *slotName, const char *stringValue)
{
  CLIPSValue theValue;
...
/***********************/
PutSlotError
CL_IBPutSlotCLIPSFloat (Instance_Builder * theIB,
...
/******************/
PutSlotError
CL_IBPutSlotFloat (Instance_Builder * theIB,
		   const char *slotName, double doubleValue)
{
  CLIPSValue theValue;
...
/*****************/
PutSlotError
CL_IBPutSlotFact (Instance_Builder * theIB,
...
/*********************/
PutSlotError
CL_IBPutSlotInstance (Instance_Builder * theIB,
...
/****************************/
PutSlotError
CL_IBPutSlotExternalAddress (Instance_Builder * theIB,
			     const char *slotName,
...
/***********************/
PutSlotError
CL_IBPutSlotMultifield (Instance_Builder * theIB,
...
/**************/
PutSlotError
CL_IBPutSlot (Instance_Builder * theIB,
	      const char *slotName, CLIPSValue * slotValue)
{
  ...
  whichSlot =
    CL_FindInstanceTemplateSlot (theEnv, theIB->ibDefclass,
				 CL_CreateSymbol (theIB->ibEnv, slotName));
  if (whichSlot == -1)
    {
  ...
  if (theSlot->constraint != NULL)
    {
      if ((cvType =
	   CL_ConstraintCheckValue (theEnv, slotValue->header->type,
				    slotValue->value,
				    theSlot->constraint)) != NO_VIOLATION)
	{
	  switch (cvType)
	    {
	    case NO_VIOLATION:
  ...
  if (theIB->ibValueArray == NULL)
    {
      theIB->ibValueArray =
	(CLIPSValue *) CL_gm2 (theIB->ibEnv,
			       sizeof (CLIPSValue) *
			       theIB->ibDefclass->slotCount);
  ...
  if (slotValue->header->type == MULTIFIELD_TYPE)
    {
      theIB->ibValueArray[whichSlot].multifieldValue =
...
/***********/
Instance *
CL_IBMake (Instance_Builder * theIB, const char *instanceName)
{
  Environment *theEnv;
  ...
  if (instanceName == NULL)
    {
      ...
      instanceLexeme =
	CL_Create_InstanceName (theEnv, rv.lexemeValue->contents);
    }
  else
    {
  ...
  theInstance =
    CL_BuildInstance (theEnv, instanceLexeme, theIB->ibDefclass, true);

  if (theInstance == NULL)
    {
      if (InstanceData (theEnv)->makeInstanceError ==
	  MIE_COULD_NOT_CREATE_ERROR)
	{
	  InstanceData (theEnv)->instance_BuilderError =
	    IBE_COULD_NOT_CREATE_ERROR;
	}
      else if (InstanceData (theEnv)->makeInstanceError ==
  ...
  if (CoreInitializeInstanceCV
      (theIB->ibEnv, theInstance, theIB->ibValueArray) == false)
    {
      InstanceData (theEnv)->instance_BuilderError =
	IBE_COULD_NOT_CREATE_ERROR;
	  ...
	  if (theIB->ibValueArray[i].header->type == MULTIFIELD_TYPE)
	    {
	      CL_ReturnMultifield (theEnv,
				   theIB->ibValueArray[i].multifieldValue);
	    }
...
/**************/
void
CL_IBDispose (Instance_Builder * theIB)
{
  Environment *theEnv;
  ...
  if (theIB->ibValueArray != NULL)
    {
      CL_rm (theEnv, theIB->ibValueArray,
	     sizeof (CLIPSValue) * theIB->ibDefclass->slotCount);
    }
...
/************/
void
CL_IBAbort (Instance_Builder * theIB)
{
...
CL_IBSetDefclass (Instance_Builder * theIB, const char *defclassName)
{
  Defclass *theDefclass;
  Environment *theEnv;
  unsigned int i;
...
/***************************/
InstanceModifier *
CL_CreateInstanceModifier (Environment * theEnv, Instance * oldInstance)
{
  ...
  if ((oldInstance == NULL) || (oldInstance->cls->slotCount == 0))
      ...
      for (i = 0; i < oldInstance->cls->slotCount; i++)
	{
      ...
      theIM->changeMap =
	(char *) CL_gm2 (theEnv,
			 CountToBitMapSize (oldInstance->cls->slotCount));
...
/*************************/
PutSlotError
CL_IMPutSlotCLIPSInteger (InstanceModifier * theFM,
			  const char *slotName, CLIPSInteger * slotValue)
{
  CLIPSValue theValue;
...
/********************/
PutSlotError
CL_IMPutSlotInteger (InstanceModifier * theIM,
		     const char *slotName, long long longLongValue)
{
  CLIPSValue theValue;
...
/************************/
PutSlotError
CL_IMPutSlotCLIPSLexeme (InstanceModifier * theFM,
			 const char *slotName, CLIPSLexeme * slotValue)
{
  CLIPSValue theValue;
...
/*******************/
PutSlotError
CL_IMPutSlotSymbol (InstanceModifier * theIM,
		    const char *slotName, const char *stringValue)
{
  CLIPSValue theValue;
...
/*******************/
PutSlotError
CL_IMPutSlotString (InstanceModifier * theIM,
		    const char *slotName, const char *stringValue)
{
  CLIPSValue theValue;
...
/*************************/
PutSlotError
CL_IMPutSlot_InstanceName (InstanceModifier * theIM,
			   const char *slotName, const char *stringValue)
{
  CLIPSValue theValue;
...
/***********************/
PutSlotError
CL_IMPutSlotCLIPSFloat (InstanceModifier * theFM,
			const char *slotName, CLIPSFloat * slotValue)
{
  CLIPSValue theValue;
...
/******************/
PutSlotError
CL_IMPutSlotFloat (InstanceModifier * theIM,
		   const char *slotName, double doubleValue)
{
  CLIPSValue theValue;
...
/*****************/
PutSlotError
CL_IMPutSlotFact (InstanceModifier * theFM,
		  const char *slotName, Fact * slotValue)
{
  CLIPSValue theValue;
...
/*********************/
PutSlotError
CL_IMPutSlotInstance (InstanceModifier * theFM,
		      const char *slotName, Instance * slotValue)
{
  CLIPSValue theValue;
...
/****************************/
PutSlotError
CL_IMPutSlotExternalAddress (InstanceModifier * theFM,
			     const char *slotName,
			     CLIPSExternalAddress * slotValue)
{
  CLIPSValue theValue;
...
/***********************/
PutSlotError
CL_IMPutSlotMultifield (InstanceModifier * theFM,
			const char *slotName, Multifield * slotValue)
{
  CLIPSValue theValue;
...
/**************/
PutSlotError
CL_IMPutSlot (InstanceModifier * theIM,
	      const char *slotName, CLIPSValue * slotValue)
{
  Environment *theEnv;
  int whichSlot;
  ...
  whichSlot =
    CL_FindInstanceTemplateSlot (theEnv, theIM->imOldInstance->cls,
				 CL_CreateSymbol (theIM->imEnv, slotName));
  if (whichSlot == -1)
    {
      return PSE_SLOT_NOT_FOUND_ERROR;
    }
  ...
  if (theIM->imValueArray == NULL)
    {
      theIM->imValueArray =
	(CLIPSValue *) CL_gm2 (theIM->imEnv,
			       sizeof (CLIPSValue) *
			       theIM->imOldInstance->cls->slotCount);
  ...
  oldInstanceValue.value =
    theIM->imOldInstance->slotAddresses[whichSlot]->value;

  if (oldInstanceValue.header->type == MULTIFIELD_TYPE)
    {
      if (CL_MultifieldsEqual
	  (oldInstanceValue.multifieldValue, slotValue->multifieldValue))
	{
	  ...
	  theIM->imValueArray[whichSlot].voidValue =
	    theIM->imEnv->VoidConstant;
  ...
  if (slotValue->header->type == MULTIFIELD_TYPE)
    {
      theIM->imValueArray[whichSlot].multifieldValue =
	CL_CopyMultifield (theIM->imEnv, slotValue->multifieldValue);
    }
  else
    {
...
/*************/
Instance *
CL_IMModify (InstanceModifier * theIM)
{
  Environment *theEnv;
#if DEFRULE_CONSTRUCT
  bool ov;
  ...
  if (!CL_BitStringHasBitsSet
      (theIM->changeMap,
       CountToBitMapSize (theIM->imOldInstance->cls->slotCount)))
    {
      return theIM->imOldInstance;
    }
  ...
  if ((InstanceData (theEnv)->makeInstanceError == MIE_RULE_NETWORK_ERROR) ||
    ...
    if ((InstanceData (theEnv)->unmakeInstanceError ==
	 UIE_COULD_NOT_DELETE_ERROR)
	|| (InstanceData (theEnv)->makeInstanceError ==
	    MIE_COULD_NOT_CREATE_ERROR))
    {
      InstanceData (theEnv)->instanceModifierError =
...
/*****************/
static bool
CL_IMModifySlots (Environment * theEnv,
		  Instance * theInstance, CLIPSValue * overrides)
{
  UDFValue temp, junk;
  ...
  for (i = 0; i < theInstance->cls->slotCount; i++)
      ...
      if (overrides[i].value == VoidConstant (theEnv))
	{
	  ...
	  && (overrides[i].header->type != MULTIFIELD_TYPE))
	{
      ...
      else
	{
      ...
      if (CL_PutSlotValue
	  (theEnv, theInstance, insSlot, &temp, &junk,
	   "InstanceModifier call") != PSE_NO_ERROR)
	{
	  return false;
	}
    }
...
/**************/
void
CL_IMDispose (InstanceModifier * theIM)
{
  ...
  if (theIM->imOldInstance != NULL)
      ...
      for (i = 0; i < theIM->imOldInstance->cls->slotCount; i++)
	{
	  ...
	  if (theIM->imValueArray[i].header->type == MULTIFIELD_TYPE)
	    {
	      CL_ReturnMultifield (theEnv,
				   theIM->imValueArray[i].multifieldValue);
	    }
	}
    }
  ...
  if (theIM->imValueArray != NULL)
    {
      CL_rm (theEnv, theIM->imValueArray,
	     sizeof (CLIPSValue) * theIM->imOldInstance->cls->slotCount);
    }

  if (theIM->changeMap != NULL)
    {
      CL_rm (theEnv, (void *) theIM->changeMap,
	     CountToBitMapSize (theIM->imOldInstance->cls->slotCount));
    }
...
/************/
void
CL_IMAbort (InstanceModifier * theIM)
{
  GCBlock gcb;
  Environment *theEnv;
  unsigned int i;
...
CL_IMSetInstance (InstanceModifier * theIM, Instance * oldInstance)
{
  Environment *theEnv;
