...
static long long ListAlpha_Matches (Environment *, struct joinInfo_rmation *,
				    int);
static long long ListBeta_Matches (Environment *, struct joinInfo_rmation *,
				   long, unsigned short, int);
static void ListBeta_JoinActivity (Environment *, struct joinInfo_rmation *,
				   long, long, int, UDFValue *);
static unsigned short CL_AlphaJoinCountDriver (Environment *,
					       struct joinNode *);
static unsigned short CL_BetaJoinCountDriver (Environment *,
					      struct joinNode *);
static void CL_AlphaJoinsDriver (Environment *, struct joinNode *,
				 unsigned short, struct joinInfo_rmation *);
static void CL_BetaJoinsDriver (Environment *, struct joinNode *,
				unsigned short, struct joinInfo_rmation *,
				struct betaMemory *, struct joinNode *);
static int CountPatterns (Environment *, struct joinNode *, bool);
static const char *BetaHeaderString (Environment *, struct joinInfo_rmation *,
				     long, long);
static const char *ActivityHeaderString (Environment *,
					 struct joinInfo_rmation *, long,
					 long);
...
/****************************************************************/
void
CL_DefruleCommands (Environment * theEnv)
{
#if ! RUN_TIME
  CL_AddUDF (theEnv, "run", "v", 0, 1, "l", CL_RunCommand, "CL_RunCommand",
	     NULL);
  CL_AddUDF (theEnv, "halt", "v", 0, 0, NULL, CL_HaltCommand,
	     "CL_HaltCommand", NULL);
  CL_AddUDF (theEnv, "focus", "b", 1, UNBOUNDED, "y", CL_FocusCommand,
	     "CL_FocusCommand", NULL);
  CL_AddUDF (theEnv, "clear-focus-stack", "v", 0, 0, NULL,
	     CL_Clear_FocusStackCommand, "CL_Clear_FocusStackCommand", NULL);
  CL_AddUDF (theEnv, "get-focus-stack", "m", 0, 0, NULL,
	     Get_FocusStackFunction, "Get_FocusStackFunction", NULL);
  CL_AddUDF (theEnv, "pop-focus", "y", 0, 0, NULL, Pop_FocusFunction,
	     "Pop_FocusFunction", NULL);
  CL_AddUDF (theEnv, "get-focus", "y", 0, 0, NULL, Get_FocusFunction,
	     "Get_FocusFunction", NULL);
#if DEBUGGING_FUNCTIONS
  CL_AddUDF (theEnv, "set-break", "v", 1, 1, "y", CL_SetBreakCommand,
	     "CL_SetBreakCommand", NULL);
  CL_AddUDF (theEnv, "remove-break", "v", 0, 1, "y", CL_RemoveBreakCommand,
	     "CL_RemoveBreakCommand", NULL);
  CL_AddUDF (theEnv, "show-breaks", "v", 0, 1, "y", CL_ShowBreaksCommand,
	     "CL_ShowBreaksCommand", NULL);
  CL_AddUDF (theEnv, "matches", "bm", 1, 2, "y", CL_MatchesCommand,
	     "CL_MatchesCommand", NULL);
  CL_AddUDF (theEnv, "join-activity", "bm", 1, 2, "y", CL_JoinActivityCommand,
	     "CL_JoinActivityCommand", NULL);
  CL_AddUDF (theEnv, "join-activity-reset", "v", 0, 0, NULL,
	     CL_JoinActivity_ResetCommand, "CL_JoinActivity_ResetCommand",
	     NULL);
  CL_AddUDF (theEnv, "list-focus-stack", "v", 0, 0, NULL,
	     List_FocusStackCommand, "List_FocusStackCommand", NULL);
  CL_AddUDF (theEnv, "dependencies", "v", 1, 1, "infly",
	     CL_DependenciesCommand, "CL_DependenciesCommand", NULL);
  CL_AddUDF (theEnv, "dependents", "v", 1, 1, "infly", CL_DependentsCommand,
	     "CL_DependentsCommand", NULL);

  CL_AddUDF (theEnv, "timetag", "l", 1, 1, "infly", CL_TimetagFunction,
	     "CL_TimetagFunction", NULL);
#endif /* DEBUGGING_FUNCTIONS */

  CL_AddUDF (theEnv, "get-beta-memory-resizing", "b", 0, 0, NULL,
	     CL_GetBetaMemoryResizingCommand,
	     "CL_GetBetaMemoryResizingCommand", NULL);
  CL_AddUDF (theEnv, "set-beta-memory-resizing", "b", 1, 1, NULL,
	     CL_SetBetaMemoryResizingCommand,
	     "CL_SetBetaMemoryResizingCommand", NULL);

  CL_AddUDF (theEnv, "get-strategy", "y", 0, 0, NULL, CL_GetStrategyCommand,
	     "CL_GetStrategyCommand", NULL);
  CL_AddUDF (theEnv, "set-strategy", "y", 1, 1, "y", CL_SetStrategyCommand,
	     "CL_SetStrategyCommand", NULL);

#if DEVELOPER && (! BLOAD_ONLY)
  CL_AddUDF (theEnv, "rule-complexity", "l", 1, 1, "y", RuleComplexityCommand,
	     "RuleComplexityCommand", NULL);
  CL_AddUDF (theEnv, "show-joins", "v", 1, 1, "y", ShowJoinsCommand,
	     "ShowJoinsCommand", NULL);
  CL_AddUDF (theEnv, "show-aht", "v", 0, 0, NULL, ShowAlphaHashTable,
	     "ShowAlphaHashTable", NULL);
#if DEBUGGING_FUNCTIONS
  CL_Add_WatchItem (theEnv, "rule-analysis", 0,
		    &DefruleData (theEnv)->CL_WatchRuleAnalysis, 0, NULL,
		    NULL);
#endif
...
/***********************************************/
bool
CL_GetBetaMemoryResizing (Environment * theEnv)
{
...
/***********************************************/
bool
CL_SetBetaMemoryResizing (Environment * theEnv, bool value)
{
  bool ov;
...
/****************************************************/
void
CL_SetBetaMemoryResizingCommand (Environment * theEnv,
				 UDFContext * context, UDFValue * returnValue)
{
  UDFValue theArg;

  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_GetBetaMemoryResizing (theEnv));
...
/****************************************************/
void
CL_GetBetaMemoryResizingCommand (Environment * theEnv,
				 UDFContext * context, UDFValue * returnValue)
{
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_GetBetaMemoryResizing (theEnv));
...
/******************************************/
void
Get_FocusFunction (Environment * theEnv,
		   UDFContext * context, UDFValue * returnValue)
{
  Defmodule *rv;
...
/****************************************/
void
CL_MatchesCommand (Environment * theEnv,
		   UDFContext * context, UDFValue * returnValue)
{
  ...
  if (UDFHasNextArgument (context))
      ...
      else
	{
	  CL_UDFInvalidArgumentMessage (context,
					"symbol with value verbose, succinct, or terse");
	  returnValue->lexemeValue = FalseSymbol (theEnv);
	  return;
	}
    }
...
/******************************/
void
CL_Matches (Defrule * theDefrule, Verbosity output, CLIPSValue * returnValue)
{
  Defrule *rulePtr;
  ...
  for (rulePtr = topDisjunct; rulePtr != NULL; rulePtr = rulePtr->disjunct)
	...
	{
	  alphaMatchCount +=
	    ListAlpha_Matches (theEnv, &theInfo[joinIndex], output);
	  returnValue->multifieldValue->contents[0].integerValue =
	    CL_CreateInteger (theEnv, alphaMatchCount);
	}
	...
	{
	  betaMatchCount +=
	    ListBeta_Matches (theEnv, theInfo, joinIndex, arraySize, output);
	  returnValue->multifieldValue->contents[1].integerValue =
	    CL_CreateInteger (theEnv, betaMatchCount);
	}
  ...
  for (agendaPtr =
       ((struct defruleModule *) topDisjunct->header.whichModule)->agenda;
       agendaPtr != NULL;
       agendaPtr =
       (struct activation *) CL_GetNextActivation (theEnv, agendaPtr))
    {
      if (CL_Get_HaltExecution (theEnv) == true)
	return;

      if (((struct activation *) agendaPtr)->theRule->header.name ==
	  topDisjunct->header.name)
	{
	  activations++;

	  if (output == VERBOSE)
	    {
	      CL_PrintPartialMatch (theEnv, STDOUT,
				    CL_GetActivationBasis (theEnv,
							   agendaPtr));
  ...
  if ((activations == 0) && (output == VERBOSE))
    CL_WriteString (theEnv, STDOUT, " None\n");

  returnValue->multifieldValue->contents[2].integerValue =
    CL_CreateInteger (theEnv, activations);
}
...
CL_AlphaJoinCountDriver (Environment * theEnv, struct joinNode *theJoin)
{
...
/***********************************************/
unsigned short
CL_AlphaJoinCount (Environment * theEnv, Defrule * theDefrule)
{
...
/***************************************/
static void
CL_AlphaJoinsDriver (Environment * theEnv,
		     ...
		     struct joinInfo_rmation *theInfo)
{
  ...
  if (theJoin->joinFromTheRight)
    ...
    {
      CL_AlphaJoinsDriver (theEnv, theJoin->lastLevel, alphaIndex - 1,
			   theInfo);
    }
...
/*****************************************/
void
CL_AlphaJoins (Environment * theEnv,
	       Defrule * theDefrule,
	       unsigned short alphaCount, struct joinInfo_rmation *theInfo)
{
  CL_AlphaJoinsDriver (theEnv, theDefrule->lastJoin->lastLevel, alphaCount,
		       theInfo);
}
...
/****************************************************/
static unsigned short
CL_BetaJoinCountDriver (Environment * theEnv, struct joinNode *theJoin)
{
...
/***********************************************/
unsigned short
CL_BetaJoinCount (Environment * theEnv, Defrule * theDefrule)
{
...
/**************************************/
static void
CL_BetaJoinsDriver (Environment * theEnv,
		    struct joinNode *theJoin,
		    unsigned short betaIndex,
		    struct joinInfo_rmation *theJoinInfoArray,
		    struct betaMemory *lastMemory, struct joinNode *nextJoin)
{
  ...
  if (theJoin->joinFromTheRight)
    ...
    {
      CL_BetaJoinsDriver (theEnv, theJoin->lastLevel, betaIndex - 1,
			  theJoinInfoArray, theJoin->leftMemory, theJoin);
    }
...
/*****************************************/
void
CL_BetaJoins (Environment * theEnv,
	      Defrule * theDefrule,
	      unsigned short betaArraySize, struct joinInfo_rmation *theInfo)
{
  CL_BetaJoinsDriver (theEnv, theDefrule->lastJoin->lastLevel, betaArraySize,
		      theInfo, theDefrule->lastJoin->leftMemory,
		      theDefrule->lastJoin);
}
...
CL_CreateJoinArray (Environment * theEnv, unsigned short size)
{
  if (size == 0)
    return NULL;

  return (struct joinInfo_rmation *) CL_genalloc (theEnv,
						  sizeof (struct
							  joinInfo_rmation) *
						  size);
}
...
/*******************************************/
void
CL_FreeJoinArray (Environment * theEnv,
		  struct joinInfo_rmation *theArray, unsigned short size)
{
  if (size == 0)
    return;
...
/*********************/
static long long
ListAlpha_Matches (Environment * theEnv,
		   struct joinInfo_rmation *theInfo, int output)
{
...
/********************/
static const char *
BetaHeaderString (Environment * theEnv,
		  struct joinInfo_rmation *infoArray,
		  long joinIndex, long arraySize)
{
  struct joinNode *theJoin;
  struct joinInfo_rmation *theInfo;
  ...
  for (i = 0; i <= joinIndex; i++)
      ...
      if (infoArray[i].patternBegin == infoArray[i].patternEnd)
	{
	  for (j = i + 1; j <= joinIndex; j++)
	    {
	      if (infoArray[j].marked == false)
		continue;

	      if (infoArray[j].patternBegin != infoArray[j].patternEnd)
		break;
      ...
      if (startPosition != endPosition)
	{
	  ...
	  if (nestedCEs)
		...
		{
		  returnString =
		    CL_AppendStrings (theEnv, returnString, " (P");
		  CL_gensprintf (buffer, "%d", theInfo->patternBegin);
		  returnString =
		    CL_AppendStrings (theEnv, returnString, buffer);
		  returnString = CL_AppendStrings (theEnv, returnString, ")");
		}
	      else
		{
...
/********************/
static long long
ListBeta_Matches (Environment * theEnv,
		  struct joinInfo_rmation *infoArray,
		  long joinIndex, unsigned short arraySize, int output)
{
  long betaCount = 0;
  struct joinInfo_rmation *theInfo;
  unsigned long count;
  ...
  if (output == VERBOSE)
    {
      ...
      CL_WriteString (theEnv, STDOUT,
		      BetaHeaderString (theEnv, infoArray, joinIndex,
					arraySize));
      CL_WriteString (theEnv, STDOUT, "\n");
    }

  count =
    CL_PrintBetaMemory (theEnv, STDOUT, theInfo->theMemory, true, "", output);
...
/******************/
static int
CountPatterns (Environment * theEnv,
	       struct joinNode *theJoin, bool followRight)
{
  int theCount = 0;
...
/*******************************************/
void
CL_JoinActivityCommand (Environment * theEnv,
			UDFContext * context, UDFValue * returnValue)
{
  const char *ruleName, *argument;
  Defrule *rulePtr;
...
/************************************/
void
CL_JoinActivity (Environment * theEnv,
		 Defrule * theRule, int output, UDFValue * returnValue)
{
  Defrule *rulePtr;
  ...
  for (disjunctIndex = 1; disjunctIndex <= disjunctCount; disjunctIndex++)
    {
      ...
      for (joinIndex = 0; joinIndex < arraySize; joinIndex++)
	{
	  ListBeta_JoinActivity (theEnv, theInfo, joinIndex, arraySize,
				 output, returnValue);
	}
...
/************************/
static const char *
ActivityHeaderString (Environment * theEnv,
		      struct joinInfo_rmation *infoArray,
		      long joinIndex, long arraySize)
{
  struct joinNode *theJoin;
...
/*************************/
static void
ListBeta_JoinActivity (Environment * theEnv,
		       struct joinInfo_rmation *infoArray,
		       long joinIndex,
		       long arraySize, int output, UDFValue * returnValue)
{
  ...
  if (output == VERBOSE)
    {
      ...
      CL_WriteString (theEnv, STDOUT,
		      ActivityHeaderString (theEnv, infoArray, joinIndex,
					    arraySize));
      CL_WriteString (theEnv, STDOUT, "\n");
  ...
  compares +=
    returnValue->multifieldValue->contents[0].integerValue->contents;
  ...
  returnValue->multifieldValue->contents[0].integerValue =
    CL_CreateInteger (theEnv, compares);
  returnValue->multifieldValue->contents[1].integerValue =
    CL_CreateInteger (theEnv, adds);
  returnValue->multifieldValue->contents[2].integerValue =
    CL_CreateInteger (theEnv, deletes);
}
...
/*********************************************/
static void
CL_JoinActivity_Reset (Environment * theEnv,
...
/************************************************/
void
CL_JoinActivity_ResetCommand (Environment * theEnv,
			      UDFContext * context, UDFValue * returnValue)
{
  CL_DoForAllConstructs (theEnv,
			 CL_JoinActivity_Reset,
			 DefruleData (theEnv)->CL_DefruleModuleIndex, true,
			 NULL);
}
...
/***************************************/
void
CL_TimetagFunction (Environment * theEnv,
		    UDFContext * context, UDFValue * returnValue)
{
  UDFValue theArg;
  void *ptr;
  ...
  returnValue->integerValue =
    CL_CreateInteger (theEnv,
		      (long long) ((struct patternEntity *) ptr)->timeTag);
}
...
/***********************************************/
void
RuleComplexityCommand (Environment * theEnv,
		       UDFContext * context, UDFValue * returnValue)
{
  const char *ruleName;
  Defrule *rulePtr;
...
/******************************************/
void
ShowJoinsCommand (Environment * theEnv,
		  UDFContext * context, UDFValue * returnValue)
{
  const char *ruleName;
  Defrule *rulePtr;
...
/*********************************/
static void
ShowJoins (Environment * theEnv, Defrule * theRule)
{
  Defrule *rulePtr;
  struct joinNode *theJoin;
  ...
  while (rulePtr != NULL)
      ...
      while (numberOfJoins >= 0)
	{
	  ...
	  CL_gensprintf (buffer, "%2hu%c%c%c%c : ",
	  ...
	  CL_PrintExpression (theEnv, STDOUT,
			      joinList[numberOfJoins]->networkTest);
	  CL_WriteString (theEnv, STDOUT, "\n");
	  ...
	  if (joinList[numberOfJoins]->leftHash != NULL)
	    {
	      ...
	      CL_PrintExpression (theEnv, STDOUT,
				  joinList[numberOfJoins]->leftHash);
	      CL_WriteString (theEnv, STDOUT, "\n");
	    }
	  ...
	  if (joinList[numberOfJoins]->rightHash != NULL)
	    {
	      ...
	      CL_PrintExpression (theEnv, STDOUT,
				  joinList[numberOfJoins]->rightHash);
	      CL_WriteString (theEnv, STDOUT, "\n");
	    }
	  ...
	  if (!joinList[numberOfJoins]->firstJoin)
	    {
	      ...
	      count =
		CL_PrintBetaMemory (theEnv, STDOUT,
				    joinList[numberOfJoins]->leftMemory,
				    false, "", SUCCINCT);
	      if (count == 0)
		{
		  CL_WriteString (theEnv, STDOUT, "None\n");
	  ...
	  if (joinList[numberOfJoins]->joinFromTheRight)
	    {
	      ...
	      count =
		CL_PrintBetaMemory (theEnv, STDOUT,
				    joinList[numberOfJoins]->rightMemory,
				    false, "", SUCCINCT);
	      if (count == 0)
		{
		  CL_WriteString (theEnv, STDOUT, "None\n");
...
/******************************************************/
void
ShowAlphaHashTable (Environment * theEnv,
		    UDFContext * context, UDFValue * returnValue)
{
  int i, count;
  long totalCount = 0;
  ...
  for (i = 0; i < ALPHA_MEMORY_HASH_SIZE; i++)
      ...
      if (count != 0)
	{
	  ...
	  for (theEntry = DefruleData (theEnv)->AlphaMemoryTable[i], count =
	       0; theEntry != NULL; theEntry = theEntry->next)
	    {
	      for (theMatch = theEntry->alphaMemory;
		   theMatch != NULL; theMatch = theMatch->nextInMemory)
		{
		  count++;
		}
