...
/************************************/
void
CL_AddConstructToModule (ConstructHeader * theConstruct)
{
...
/****************************************************/
bool
CL_DeleteNamedConstruct (Environment * theEnv,
...
/********************************************************/
ConstructHeader *
CL_FindNamedConstructInModuleOrImports (Environment * theEnv,
					const char *constructName,
					Construct * constructClass)
{
  ...
  theConstruct =
    CL_FindNamedConstructInModule (theEnv, constructName, constructClass);
  ...
  theConstruct =
    CL_FindImportedConstruct (theEnv, constructClass->constructName, NULL,
			      constructName, &count, true, NULL);

  if (count > 1)
    {
      CL_AmbiguousReferenceErrorMessage (theEnv,
...
/***********************************************/
ConstructHeader *
CL_FindNamedConstructInModule (Environment * theEnv,
			       const char *constructName,
			       Construct * constructClass)
{
  ConstructHeader *theConstruct;
  ...
  if ((constructName == NULL) ?
      true :
      ((findValue =
	CL_FindSymbolHN (theEnv, constructName, SYMBOL_BIT)) == NULL))
    {
...
/*****************************************/
void
CL_UndefconstructCommand (UDFContext * context,
			  const char *command, Construct * constructClass)
{
  ...
  if (((*constructClass->findFunction) (theEnv, constructName) == NULL) &&
    ...
    {
      CL_CantFindItemErrorMessage (theEnv, constructClass->constructName,
				   constructName, true);
      return;
    }
  ...
  else if (CL_DeleteNamedConstruct (theEnv, constructName, constructClass) ==
	   false)
    {
...
/******************************************/
void
CL_PPConstructCommand (UDFContext * context,
		       const char *command,
		       Construct * constructClass, UDFValue * returnValue)
{
  Environment *theEnv = context->environment;
  const char *constructName;
  ...
  if (CL_PPConstruct (theEnv, constructName, logicalName, constructClass) ==
      false)
    {
      CL_CantFindItemErrorMessage (theEnv, constructClass->constructName,
				   constructName, true);
...
/******************************************************/
const char *
CL_PPConstructNil (Environment * theEnv,
		   const char *constructName, Construct * constructClass)
{
  ConstructHeader *constructPtr;
...
/***********************************/
bool
CL_PPConstruct (Environment * theEnv,
		const char *constructName,
		const char *logicalName, Construct * constructClass)
{
  ConstructHeader *constructPtr;
  ...
  CL_WriteString (theEnv, logicalName,
		  (*constructClass->getPPFo_rmFunction) (constructPtr));
...
/*********************************************/
CLIPSLexeme *
CL_GetConstructModuleCommand (UDFContext * context,
			      const char *command, Construct * constructClass)
{
  Environment *theEnv = context->environment;
  const char *constructName;
  ...
  constructModule =
    CL_GetConstructModule (theEnv, constructName, constructClass);
...
/******************************************/
Defmodule *
CL_GetConstructModule (Environment * theEnv,
		       const char *constructName, Construct * constructClass)
{
  ConstructHeader *constructPtr;
  unsigned int count;
...
/****************************************/
bool
CL_UndefconstructAll (Environment * theEnv, Construct * constructClass)
{
#if BLOAD_ONLY || RUN_TIME
#if MAC_XCD
  ...
  while (currentConstruct != NULL)
    {
      ...
      nextConstruct =
	(*constructClass->getNextItemFunction) (theEnv, currentConstruct);
...
/*************************************/
bool
CL_Undefconstruct (Environment * theEnv,
		   ConstructHeader * theConstruct, Construct * constructClass)
{
#if BLOAD_ONLY || RUN_TIME
#if MAC_XCD
...
/***********************************/
void
CL_SaveConstruct (Environment * theEnv,
		  Defmodule * theModule,
		  const char *logicalName, Construct * constructClass)
{
  const char *ppfo_rm;
...
/*********************************************************/
const char *
CL_GetConstructModuleName (ConstructHeader * theConstruct)
{
...
/*********************************************************/
const char *
CL_GetConstructNameString (ConstructHeader * theConstruct)
{
...
/**********************************************************/
CLIPSLexeme *
CL_GetConstructNamePointer (ConstructHeader * theConstruct)
{
...
/************************************************/
void
CL_GetConstructListFunction (UDFContext * context,
...
/********************************************/
void
CL_GetConstructList (Environment * theEnv,
		     UDFValue * returnValue,
		     Construct * constructClass, Defmodule * theModule)
{
  ConstructHeader *theConstruct;
  ...
  while (loopModule != NULL)
    {
      ...
      while ((theConstruct =
	      (*constructClass->getNextItemFunction) (theEnv,
						      theConstruct)) != NULL)
	{
	 /*================================*/
      ...
      if ((tempSize + largestConstructNameSize + 5) > bufferSize)
	{
      ...
      if (allModules)
	loopModule = CL_GetNextDefmodule (theEnv, loopModule);
      else
	loopModule = NULL;
    }
	  ...
	  if (allModules)
	    {
	      ...
	      theList->contents[count].value =
		CL_CreateSymbol (theEnv, buffer);
	    }
	  else
	    {
	      theList->contents[count].value =
		CL_CreateSymbol (theEnv, theName->contents);
	    }
	  count++;
	}
...
/*********************************************/
void
CL_ListConstructCommand (UDFContext * context, Construct * constructClass)
{
  Defmodule *theModule;
  UDFValue result;
  unsigned int numArgs;
...
/*****************************************/
void
CL_ListConstruct (Environment * theEnv,
		  Construct * constructClass,
  ...
  while (theModule != NULL)
      ...
      if (allModules)
	{
      ...
      for (constructPtr =
	   ...
	   constructPtr != NULL;
	   constructPtr =
	   (*constructClass->getNextItemFunction) (theEnv, constructPtr))
	{
	  if (CL_EvaluationData (theEnv)->CL_HaltExecution == true)
	    return;

	  constructName =
	    (*constructClass->getConstructNameFunction) (constructPtr);
...
/**********************************************************/
void
CL_SetNextConstruct (ConstructHeader * theConstruct,
...
CL_GetConstructModuleItem (ConstructHeader * theConstruct)
{
...
/*************************************************/
const char *
CL_GetConstructPPFo_rm (ConstructHeader * theConstruct)
{
...
/****************************************************/
ConstructHeader *
CL_GetNextConstructItem (Environment * theEnv,
			 ConstructHeader * theConstruct, unsigned moduleIndex)
{
...
/*******************************************************/
struct defmoduleItemHeader *
CL_GetConstructModuleItemByIndex (Environment * theEnv,
				  Defmodule * theModule, unsigned moduleIndex)
{
  if (theModule != NULL)
    {
  ...
  return ((struct defmoduleItemHeader *)
	  CL_GetModuleItem (theEnv, CL_GetCurrentModule (theEnv),
			    moduleIndex));
}
...
/******************************************/
void
CL_FreeConstructHeaderModule (Environment * theEnv,
			      struct defmoduleItemHeader *theModuleItem,
			      Construct * constructClass)
{
...
/**********************************************/
void
CL_DoForAllConstructs (Environment * theEnv,
		       void (*actionFunction) (Environment *,
					       ConstructHeader *, void *),
		       unsigned moduleItemIndex, bool interruptable,
		       void *userBuffer)
{
  ...
  for (theModule = CL_GetNextDefmodule (theEnv, NULL);
	...
	CL_GetModuleItem (theEnv, theModule, moduleItemIndex);
      ...
      for (theConstruct = theModuleItem->firstItem;
	   theConstruct != NULL; theConstruct = next)
	{
	 /*==========================================*/
...
/******************************************************/
void
CL_DoForAllConstructsInModule (Environment * theEnv,
			       Defmodule * theModule,
			       ConstructActionFunction * actionFunction,
			       unsigned moduleItemIndex,
			       bool interruptable, void *userBuffer)
{
  ConstructHeader *theConstruct;
  struct defmoduleItemHeader *theModuleItem;
  ...
  for (theConstruct = theModuleItem->firstItem;
       theConstruct != NULL; theConstruct = theConstruct->next)
    {
      if (interruptable)
	{
	  if (CL_Get_HaltExecution (theEnv) == true)
...
/*****************************************************/
void
CL_InitializeConstructHeader (Environment * theEnv,
...
/*************************************************/
void
SetConstructPPFo_rm (Environment * theEnv,
		     ConstructHeader * theConstruct, const char *ppFo_rm)
{
...
/******************************************************/
bool
CL_ConstructPrint_WatchAccess (Environment * theEnv,
			       Construct * constructClass,
			       const char *logName,
			       Expression * argExprs,
...
{
  return (Construct_WatchSupport
	  (theEnv, constructClass, "list-watch-items", logName, argExprs,
...
/**************************************************/
bool
CL_ConstructSet_WatchAccess (Environment * theEnv,
			     Construct * constructClass,
			     bool newState,
			     Expression * argExprs,
			     ConstructGet_WatchFunction * get_WatchFunc,
...
{
  return (Construct_WatchSupport
	  (theEnv, constructClass, "watch", STDERR, argExprs, true, newState,
	   get_WatchFunc, set_WatchFunc));
}
...
/******************************************************/
static bool
Construct_WatchSupport (Environment * theEnv,
			Construct * constructClass,
			const char *funcName,
			const char *logName,
			Expression * argExprs,
			...
			ConstructSet_WatchFunction * set_WatchFunc)
{
	   ...
	   theModule = CL_GetNextDefmodule (theEnv, theModule))
	{
	 ...
	 /*============================================*/
	  ...
	  for (theConstruct =
		...
		{
		  CL_WriteString (theEnv, logName, "   ");
		  ConstructPrint_Watch (theEnv, logName, constructClass,
					theConstruct, get_WatchFunc);
		}
	    }
	}
  ...
  while (argExprs != NULL)
	...
	{
	  ConstructPrint_Watch (theEnv, logName, constructClass, theConstruct,
				get_WatchFunc);
	}
...
/*************************************************/
static void
ConstructPrint_Watch (Environment * theEnv,
		      const char *logName,
		      Construct * constructClass,
		      ConstructHeader * theConstruct,
		      ConstructGet_WatchFunction * get_WatchFunc)
...
/*****************************************************/
ConstructHeader *
CL_LookupConstruct (Environment * theEnv,
		    Construct * constructClass,
		    const char *constructName, bool moduleNameAllowed)
{
  ConstructHeader *theConstruct;
  ...
  theConstruct =
    CL_FindImportedConstruct (theEnv, constructType, NULL, constructName,
  ...
  if (theConstruct != NULL)
    {
      if (moduleCount > 1)
	{
	  CL_AmbiguousReferenceErrorMessage (theEnv, constructType,
					     constructName);
	  return NULL;
	}
      return (theConstruct);
...
/***********************************************************/
bool
CL_ConstructsDeletable (Environment * theEnv)
{
