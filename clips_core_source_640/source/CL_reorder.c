...
static struct lhsParseNode *ReverseAndOr (Environment *,
					  struct lhsParseNode *,
					  struct lhsParseNode *, int);
static struct lhsParseNode *Perfo_rmReorder1 (Environment *,
					      struct lhsParseNode *, bool *,
					      int);
static struct lhsParseNode *Perfo_rmReorder2 (Environment *,
					      struct lhsParseNode *, bool *,
					      int);
static struct lhsParseNode *CompressCEs (Environment *, struct lhsParseNode *,
					 bool *, int);
...
static struct lhsParseNode *ReorderDriver (Environment *,
					   struct lhsParseNode *, bool *, int,
					   int);
static struct lhsParseNode *AddRe_mainingInitialPatterns (Environment *,
							  struct lhsParseNode
							  *);
static struct lhsParseNode *AssignPatternIndices (struct lhsParseNode *,
						  short, int, unsigned short);
static void PropagateIndexSlotPatternValues (struct lhsParseNode *, short,
					     unsigned short, CLIPSLexeme *,
					     unsigned short);
...
static unsigned short PropagateWhichCE (struct lhsParseNode *,
					unsigned short);
   /*
...
/********************************************/
/* CL_ReorderPatterns: Reorders a group of CEs */
/*   to accommodate KB Rete topology.       */
/********************************************/
struct lhsParseNode *
CL_ReorderPatterns (Environment * theEnv,
		    struct lhsParseNode *theLHS, bool *anyChange)
{
  ...
  newLHS = CL_GetLHSParseNode (theEnv);
  ...
  if (newLHS->pnType == OR_CE_NODE)
    {
      for (tempLHS = newLHS->right, lastLHS = NULL;
	   tempLHS != NULL; lastLHS = tempLHS, tempLHS = tempLHS->bottom)
	{
	  if (tempLHS->pnType != AND_CE_NODE)
	    {
	      theLHS = CL_GetLHSParseNode (theEnv);
  ...
  if (newLHS->pnType == OR_CE_NODE)
    {
      for (theLHS = newLHS->right; theLHS != NULL; theLHS = theLHS->bottom)
	{
  ...
  CL_AddInitialPatterns (theEnv, newLHS);
...
ReorderDriver (Environment * theEnv,
	       struct lhsParseNode *theLHS,
	       bool *anyChange, int pass, int depth)
{
  ...
  while (change)
    {
	  ...
	  (theLHS->pnType == NOT_CE_NODE) || (theLHS->pnType == OR_CE_NODE))
	{
	  if (pass == 1)
	    theLHS = Perfo_rmReorder1 (theEnv, theLHS, &newChange, depth);
	  else
	    theLHS = Perfo_rmReorder2 (theEnv, theLHS, &newChange, depth);
	 ...
	 /*============================================*/
	  ...
	  if ((argPtr->pnType == AND_CE_NODE) ||
	      ...
	      if (before == NULL)
		{
		  argPtr->bottom = NULL;
		  theLHS->right =
		    ReorderDriver (theEnv, argPtr, &newChange, pass,
				   depth + 1);
...
/********************/
static void
MarkExistsNands (struct lhsParseNode *theLHS)
{
  int currentDepth = 1;
  ...
  while (theLHS != NULL)
    {
      if (CL_IsExistsSubjoin (theLHS, currentDepth))
	{
	  theLHS->existsNand = true;

	  for (tmpLHS = theLHS; tmpLHS != NULL; tmpLHS = tmpLHS->bottom)
	    {
...
/****************************************************************/
/* CL_AddInitialPatterns: Add initial patterns to CEs where needed */
/*   (such as before a "test" CE or "not" CE which is the first */
/*   CE within an "and" CE).                                    */
/****************************************************************/
void
CL_AddInitialPatterns (Environment * theEnv, struct lhsParseNode *theLHS)
{
  ...
  if (theLHS->pnType == OR_CE_NODE)
	...
	{
	  CL_AddInitialPatterns (theEnv, thePattern);
	}
...
/***********************************************************/
static struct lhsParseNode *
Perfo_rmReorder1 (Environment * theEnv,
		  struct lhsParseNode *theLHS, bool *newChange, int depth)
{
  ...
  while (change)
    {
      ...
      for (argPtr = theLHS->right; argPtr != NULL;)
	{
	 /*=============================================================*/
	 ...
	 /*==============================================================*/
		   ...
		   && (argPtr->pnType == OR_CE_NODE))
	    {
	      ...
	      *newChange = true;
	      ...
	      argPtr->right = NULL;
	      argPtr->bottom = NULL;
	      CL_ReturnLHSParseNodes (theEnv, argPtr);
	      theLHS->pnType = AND_CE_NODE;
	      theLHS->right = tempArg;

	      while (tempArg != NULL)
		{
		  newNode = CL_GetLHSParseNode (theEnv);
		  CL_CopyLHSParseNode (theEnv, newNode, tempArg, false);
...
/***********************************************************/
static struct lhsParseNode *
Perfo_rmReorder2 (Environment * theEnv,
		  struct lhsParseNode *theLHS, bool *newChange, int depth)
{
  struct lhsParseNode *argPtr;
  bool change;
  ...
  while (change)
      ...
      for (argPtr = theLHS->right; argPtr != NULL;)
	{
	 ...
	 /*==========================================*/
		   ...
		   (argPtr->right->pnType == PATTERN_CE_NODE))
	    {
	      ...
	      *newChange = true;
	      ...
	      CL_CopyLHSParseNode (theEnv, theLHS, argPtr->right, false);
	 ...
	 /*======================================*/
		   ...
		   && (argPtr->pnType == PATTERN_CE_NODE))
	    {
	      change = true;
	      *newChange = true;

	      CL_CopyLHSParseNode (theEnv, theLHS, argPtr, false);
...
/**************************************************/
static struct lhsParseNode *
ReverseAndOr (Environment * theEnv,
	      struct lhsParseNode *listOfCEs,
	      struct lhsParseNode *orCE, int orPosition)
{
  int count;
  ...
  while (orCE != NULL)
    {
      ...
      copyOfCEs = CL_CopyLHSParseNodes (theEnv, listOfCEs);
      ...
      for (count = 1, replaceCE = copyOfCEs->right;
	   count != orPosition; count++, replaceCE = replaceCE->bottom)
	{			/* Do Nothing */
	}
      ...
      CL_ReturnLHSParseNodes (theEnv, replaceCE->right);
      ...
      CL_CopyLHSParseNode (theEnv, replaceCE, orCE, true);
      replaceCE->right = CL_CopyLHSParseNodes (theEnv, orCE->right);
   ...
   /*=====================================================*/
  /* CL_Release the original and/or CE list to free memory. */
   /*=====================================================*/

  CL_ReturnLHSParseNodes (theEnv, listOfCEs);

   /*================================================*/
  /* Wrap an or CE around the list of expanded CEs. */
   /*================================================*/

  copyOfCEs = CL_GetLHSParseNode (theEnv);
...
/****************/
static struct lhsParseNode *
CompressCEs (Environment * theEnv,
	     struct lhsParseNode *theLHS, bool *newChange, int depth)
{
  struct lhsParseNode *argPtr, *lastArg, *nextArg;
  ...
  while (change)
      ...
      for (argPtr = theLHS->right; argPtr != NULL;)
	{
	 ...
	 /*=======================================================*/
		   ...
		   && (argPtr->pnType == TEST_CE_NODE))
	    {
	      change = true;
	      *newChange = true;

	      tempArg = CL_GetLHSParseNode (theEnv);
	      ...
	      CL_CopyLHSParseNode (theEnv, theLHS, argPtr, true);
	      CL_ReturnLHSParseNodes (theEnv, argPtr);
	      theLHS->right = NULL;
	      break;
	    }
	 ...
	 /*==============================*/
	  ...
	  else if ((theLHS->pnType == AND_CE_NODE)
		   && (argPtr->pnType == TEST_CE_NODE)
		   && ((argPtr->bottom != NULL) ? argPtr->bottom->pnType ==
		       TEST_CE_NODE : false)
		   && (argPtr->beginNandDepth == argPtr->endNandDepth)
		   && (argPtr->endNandDepth ==
		       argPtr->bottom->beginNandDepth))
	    {
	      change = true;
	      *newChange = true;

	      argPtr->expression =
		CL_CombineLHSParseNodes (theEnv, argPtr->expression,
	      ...
	      CL_ReturnLHSParseNodes (theEnv, tempArg);
	    }
	 ...
	 /*========================================================*/
		   ...
		   && (argPtr->pnType == PATTERN_CE_NODE)
		   && ((argPtr->bottom != NULL) ? argPtr->bottom->pnType ==
		       TEST_CE_NODE : false) && (argPtr->negated == false)
		   && (argPtr->exists == false)
		   && (argPtr->beginNandDepth == argPtr->endNandDepth)
		   && (argPtr->endNandDepth ==
		       argPtr->bottom->beginNandDepth))
	      ...
	      if (argPtr->negated || argPtr->exists)
		{
		  ...
		  e2 =
		    CL_LHSParseNodesToExpression (theEnv,
						  argPtr->bottom->expression);
		  e1 = CL_CombineExpressions (theEnv, e1, e2);
		  CL_ReturnLHSParseNodes (theEnv,
					  argPtr->secondaryExpression);
		  argPtr->secondaryExpression =
		    CL_ExpressionToLHSParseNodes (theEnv, e1);
		  CL_ReturnExpression (theEnv, e1);
		}
	      else
		{
		  argPtr->expression =
	      ...
	      if ((theLHS->right == argPtr)
		  && ((argPtr->beginNandDepth - 1) == endNandDepth))
		{
		  if (argPtr->negated)
		    {
...
/*********************************************************************/
/* CL_CopyLHSParseNodes: Copies a linked group of conditional elements. */
/*********************************************************************/
struct lhsParseNode *
CL_CopyLHSParseNodes (Environment * theEnv, struct lhsParseNode *listOfCEs)
{
  ...
  CL_CopyLHSParseNode (theEnv, newList, listOfCEs, true);

  newList->right = CL_CopyLHSParseNodes (theEnv, listOfCEs->right);
  newList->bottom = CL_CopyLHSParseNodes (theEnv, listOfCEs->bottom);

  return (newList);
}

/**********************************************************/
/* CL_CopyLHSParseNode: Copies a single conditional element. */
/**********************************************************/
void
CL_CopyLHSParseNode (Environment * theEnv,
		     struct lhsParseNode *dest,
		     struct lhsParseNode *src, bool duplicate)
{
  ...
  if (duplicate)
    {
      dest->networkTest = CL_CopyExpression (theEnv, src->networkTest);
      dest->externalNetworkTest =
	CL_CopyExpression (theEnv, src->externalNetworkTest);
      dest->secondaryNetworkTest =
	CL_CopyExpression (theEnv, src->secondaryNetworkTest);
      dest->externalRightHash =
	CL_CopyExpression (theEnv, src->externalRightHash);
      dest->externalLeftHash =
	CL_CopyExpression (theEnv, src->externalLeftHash);
      dest->constantSelector =
	CL_CopyExpression (theEnv, src->constantSelector);
      dest->constantValue = CL_CopyExpression (theEnv, src->constantValue);
      dest->leftHash = CL_CopyExpression (theEnv, src->leftHash);
      dest->betaHash = CL_CopyExpression (theEnv, src->betaHash);
      dest->rightHash = CL_CopyExpression (theEnv, src->rightHash);
	...
	{
	  dest->userData =
	    (*src->patternType->copyUserDataFunction) (theEnv, src->userData);
	}
      dest->expression = CL_CopyLHSParseNodes (theEnv, src->expression);
      dest->secondaryExpression =
	CL_CopyLHSParseNodes (theEnv, src->secondaryExpression);
      dest->constraints = CL_CopyConstraintRecord (theEnv, src->constraints);
      if (dest->constraints != NULL)
	dest->derivedConstraints = true;
      else
...
/****************************************************/
/* CL_GetLHSParseNode: Creates an empty node structure */
/*   used for building conditional elements.        */
/****************************************************/
struct lhsParseNode *
CL_GetLHSParseNode (Environment * theEnv)
{
...
/********************************************************/
/* CL_ReturnLHSParseNodes:  Returns a multiply linked list */
/*   of lhsParseNode structures to the memory manager.  */
/********************************************************/
void
CL_ReturnLHSParseNodes (Environment * theEnv, struct lhsParseNode *waste)
{
  if (waste != NULL)
    {
      CL_ReturnExpression (theEnv, waste->networkTest);
      CL_ReturnExpression (theEnv, waste->externalNetworkTest);
      CL_ReturnExpression (theEnv, waste->secondaryNetworkTest);
      CL_ReturnExpression (theEnv, waste->externalRightHash);
      CL_ReturnExpression (theEnv, waste->externalLeftHash);
      CL_ReturnExpression (theEnv, waste->constantSelector);
      CL_ReturnExpression (theEnv, waste->constantValue);
      CL_ReturnExpression (theEnv, waste->leftHash);
      CL_ReturnExpression (theEnv, waste->betaHash);
      CL_ReturnExpression (theEnv, waste->rightHash);
      CL_ReturnLHSParseNodes (theEnv, waste->right);
      CL_ReturnLHSParseNodes (theEnv, waste->bottom);
      CL_ReturnLHSParseNodes (theEnv, waste->expression);
      CL_ReturnLHSParseNodes (theEnv, waste->secondaryExpression);
      if (waste->derivedConstraints)
	CL_RemoveConstraint (theEnv, waste->constraints);
      if ((waste->userData != NULL) &&
	  (waste->patternType->returnUserDataFunction != NULL))
	{
	  (*waste->patternType->returnUserDataFunction) (theEnv,
							 waste->userData);
	}
...
/********************************************************/
/* CL_ExpressionToLHSParseNodes: Copies an expression into */
/*   the equivalent lhsParseNode data structures.       */
/********************************************************/
struct lhsParseNode *
CL_ExpressionToLHSParseNodes (Environment * theEnv,
			      struct expr *expressionList)
{
  ...
  newList = CL_GetLHSParseNode (theEnv);
  newList->pnType = CL_TypeToNodeType (expressionList->type);
  newList->value = expressionList->value;
  newList->right =
    CL_ExpressionToLHSParseNodes (theEnv, expressionList->nextArg);
  newList->bottom =
    CL_ExpressionToLHSParseNodes (theEnv, expressionList->argList);
...
/******************************************************************/
/* CL_LHSParseNodesToExpression: Copies lhsParseNode data structures */
/*   into the equivalent expression data structures.              */
/******************************************************************/
struct expr *
CL_LHSParseNodesToExpression (Environment * theEnv,
			      struct lhsParseNode *nodeList)
{
  ...
  newList->type = CL_NodeTypeToType (nodeList);
  newList->value = nodeList->value;
  newList->nextArg = CL_LHSParseNodesToExpression (theEnv, nodeList->right);
  newList->argList = CL_LHSParseNodesToExpression (theEnv, nodeList->bottom);

  return (newList);
}

/******************************************/
/* CL_ConstantNode: Returns true if the node */
/*   is a constant, otherwise false.      */
/******************************************/
bool
CL_ConstantNode (struct lhsParseNode *theNode)
{
...
/*******************/
unsigned short
CL_NodeTypeToType (struct lhsParseNode *theNode)
{
  switch (theNode->pnType)
    {
    case FLOAT_NODE:
...
/*******************/
ParseNodeType
CL_TypeToNodeType (unsigned short theType)
{
  switch (theType)
    {
    case FLOAT_TYPE:
...
/************************************************************/
static void
IncrementNandDepth (Environment * theEnv,
		    struct lhsParseNode *theLHS, bool lastCE)
{
   /*======================================*/
  ...
  for (; theLHS != NULL; theLHS = theLHS->bottom)
    {
	  ...
	  || (theLHS->pnType == TEST_CE_NODE))
	{
	  theLHS->beginNandDepth++;

	  if (lastCE == false)
	    theLHS->endNandDepth++;
	  else if (theLHS->bottom != NULL)
	    theLHS->endNandDepth++;
	}
	...
	{
	  CL_SystemError (theEnv, "REORDER", 1);
	}
    }
}
...
/***********************************************************/
static struct lhsParseNode *
CreateInitialPattern (Environment * theEnv)
{
...
/*****************************************************************/
static struct lhsParseNode *
AddRe_mainingInitialPatterns (Environment * theEnv,
			      struct lhsParseNode *theLHS)
{
...
/*************************************************************/
static struct lhsParseNode *
AssignPatternIndices (struct lhsParseNode *theLHS,
		      short startIndex,
		      int nandDepth, unsigned short joinDepth)
{
  ...
  while (theLHS != NULL)
	...
	{
	  theLHS =
	    AssignPatternIndices (theLHS, startIndex, nandDepth + 1,
				  joinDepth);
      ...
      else if (theLHS->pnType == TEST_CE_NODE)
	{
	  ...
	  PropagateNandDepth (theLHS->expression, theLHS->beginNandDepth,
			      theLHS->endNandDepth);
	  if (theLHS->endNandDepth < nandDepth)
	    return (theLHS);
	}
      ...
      else if (theLHS->pnType == PATTERN_CE_NODE)
	{
	  ...
	  PropagateNandDepth (theLHS->right, theLHS->beginNandDepth,
			      theLHS->endNandDepth);
	  for (theField = theLHS->right; theField != NULL;
...
/***********************************************************/
static void
PropagateIndexSlotPatternValues (struct lhsParseNode *theField,
				 short thePattern,
				 ...
				 unsigned short theSlotNumber)
{
  ...
  if (theField->multifieldSlot)
      ...
      if ((theIndex > 0) && (theIndex != NO_INDEX))
	{
      ...
      for (tmpNode = theField->bottom;
	   tmpNode != NULL; tmpNode = tmpNode->right)
	{
	  tmpNode->pattern = thePattern;
	  tmpNode->slot = theSlot;
	  PropagateIndexSlotPatternValues (tmpNode, thePattern,
					   tmpNode->index, theSlot,
					   theSlotNumber);
	}
  ...
  for (theField = theField->bottom;
       theField != NULL; theField = theField->bottom)
    {
      /*===========================================================*/
...
/*****************************************************************/
static void
PropagateJoinDepth (struct lhsParseNode *theField, unsigned short joinDepth)
{
  while (theField != NULL)
    {
...
/**************************************************************/
static void
PropagateNandDepth (struct lhsParseNode *theField,
  ...
  for (; theField != NULL; theField = theField->right)
    {
      ...
      PropagateNandDepth (theField->secondaryExpression, beginDepth,
			  endDepth);
...
PropagateWhichCE (struct lhsParseNode *theField, unsigned short whichCE)
{
  while (theField != NULL)
    {
      if ((theField->pnType == PATTERN_CE_NODE)
	  || (theField->pnType == TEST_CE_NODE))
	{
	  whichCE++;
	}
...
/********************/
bool
CL_IsExistsSubjoin (struct lhsParseNode *theLHS, int parentDepth)
{
...
/***************************************************************************/
/* CL_CombineLHSParseNodes: Combines two expressions into a single equivalent */
...
/***************************************************************************/
struct lhsParseNode *
CL_CombineLHSParseNodes (Environment * theEnv,
			 ...
			 struct lhsParseNode *expr2)
{
  ...
  tempPtr = CL_GetLHSParseNode (theEnv);
      ...
      switch (theNode->type)
        {
         case PATTERN_CE:
           ...
           if (theNode->negated) CL_WriteString(theEnv,fileid,"n");
           if (theNode->exists) CL_WriteString(theEnv,fileid,"x");
           if (theNode->logical) CL_WriteString(theEnv,fileid,"l");
           CL_PrintUnsignedInteger(theEnv,fileid,theNode->beginNandDepth);
           CL_WriteString(theEnv,fileid,"-");
           CL_PrintUnsignedInteger(theEnv,fileid,theNode->endNandDepth);
           CL_WriteString(theEnv,fileid," ");
           CL_WriteString(theEnv,fileid,ValueToString(theNode->right->bottom->value));
           CL_WriteString(theEnv,fileid,")");
           break;

         case TEST_CE:
           CL_WriteString(theEnv,fileid,"(test ");
           CL_PrintUnsignedInteger(theEnv,fileid,theNode->beginNandDepth);
           CL_WriteString(theEnv,fileid,"-");
           CL_PrintUnsignedInteger(theEnv,fileid,theNode->endNandDepth);
           CL_WriteString(theEnv,fileid,")");
         ...
         case NOT_CE:
           if (theNode->logical) CL_WriteString(theEnv,fileid,"(lnot ");
           else CL_WriteString(theEnv,fileid,"(not ");;
           PrintNodes(theEnv,fileid,theNode->right);
           CL_WriteString(theEnv,fileid,")");
           break;

         case OR_CE:
           if (theNode->logical) CL_WriteString(theEnv,fileid,"(lor ");
           else CL_WriteString(theEnv,fileid,"(or ");
           PrintNodes(theEnv,fileid,theNode->right);
           CL_WriteString(theEnv,fileid,")");
           break;

         case AND_CE:
           if (theNode->logical) CL_WriteString(theEnv,fileid,"(land ");
           else CL_WriteString(theEnv,fileid,"(and ");
           PrintNodes(theEnv,fileid,theNode->right);
           CL_WriteString(theEnv,fileid,")");
           break;

         default:
           CL_WriteString(theEnv,fileid,"(unknown)");
           break;

        }

      theNode = theNode->bottom;
      if (theNode != NULL) CL_WriteString(theEnv,fileid," ");
     }
