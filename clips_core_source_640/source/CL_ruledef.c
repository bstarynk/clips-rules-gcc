...
static void CL_DestroyDefruleAction (Environment *, ConstructHeader *,
				     void *);
#if RUN_TIME
...
/**********************************************************/
void
CL_InitializeDefrules (Environment * theEnv)
{
  unsigned long i;
  CL_AllocateEnvironmentData (theEnv, DEFRULE_DATA,
			      sizeof (struct defruleData),
			      DeallocateDefruleData);
  ...
  for (i = 0; i < ALPHA_MEMORY_HASH_SIZE; i++)
    DefruleData (theEnv)->AlphaMemoryTable[i] = NULL;
...
/**************************************************/
static void
DeallocateDefruleData (Environment * theEnv)
{
...
/********************************************************/
static void
CL_DestroyDefruleAction (Environment * theEnv,
...
/*****************************************************/
static void
Initialize_DefruleModules (Environment * theEnv)
{
  DefruleData (theEnv)->CL_DefruleModuleIndex =
    CL_RegisterModuleItem (theEnv, "defrule", AllocateModule, ReturnModule,
...
Get_DefruleModuleItem (Environment * theEnv, Defmodule * theModule)
{
...
/****************************************************************/
Defrule *
CL_FindDefrule (Environment * theEnv, const char *defruleName)
{
...
/******************************************************/
bool
CL_DefruleIsDeletable (Defrule * theDefrule)
{
  ...
  for (; theDefrule != NULL; theDefrule = theDefrule->disjunct)
    {
      if (theDefrule->executing)
	return false;
    }
...
/********************************************************/
long
CL_GetDisjunctCount (Environment * theEnv, Defrule * theDefrule)
{
  long count = 0;
...
/*******************************************************/
Defrule *
CL_GetNthDisjunct (Environment * theEnv, Defrule * theDefrule, long index)
{
  long count = 0;
...
/******************************************/
void
Defrule_RunTimeInitialize (Environment * theEnv,
  ...
  for (theModule = CL_GetNextDefmodule (theEnv, NULL);
       theModule != NULL; theModule = CL_GetNextDefmodule (theEnv, theModule))
    {
      ...
      for (theRule = CL_GetNextDefrule (theEnv, NULL);
	   theRule != NULL; theRule = CL_GetNextDefrule (theEnv, theRule))
	{
...
/**************************/
static void
AddBetaMemoriesToRule (Environment * theEnv, struct joinNode *theNode)
{
...
/**************************/
void
CL_AddBetaMemoriesToJoin (Environment * theEnv, struct joinNode *theNode)
{
  ...
  if ((!theNode->firstJoin) || theNode->patternIsExists
      || theNode->patternIsNegated || theNode->joinFromTheRight)
    {
      if (theNode->leftHash == NULL)
	{
	  theNode->leftMemory = get_struct (theEnv, betaMemory);
	  theNode->leftMemory->beta =
      ...
      else
	{
	  ...
	  memset (theNode->leftMemory->beta, 0,
	  ...
	  && (theNode->patternIsExists || theNode->patternIsNegated
	      || theNode->joinFromTheRight))
	{
  ...
  if (theNode->joinFromTheRight)
      ...
      if (theNode->leftHash == NULL)
	{
	  ...
	  theNode->rightMemory->beta =
	    (struct partialMatch **) CL_genalloc (theEnv,
						  sizeof (struct partialMatch
							  *));
	  theNode->rightMemory->last =
	    (struct partialMatch **) CL_genalloc (theEnv,
						  sizeof (struct partialMatch
							  *));
	  ...
	  theNode->rightMemory->last =
	    (struct partialMatch **) CL_genalloc (theEnv,
						  sizeof (struct partialMatch
							  *) *
						  INITIAL_BETA_HASH_SIZE);
	  memset (theNode->rightMemory->beta, 0,
		  sizeof (struct partialMatch **) * INITIAL_BETA_HASH_SIZE);
	  memset (theNode->rightMemory->last, 0,
		  sizeof (struct partialMatch **) * INITIAL_BETA_HASH_SIZE);
...
const char *
CL_DefruleModule (Defrule * theDefrule)
{
  return CL_GetConstructModuleName (&theDefrule->header);
}

const char *
CL_DefruleName (Defrule * theDefrule)
{
  return CL_GetConstructNameString (&theDefrule->header);
}

const char *
CL_DefrulePPFo_rm (Defrule * theDefrule)
{
