...
/* =========================================
   *****************************************
   ...
   =========================================
   ***************************************** */
typedef struct bsave_DeffunctionModule
{
...
static void CL_BsaveDeffunctionExpression (Environment *, ConstructHeader *,
					   void *);
...
static void Update_DeffunctionModule (Environment *, void *, unsigned long);
static void UpdateDeffunction (Environment *, void *, unsigned long);
static void CL_ClearDeffunction_Bload (Environment *);
static void DeallocateDeffunction_BloadData (Environment *);
...
/***********************************************************
  NAME         : CL_SetupDeffunctions_Bload
 ...
 ***********************************************************/
void
CL_SetupDeffunctions_Bload (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, DFFNXBIN_DATA,
			      sizeof (struct deffunctionBinaryData),
			      DeallocateDeffunction_BloadData);
#if BLOAD_AND_BSAVE
  CL_AddBinaryItem (theEnv, "deffunctions", 0, CL_BsaveDeffunctionFind,
		    CL_BsaveDeffunctionExpressions,
		    CL_BsaveStorageDeffunctions, CL_BsaveDeffunctions,
		    CL_BloadStorageDeffunctions, CL_BloadDeffunctions,
		    CL_ClearDeffunction_Bload);
#else
...
/***********************************************************/
/* DeallocateDeffunction_BloadData: Deallocates environment */
/*    data for the deffunction bsave functionality.        */
/***********************************************************/
static void
DeallocateDeffunction_BloadData (Environment * theEnv)
{
  size_t space;

#if (BLOAD || BLOAD_ONLY || BLOAD_AND_BSAVE) && (! RUN_TIME)
  space =
    DeffunctionBinaryData (theEnv)->DeffunctionCount * sizeof (Deffunction);
  if (space != 0)
    CL_genfree (theEnv, DeffunctionBinaryData (theEnv)->DeffunctionArray,
		space);

  space =
    DeffunctionBinaryData (theEnv)->ModuleCount *
    sizeof (struct deffunctionModuleData);
  if (space != 0)
...
/***************************************************
  NAME         : CL_Bload_DeffunctionModuleReference
 ...
 ***************************************************/
void *
CL_Bload_DeffunctionModuleReference (Environment * theEnv,
 ...
 ***************************************************************************/
static void
CL_BsaveDeffunctionFind (Environment * theEnv)
{
  CL_Save_BloadCount (theEnv, DeffunctionBinaryData (theEnv)->ModuleCount);
  CL_Save_BloadCount (theEnv,
		      DeffunctionBinaryData (theEnv)->DeffunctionCount);
 ...
 ***************************************************/
static void
MarkDeffunctionItems (Environment * theEnv,
		      ConstructHeader * theDeffunction, void *userBuffer)
{
#if MAC_XCD
 ...
 ***************************************************/
static void
CL_BsaveDeffunctionExpressions (Environment * theEnv, FILE * fp)
{
  CL_DoForAllConstructs (theEnv, CL_BsaveDeffunctionExpression,
			 DeffunctionData (theEnv)->CL_DeffunctionModuleIndex,
			 false, fp);
}
 ...
 ***************************************************/
static void
CL_BsaveDeffunctionExpression (Environment * theEnv,
			       ConstructHeader * theDeffunction,
			       void *userBuffer)
{
  CL_BsaveExpression (theEnv, ((Deffunction *) theDeffunction)->code,
		      (FILE *) userBuffer);
}
 ...
 ***********************************************************/
static void
CL_BsaveStorageDeffunctions (Environment * theEnv, FILE * fp)
{
  size_t space;

  space = sizeof (unsigned long) * 2;
  CL_Gen_Write (&space, sizeof (size_t), fp);
  CL_Gen_Write (&DeffunctionBinaryData (theEnv)->ModuleCount,
		sizeof (unsigned long), fp);
  CL_Gen_Write (&DeffunctionBinaryData (theEnv)->DeffunctionCount,
		sizeof (unsigned long), fp);
}

/*************************************************************************************
  NAME         : CL_BsaveDeffunctions
  DESCRIPTION  : CL_Writes out deffunction in binary fo_rmat
 ...
 *************************************************************************************/
static void
CL_BsaveDeffunctions (Environment * theEnv, FILE * fp)
{
  size_t space;
  Defmodule *theModule;
  ...
  space =
    ((sizeof (BSAVE_DEFFUNCTION_MODULE) *
      DeffunctionBinaryData (theEnv)->ModuleCount) +
     (sizeof (BSAVE_DEFFUNCTION) *
      DeffunctionBinaryData (theEnv)->DeffunctionCount));
  CL_Gen_Write (&space, sizeof (size_t), fp);
  ...
  while (theModule != NULL)
	...
	CL_GetModuleItem (theEnv, theModule,
			  CL_FindModuleItem (theEnv,
					     "deffunction")->moduleIndex);
      CL_Assign_BsaveDefmdlItemHdrVals (&dummy_mitem.header,
					&theModuleItem->header);
      CL_Gen_Write (&dummy_mitem, sizeof (BSAVE_DEFFUNCTION_MODULE), fp);
  ...
  /* ==========================
     ...
     ========================== */
  CL_DoForAllConstructs (theEnv, CL_BsaveDeffunction,
			 DeffunctionData (theEnv)->CL_DeffunctionModuleIndex,
			 false, fp);

  Restore_BloadCount (theEnv, &DeffunctionBinaryData (theEnv)->ModuleCount);
 ...
 ***************************************************/
static void
CL_BsaveDeffunction (Environment * theEnv,
		     ConstructHeader * theDeffunction, void *userBuffer)
{
  ...
  if (dptr->code != NULL)
    {
      ...
      ExpressionData (theEnv)->ExpressionCount +=
	CL_ExpressionSize (dptr->code);
    }
  else
    dummy_df.code = ULONG_MAX;
  CL_Gen_Write (&dummy_df, sizeof (BSAVE_DEFFUNCTION), (FILE *) userBuffer);
}
 ...
 ***********************************************************************/
static void
CL_BloadStorageDeffunctions (Environment * theEnv)
{
  size_t space;
  ...
  if (space == 0L)
    return;
  CL_GenReadBinary (theEnv, &DeffunctionBinaryData (theEnv)->ModuleCount,
		    sizeof (unsigned long));
  CL_GenReadBinary (theEnv, &DeffunctionBinaryData (theEnv)->DeffunctionCount,
		    sizeof (unsigned long));
  ...
  space =
    (DeffunctionBinaryData (theEnv)->ModuleCount *
     sizeof (CL_DeffunctionModuleData));
  DeffunctionBinaryData (theEnv)->ModuleArray =
    (CL_DeffunctionModuleData *) CL_genalloc (theEnv, space);
  ...
  space =
    (DeffunctionBinaryData (theEnv)->DeffunctionCount * sizeof (Deffunction));
  DeffunctionBinaryData (theEnv)->DeffunctionArray =
    (Deffunction *) CL_genalloc (theEnv, space);
}

/*********************************************************************
  NAME         : CL_BloadDeffunctions
  DESCRIPTION  : This routine reads deffunction info_rmation from
                   a binary file
 ...
 ********************************************************************/
static void
CL_BloadDeffunctions (Environment * theEnv)
{
  size_t space;

  CL_GenReadBinary (theEnv, &space, sizeof (size_t));
  CL_Bloadand_Refresh (theEnv, DeffunctionBinaryData (theEnv)->ModuleCount,
		       sizeof (BSAVE_DEFFUNCTION_MODULE),
		       Update_DeffunctionModule);
  CL_Bloadand_Refresh (theEnv,
		       DeffunctionBinaryData (theEnv)->DeffunctionCount,
		       sizeof (BSAVE_DEFFUNCTION), UpdateDeffunction);
}

/*******************************************************
  NAME         : Update_DeffunctionModule
 ...
 *******************************************************/
static void
Update_DeffunctionModule (Environment * theEnv, void *buf, unsigned long obji)
{
 ...
 ***************************************************/
static void
UpdateDeffunction (Environment * theEnv, void *buf, unsigned long obji)
{
  ...
  CL_UpdateConstructHeader (theEnv, &bdptr->header, &dptr->header,
			    DEFFUNCTION, sizeof (CL_DeffunctionModuleData),
			    DeffunctionBinaryData (theEnv)->ModuleArray,
			    sizeof (Deffunction),
			    DeffunctionBinaryData (theEnv)->DeffunctionArray);
...
/***************************************************************
  NAME         : CL_ClearDeffunction_Bload
 ...
 ***************************************************************/
static void
CL_ClearDeffunction_Bload (Environment * theEnv)
{
  unsigned long i;
  size_t space;
