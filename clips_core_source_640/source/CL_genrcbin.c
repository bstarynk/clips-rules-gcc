...
static void CL_BsaveMethodExpressions (Environment *, ConstructHeader *,
				       void *);
static void CL_BsaveRestrictionExpressions (Environment *, ConstructHeader *,
					    void *);
static void CL_BsaveGenerics (Environment *, FILE *);
static void CL_BsaveDefgenericHeader (Environment *, ConstructHeader *,
				      void *);
static void CL_BsaveMethods (Environment *, ConstructHeader *, void *);
static void CL_BsaveMethodRestrictions (Environment *, ConstructHeader *,
					void *);
static void CL_BsaveRestrictionTypes (Environment *, ConstructHeader *,
				      void *);
 ...
 ***********************************************************/
void
SetupGenerics_Bload (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, GENRCBIN_DATA,
			      sizeof (struct defgenericBinaryData),
			      DeallocateDefgenericBinaryData);
#if BLOAD_AND_BSAVE
  CL_AddBinaryItem (theEnv, "generic functions", 0, CL_BsaveGenericsFind,
		    CL_BsaveGenericsExpressions, CL_BsaveStorageGenerics,
		    CL_BsaveGenerics, CL_BloadStorageGenerics,
		    CL_BloadGenerics, CL_Clear_BloadGenerics);
#endif
#if BLOAD || BLOAD_ONLY
...
/***********************************************************/
static void
DeallocateDefgenericBinaryData (Environment * theEnv)
{
  ...
  if (space != 0)
    CL_genfree (theEnv, DefgenericBinaryData (theEnv)->DefgenericArray,
		space);
  ...
  space =
    DefgenericBinaryData (theEnv)->RestrictionCount *
    sizeof (struct restriction);
  if (space != 0)
    CL_genfree (theEnv, DefgenericBinaryData (theEnv)->RestrictionArray,
		space);
  ...
  space =
    ...
    sizeof (struct defgenericModule);
  if (space != 0)
 ...
 ***************************************************/
void *
CL_Bload_DefgenericModuleReference (Environment * theEnv,
 ...
 ***************************************************************************/
static void
CL_BsaveGenericsFind (Environment * theEnv)
{
 ...
 ***************************************************/
static void
MarkDefgenericItems (Environment * theEnv,
		     ConstructHeader * theDefgeneric, void *userBuffer)
{
#if MAC_XCD
#pragma unused(userBuffer)
  ...
  for (i = 0; i < gfunc->mcnt; i++)
	...
	CL_ExpressionSize (meth->actions);
      CL_MarkNeededItems (theEnv, meth->actions);
      DefgenericBinaryData (theEnv)->RestrictionCount +=
	meth->restrictionCount;
      for (j = 0; j < meth->restrictionCount; j++)
	{
	  rptr = &meth->restrictions[j];
	  ExpressionData (theEnv)->ExpressionCount +=
	    CL_ExpressionSize (rptr->query);
 ...
 ***************************************************/
static void
CL_BsaveGenericsExpressions (Environment * theEnv, FILE * fp)
{
   /*===================================================================*/
  ...
  CL_DoForAllConstructs (theEnv, CL_BsaveMethodExpressions,
			 DefgenericData (theEnv)->CL_DefgenericModuleIndex,
			 false, fp);

  CL_DoForAllConstructs (theEnv, CL_BsaveRestrictionExpressions,
			 DefgenericData (theEnv)->CL_DefgenericModuleIndex,
			 false, fp);
}
 ...
 ***************************************************/
static void
CL_BsaveMethodExpressions (Environment * theEnv,
			   ConstructHeader * theDefgeneric, void *userBuffer)
{
  Defgeneric *gfunc = (Defgeneric *) theDefgeneric;
  long i;
  ...
  for (i = 0; i < gfunc->mcnt; i++)
    CL_BsaveExpression (theEnv, gfunc->methods[i].actions,
			(FILE *) userBuffer);
}
 ...
 ***************************************************/
static void
CL_BsaveRestrictionExpressions (Environment * theEnv,
  ...
  for (i = 0; i < gfunc->mcnt; i++)
    {
      meth = &gfunc->methods[i];
      for (j = 0; j < meth->restrictionCount; j++)
	CL_BsaveExpression (theEnv, meth->restrictions[j].query,
			    (FILE *) userBuffer);
    }
}
 ...
 ***********************************************************/
static void
CL_BsaveStorageGenerics (Environment * theEnv, FILE * fp)
{
  size_t space;

  space = sizeof (long) * 5;
  CL_Gen_Write (&space, sizeof (size_t), fp);
  CL_Gen_Write (&DefgenericBinaryData (theEnv)->ModuleCount, sizeof (long),
		fp);
  CL_Gen_Write (&DefgenericBinaryData (theEnv)->GenericCount, sizeof (long),
		fp);
  CL_Gen_Write (&DefgenericBinaryData (theEnv)->MethodCount, sizeof (long),
		fp);
  CL_Gen_Write (&DefgenericBinaryData (theEnv)->RestrictionCount,
		sizeof (long), fp);
 ...
 ****************************************************************************************/
static void
CL_BsaveGenerics (Environment * theEnv, FILE * fp)
{
  Defmodule *theModule;
  ...
  while (theModule != NULL)
	...
	CL_GetModuleItem (theEnv, theModule,
			  CL_FindModuleItem (theEnv,
					     "defgeneric")->moduleIndex);
      CL_Assign_BsaveDefmdlItemHdrVals (&dummy_generic_module.header,
					&theModuleItem->header);
      CL_Gen_Write (&dummy_generic_module, sizeof (BSAVE_DEFGENERIC_MODULE),
		    fp);
  ...
  /* ======================================
     ...
     ====================================== */
  DefgenericBinaryData (theEnv)->MethodCount = 0L;
  CL_DoForAllConstructs (theEnv, CL_BsaveDefgenericHeader,
			 DefgenericData (theEnv)->CL_DefgenericModuleIndex,
			 false, fp);
  ...
  /* =====================
     ...
     ===================== */
  DefgenericBinaryData (theEnv)->RestrictionCount = 0L;
  CL_DoForAllConstructs (theEnv, CL_BsaveMethods,
			 DefgenericData (theEnv)->CL_DefgenericModuleIndex,
			 false, fp);
  ...
  /* =================================
     ...
     ================================= */
  DefgenericBinaryData (theEnv)->TypeCount = 0L;
  CL_DoForAllConstructs (theEnv, CL_BsaveMethodRestrictions,
			 DefgenericData (theEnv)->CL_DefgenericModuleIndex,
			 false, fp);
  ...
  /* =============================================================
     ...
     ============================================================= */
  CL_DoForAllConstructs (theEnv, CL_BsaveRestrictionTypes,
			 DefgenericData (theEnv)->CL_DefgenericModuleIndex,
			 false, fp);
 ...
 ***************************************************/
static void
CL_BsaveDefgenericHeader (Environment * theEnv,
			  ConstructHeader * theDefgeneric, void *userBuffer)
{
  Defgeneric *gfunc = (Defgeneric *) theDefgeneric;
 ...
 ***************************************************/
static void
CL_BsaveMethods (Environment * theEnv,
		 ConstructHeader * theDefgeneric, void *userBuffer)
{
  Defgeneric *gfunc = (Defgeneric *) theDefgeneric;
  Defmethod *meth;
  ...
  for (i = 0; i < gfunc->mcnt; i++)
    {
      ...
      CL_Assign_BsaveConstructHeaderVals (&dummy_method.header,
					  &meth->header);
	...
	dummy_method.actions = ULONG_MAX;
      CL_Gen_Write (&dummy_method, sizeof (BSAVE_METHOD),
		    (FILE *) userBuffer);
    }
}
 ...
 ******************************************************/
static void
CL_BsaveMethodRestrictions (Environment * theEnv,
			    ConstructHeader * theDefgeneric, void *userBuffer)
{
  Defgeneric *gfunc = (Defgeneric *) theDefgeneric;
  ...
  for (i = 0; i < gfunc->mcnt; i++)
      ...
      for (j = 0; j < gfunc->methods[i].restrictionCount; j++)
	{
	  ...
	  if (rptr->types != NULL)
	    {
	      dummy_restriction.types =
		DefgenericBinaryData (theEnv)->TypeCount;
	      DefgenericBinaryData (theEnv)->TypeCount += rptr->tcnt;
	    }
	  else
 ...
 *************************************************************/
static void
CL_BsaveRestrictionTypes (Environment * theEnv,
			  ConstructHeader * theDefgeneric, void *userBuffer)
{
  Defgeneric *gfunc = (Defgeneric *) theDefgeneric;
...
#else
	      dummy_type =
		(unsigned long) ((CLIPSInteger *) rptr->types[k])->contents;
#endif
	      CL_Gen_Write (&dummy_type, sizeof (unsigned long),
			    (FILE *) userBuffer);
	    }
	}
    }
 ...
 ***********************************************************************/
static void
CL_BloadStorageGenerics (Environment * theEnv)
{
  size_t space;
  ...
  if (DefgenericBinaryData (theEnv)->ModuleCount != 0L)
	...
	(DEFGENERIC_MODULE *) CL_genalloc (theEnv, space);
    }
  else
    return;
  if (DefgenericBinaryData (theEnv)->GenericCount != 0L)
    {
      space =
	(sizeof (Defgeneric) * DefgenericBinaryData (theEnv)->GenericCount);
      DefgenericBinaryData (theEnv)->DefgenericArray =
	(Defgeneric *) CL_genalloc (theEnv, space);
    }
  else
    return;
  if (DefgenericBinaryData (theEnv)->MethodCount != 0L)
    {
      space =
	(sizeof (Defmethod) * DefgenericBinaryData (theEnv)->MethodCount);
      DefgenericBinaryData (theEnv)->MethodArray =
	(Defmethod *) CL_genalloc (theEnv, space);
    }
  else
    return;
  if (DefgenericBinaryData (theEnv)->RestrictionCount != 0L)
	...
	(sizeof (RESTRICTION) *
	 DefgenericBinaryData (theEnv)->RestrictionCount);
      DefgenericBinaryData (theEnv)->RestrictionArray =
	(RESTRICTION *) CL_genalloc (theEnv, space);
    }
  else
    return;
  if (DefgenericBinaryData (theEnv)->TypeCount != 0L)
    {
      ...
      DefgenericBinaryData (theEnv)->TypeArray =
	(void * *) CL_genalloc (theEnv, space);
    }
}
 ...
 ********************************************************************/
static void
CL_BloadGenerics (Environment * theEnv)
{
  size_t space;
  ...
  if (DefgenericBinaryData (theEnv)->ModuleCount == 0L)
    return;
  CL_Bloadand_Refresh (theEnv, DefgenericBinaryData (theEnv)->ModuleCount,
		       sizeof (BSAVE_DEFGENERIC_MODULE), UpdateGenericModule);
  if (DefgenericBinaryData (theEnv)->GenericCount == 0L)
    return;
  CL_Bloadand_Refresh (theEnv, DefgenericBinaryData (theEnv)->GenericCount,
		       sizeof (BSAVE_GENERIC), UpdateGeneric);
  CL_Bloadand_Refresh (theEnv, DefgenericBinaryData (theEnv)->MethodCount,
		       sizeof (BSAVE_METHOD), UpdateMethod);
  CL_Bloadand_Refresh (theEnv,
		       DefgenericBinaryData (theEnv)->RestrictionCount,
		       sizeof (BSAVE_RESTRICTION), UpdateRestriction);
  CL_Bloadand_Refresh (theEnv, DefgenericBinaryData (theEnv)->TypeCount,
		       sizeof (long), UpdateType);
}
 ...
 *********************************************/
static void
UpdateGenericModule (Environment * theEnv, void *buf, unsigned long obji)
{
...
static void
UpdateGeneric (Environment * theEnv, void *buf, unsigned long obji)
{
  BSAVE_GENERIC *bgp;
  Defgeneric *gp;
  ...
  DefgenericBinaryData (theEnv)->DefgenericArray[obji].trace =
    DefgenericData (theEnv)->CL_WatchGenerics;
#endif
  DefgenericBinaryData (theEnv)->DefgenericArray[obji].methods =
    MethodPointer (bgp->methods);
...
static void
UpdateMethod (Environment * theEnv, void *buf, unsigned long obji)
{
  BSAVE_METHOD *bmth;
...
#if DEBUGGING_FUNCTIONS
  DefgenericBinaryData (theEnv)->MethodArray[obji].trace =
    DefgenericData (theEnv)->CL_WatchMethods;
#endif
  DefgenericBinaryData (theEnv)->MethodArray[obji].restrictionCount =
    bmth->restrictionCount;
  DefgenericBinaryData (theEnv)->MethodArray[obji].minRestrictions =
    bmth->minRestrictions;
  DefgenericBinaryData (theEnv)->MethodArray[obji].maxRestrictions =
    bmth->maxRestrictions;
  DefgenericBinaryData (theEnv)->MethodArray[obji].localVarCount =
    bmth->localVarCount;
  DefgenericBinaryData (theEnv)->MethodArray[obji].system = bmth->system;
  DefgenericBinaryData (theEnv)->MethodArray[obji].restrictions =
    RestrictionPointer (bmth->restrictions);
  DefgenericBinaryData (theEnv)->MethodArray[obji].actions =
...
static void
UpdateRestriction (Environment * theEnv, void *buf, unsigned long obji)
{
  BSAVE_RESTRICTION *brp;

  brp = (BSAVE_RESTRICTION *) buf;
  DefgenericBinaryData (theEnv)->RestrictionArray[obji].tcnt = brp->tcnt;
  DefgenericBinaryData (theEnv)->RestrictionArray[obji].types =
    TypePointer (brp->types);
  DefgenericBinaryData (theEnv)->RestrictionArray[obji].query =
    ExpressionPointer (brp->query);
}

static void
UpdateType (Environment * theEnv, void *buf, unsigned long obji)
{
#if OBJECT_SYSTEM
  DefgenericBinaryData (theEnv)->TypeArray[obji] =
    DefclassPointer (*(unsigned long *) buf);
#else
  if ((*(long *) buf) > INSTANCE_TYPE_CODE)
    {
      CL_PrintWarningID (theEnv, "GENRCBIN", 1, false);
      CL_WriteString (theEnv, STDWRN,
		      "COOL not installed!  User-defined class\n");
      CL_WriteString (theEnv, STDWRN,
		      "  in method restriction substituted with OBJECT.\n");
      DefgenericBinaryData (theEnv)->TypeArray[obji] =
 ...
 ***************************************************************/
static void
CL_Clear_BloadGenerics (Environment * theEnv)
{
  unsigned long i;
  size_t space;

  space =
    (sizeof (DEFGENERIC_MODULE) * DefgenericBinaryData (theEnv)->ModuleCount);
  if (space == 0L)
    return;
  CL_genfree (theEnv, DefgenericBinaryData (theEnv)->ModuleArray, space);
  ...
  space =
    (sizeof (RESTRICTION) * DefgenericBinaryData (theEnv)->RestrictionCount);
  if (space == 0L)
    return;
