...
static bool GetVariableDefinition (Environment *, const char *, bool *, bool,
				   struct token *);
static void AddDefglobal (Environment *, CLIPSLexeme *, UDFValue *,
			  struct expr *);
#endif
...
/*********************************************************************/
bool
CL_ParseDefglobal (Environment * theEnv, const char *readSource)
{
  ...
  /* defglobal's pretty print representation.  */
   /*===========================================*/
  ...
  else
    {
      CL_PPBackup (theEnv);
      CL_SavePPBuffer (theEnv,
		       CL_DefmoduleName (CL_GetCurrentModule (theEnv)));
      CL_SavePPBuffer (theEnv, " ");
  ...
  while (GetVariableDefinition
	 (theEnv, readSource, &defglobalError, tokenRead, &theToken))
    {
      tokenRead = false;
...
/***************************************************************/
static bool
GetVariableDefinition (Environment * theEnv,
		       const char *readSource,
		       bool *defglobalError,
		       bool tokenRead, struct token *theToken)
{
  ...
  if ((CL_Get_WatchItem (theEnv, "compilations") == 1)
	...
	CL_WriteString (theEnv, outRouter, "Defining defglobal: ");
  ...
  if (CL_FindImportExportConflict
      (theEnv, "defglobal", CL_GetCurrentModule (theEnv),
       variableName->contents))
    {
      CL_ImportExportConflictMessage (theEnv, "defglobal",
				      variableName->contents, NULL, NULL);
      *defglobalError = true;
      return false;
    }
#endif
...
/*********************************************************/
static void
AddDefglobal (Environment * theEnv,
	      CLIPSLexeme * name, UDFValue * vPtr, struct expr *ePtr)
{
...
#if DEBUGGING_FUNCTIONS
  defglobalPtr->watch =
    globalHad_Watch ? true : DefglobalData (theEnv)->CL_WatchGlobals;
#endif
  ...
  defglobalPtr->header.whichModule = (struct defmoduleItemHeader *)
    CL_GetModuleItem (theEnv, NULL,
		      CL_FindModuleItem (theEnv, "defglobal")->moduleIndex);
...
/*****************************************************************/
bool
CL_ReplaceGlobalVariable (Environment * theEnv, struct expr *ePtr)
{
  Defglobal *theGlobal;
  unsigned int count;
  ...
  theGlobal = (Defglobal *)
    CL_FindImportedConstruct (theEnv, "defglobal", NULL,
			      ePtr->lexemeValue->contents, &count, true,
			      NULL);
...
/*****************************************************************/
void
CL_GlobalReferenceErrorMessage (Environment * theEnv,
