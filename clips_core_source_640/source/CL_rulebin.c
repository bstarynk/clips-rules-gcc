...
static void Update_DefruleModule (Environment *, void *, unsigned long);
...
static void DeallocateDefrule_BloadData (Environment *);
...
/*****************************************************/
void
CL_DefruleBinarySetup (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, RULEBIN_DATA,
			      sizeof (struct defruleBinaryData),
			      DeallocateDefrule_BloadData);
...
/*******************************************************/
/* DeallocateDefrule_BloadData: Deallocates environment */
/*    data for the defrule bsave functionality.        */
/*******************************************************/
static void
DeallocateDefrule_BloadData (Environment * theEnv)
{
  ...
  for (i = 0; i < DefruleBinaryData (theEnv)->NumberOfJoins; i++)
    {
      CL_DestroyBetaMemory (theEnv, &DefruleBinaryData (theEnv)->JoinArray[i],
			    LHS);
      CL_DestroyBetaMemory (theEnv, &DefruleBinaryData (theEnv)->JoinArray[i],
			    RHS);
      CL_ReturnLeftMemory (theEnv, &DefruleBinaryData (theEnv)->JoinArray[i]);
      CL_ReturnRightMemory (theEnv,
			    &DefruleBinaryData (theEnv)->JoinArray[i]);
    }

  for (i = 0; i < DefruleBinaryData (theEnv)->NumberOf_DefruleModules; i++)
    {
  ...
  space =
    DefruleBinaryData (theEnv)->NumberOf_DefruleModules *
    sizeof (struct defruleModule);
  if (space != 0)
    CL_genfree (theEnv, DefruleBinaryData (theEnv)->ModuleArray, space);

  space = DefruleBinaryData (theEnv)->NumberOfDefrules * sizeof (Defrule);
  if (space != 0)
    CL_genfree (theEnv, DefruleBinaryData (theEnv)->DefruleArray, space);

  space =
    DefruleBinaryData (theEnv)->NumberOfJoins * sizeof (struct joinNode);
  if (space != 0)
    CL_genfree (theEnv, DefruleBinaryData (theEnv)->JoinArray, space);

  space =
    DefruleBinaryData (theEnv)->NumberOfLinks * sizeof (struct joinLink);
  if (space != 0)
    CL_genfree (theEnv, DefruleBinaryData (theEnv)->LinkArray, space);

  if (CL_Bloaded (theEnv))
    {
      CL_rm (theEnv, DefruleData (theEnv)->AlphaMemoryTable,
	     sizeof (ALPHA_MEMORY_HASH *) * ALPHA_MEMORY_HASH_SIZE);
    }
#endif
}
...
/*************************************************************/
/* CL_BsaveFind: Dete_rmines the amount of memory needed to save */
...
CL_BsaveFind (Environment * theEnv)
{
  ...
  CL_Save_BloadCount (theEnv,
		      DefruleBinaryData (theEnv)->NumberOf_DefruleModules);
  CL_Save_BloadCount (theEnv, DefruleBinaryData (theEnv)->NumberOfDefrules);
  CL_Save_BloadCount (theEnv, DefruleBinaryData (theEnv)->NumberOfJoins);
  CL_Save_BloadCount (theEnv, DefruleBinaryData (theEnv)->NumberOfLinks);
  ...
  for (theModule = CL_GetNextDefmodule (theEnv, NULL);
	   ...
	   theDefrule = CL_GetNextDefrule (theEnv, theDefrule))
	{
	 ...
	 /*================================================*/
	  ...
	  CL_MarkConstructHeaderNeededItems (&theDefrule->header,
					     theDefrule->header.bsaveID);
	 ...
	 /*===========================================*/
	  ...
	  ExpressionData (theEnv)->ExpressionCount +=
	    CL_ExpressionSize (theDefrule->dynamicSalience);
	 ...
	 /*==========================================*/
	  ...
	  for (theDisjunct = theDefrule;
	    ...
	    {
	      ExpressionData (theEnv)->ExpressionCount +=
		CL_ExpressionSize (theDisjunct->actions);
...
/************************************************/
static void
CL_BsaveExpressions (Environment * theEnv, FILE * fp)
{
  Defrule *theDefrule, *theDisjunct;
  Defmodule *theModule;
...
CL_BsaveStorage (Environment * theEnv, FILE * fp)
{
  ...
  CL_Gen_Write (&DefruleBinaryData (theEnv)->NumberOf_DefruleModules,
		sizeof (long), fp);
  CL_Gen_Write (&DefruleBinaryData (theEnv)->NumberOfDefrules, sizeof (long),
		fp);
  CL_Gen_Write (&DefruleBinaryData (theEnv)->NumberOfJoins, sizeof (long),
		fp);
  CL_Gen_Write (&DefruleBinaryData (theEnv)->NumberOfLinks, sizeof (long),
		fp);
  ...
  CL_Gen_Write (&value, sizeof (unsigned long), fp);
...
CL_BsaveBinaryItem (Environment * theEnv, FILE * fp)
{
  ...
  Defmodule *theModule;
  struct defruleModule *theModuleItem;
  struct bsave_DefruleModule temp_DefruleModule;
  ...
  space =
    (DefruleBinaryData (theEnv)->NumberOfDefrules *
     sizeof (struct bsaveDefrule)) +
    (DefruleBinaryData (theEnv)->NumberOfJoins *
     sizeof (struct bsaveJoinNode)) +
    (DefruleBinaryData (theEnv)->NumberOfLinks *
     sizeof (struct bsaveJoinLink)) +
    (DefruleBinaryData (theEnv)->NumberOf_DefruleModules *
     sizeof (struct bsave_DefruleModule));
  CL_Gen_Write (&space, sizeof (size_t), fp);
  ...
  for (theModule = CL_GetNextDefmodule (theEnv, NULL);
	...
	CL_GetModuleItem (theEnv, NULL,
      ...
      CL_Assign_BsaveDefmdlItemHdrVals (&temp_DefruleModule.header,
					&theModuleItem->header);
      CL_Gen_Write (&temp_DefruleModule, sizeof (struct bsave_DefruleModule),
		    fp);
    }
  ...
  Restore_BloadCount (theEnv,
		      &DefruleBinaryData (theEnv)->NumberOf_DefruleModules);
  Restore_BloadCount (theEnv, &DefruleBinaryData (theEnv)->NumberOfDefrules);
  Restore_BloadCount (theEnv, &DefruleBinaryData (theEnv)->NumberOfJoins);
  Restore_BloadCount (theEnv, &DefruleBinaryData (theEnv)->NumberOfLinks);
}
...
/************************************************************/
static void
CL_BsaveDisjuncts (Environment * theEnv, FILE * fp, Defrule * theDefrule)
{
  Defrule *theDisjunct;
  ...
  for (theDisjunct = theDefrule, first = true;
       ...
       theDisjunct = theDisjunct->disjunct, first = false)
    {
      ...
      CL_Assign_BsaveConstructHeaderVals (&tempDefrule.header,
					  &theDisjunct->header);
      ...
      tempDefrule.auto_Focus = theDisjunct->auto_Focus;
      ...
      if (theDisjunct->dynamicSalience != NULL)
	{
	  if (first)
	    {
	      tempDefrule.dynamicSalience =
		ExpressionData (theEnv)->ExpressionCount;
	      disjunctExpressionCount =
		ExpressionData (theEnv)->ExpressionCount;
	      ExpressionData (theEnv)->ExpressionCount +=
		CL_ExpressionSize (theDisjunct->dynamicSalience);
	    }
	  else
	    {
      ...
      else
	{
      ...
      CL_Gen_Write (&tempDefrule, sizeof (struct bsaveDefrule), fp);
    }
}
...
/********************************************/
static void
CL_BsaveJoins (Environment * theEnv, FILE * fp)
{
...
/**************************************************************/
static void
CL_BsaveTraverseJoins (Environment * theEnv,
		       FILE * fp, struct joinNode *joinPtr)
{
  for (; joinPtr != NULL; joinPtr = joinPtr->lastLevel)
    {
      if (joinPtr->marked)
	CL_BsaveJoin (theEnv, fp, joinPtr);
...
/********************************************/
static void
CL_BsaveJoin (Environment * theEnv, FILE * fp, struct joinNode *joinPtr)
{
  ...
  if (joinPtr->joinFromTheRight)
    {
      tempJoin.rightSideEntryStructure =
	CL_BsaveJoinIndex (joinPtr->rightSideEntryStructure);
    }
  else
    {
  ...
  tempJoin.networkTest =
    CL_HashedExpressionIndex (theEnv, joinPtr->networkTest);
  tempJoin.secondaryNetworkTest =
    CL_HashedExpressionIndex (theEnv, joinPtr->secondaryNetworkTest);
  ...
  if (joinPtr->ruleToActivate != NULL)
    {
      tempJoin.ruleToActivate = GetDisjunctIndex (joinPtr->ruleToActivate);
    }
  else
    {
  ...
  CL_Gen_Write (&tempJoin, sizeof (struct bsaveJoinNode), fp);
}
...
/********************************************/
static void
CL_BsaveLinks (Environment * theEnv, FILE * fp)
{
  Defrule *rulePtr, *disjunctPtr;
  Defmodule *theModule;
...
/**************************************************/
static void
CL_BsaveTraverseLinks (Environment * theEnv,
		       FILE * fp, struct joinNode *joinPtr)
{
  struct joinLink *theLink;
...
/********************************************/
static void
CL_BsaveLink (FILE * fp, struct joinLink *linkPtr)
{
  ...
  CL_Gen_Write (&tempLink, sizeof (struct bsaveJoinLink), fp);
}

/***********************************************************/
/* CL_Assign_BsavePatternHeaderValues: Assigns the appropriate */
/*   values to a bsave pattern header record.              */
/***********************************************************/
void
CL_Assign_BsavePatternHeaderValues (Environment * theEnv,
				    struct bsavePatternNodeHeader
				    *the_BsaveHeader,
				    struct patternNodeHeader *theHeader)
{
  the_BsaveHeader->multifieldNode = theHeader->multifieldNode;
  the_BsaveHeader->entryJoin = CL_BsaveJoinIndex (theHeader->entryJoin);
  the_BsaveHeader->rightHash =
    CL_HashedExpressionIndex (theEnv, theHeader->rightHash);
  the_BsaveHeader->singlefieldNode = theHeader->singlefieldNode;
  the_BsaveHeader->stopNode = theHeader->stopNode;
  the_BsaveHeader->beginSlot = theHeader->beginSlot;
  the_BsaveHeader->endSlot = theHeader->endSlot;
  the_BsaveHeader->selector = theHeader->selector;
}
...
CL_BloadStorage (Environment * theEnv)
{
  size_t space;
   ...
   /*=================================================*/
  /* Dete_rmine the number of defrule, defruleModule, */
  ...
  CL_GenReadBinary (theEnv,
		    &DefruleBinaryData (theEnv)->NumberOf_DefruleModules,
		    sizeof (long));
  CL_GenReadBinary (theEnv, &DefruleBinaryData (theEnv)->NumberOfDefrules,
		    sizeof (long));
  CL_GenReadBinary (theEnv, &DefruleBinaryData (theEnv)->NumberOfJoins,
		    sizeof (long));
  CL_GenReadBinary (theEnv, &DefruleBinaryData (theEnv)->NumberOfLinks,
		    sizeof (long));
  CL_GenReadBinary (theEnv, &DefruleBinaryData (theEnv)->RightPrimeIndex,
		    sizeof (long));
  CL_GenReadBinary (theEnv, &DefruleBinaryData (theEnv)->LeftPrimeIndex,
		    sizeof (long));
  ...
  if (DefruleBinaryData (theEnv)->NumberOf_DefruleModules == 0)
    {
  ...
  DefruleBinaryData (theEnv)->ModuleArray =
    (struct defruleModule *) CL_genalloc (theEnv, space);
  ...
  DefruleBinaryData (theEnv)->DefruleArray =
    (Defrule *) CL_genalloc (theEnv, space);
  ...
  space =
    DefruleBinaryData (theEnv)->NumberOfJoins * sizeof (struct joinNode);
  DefruleBinaryData (theEnv)->JoinArray =
    (struct joinNode *) CL_genalloc (theEnv, space);
  ...
  DefruleBinaryData (theEnv)->LinkArray =
    (struct joinLink *) CL_genalloc (theEnv, space);
}
...
CL_BloadBinaryItem (Environment * theEnv)
{
  ...
  CL_Bloadand_Refresh (theEnv, DefruleBinaryData (theEnv)->NumberOfDefrules,
  ...
  CL_Bloadand_Refresh (theEnv, DefruleBinaryData (theEnv)->NumberOfJoins,
  ...
  CL_Bloadand_Refresh (theEnv, DefruleBinaryData (theEnv)->NumberOfLinks,
		       sizeof (struct bsaveJoinLink), UpdateLink);

  DefruleData (theEnv)->RightPrimeJoins =
    CL_BloadJoinLinkPointer (DefruleBinaryData (theEnv)->RightPrimeIndex);
  DefruleData (theEnv)->LeftPrimeJoins =
    CL_BloadJoinLinkPointer (DefruleBinaryData (theEnv)->LeftPrimeIndex);
}

/**********************************************/
/* Update_DefruleModule: CL_Bload refresh routine */
/*   for defrule module data structures.      */
/**********************************************/
static void
Update_DefruleModule (Environment * theEnv, void *buf, unsigned long obji)
{
  struct bsave_DefruleModule *bdmPtr;
...
/****************************************/
static void
UpdateDefrule (Environment * theEnv, void *buf, unsigned long obji)
{
  ...
  DefruleBinaryData (theEnv)->DefruleArray[obji].actions =
    ExpressionPointer (br->actions);
  DefruleBinaryData (theEnv)->DefruleArray[obji].logicalJoin =
    CL_BloadJoinPointer (br->logicalJoin);
  DefruleBinaryData (theEnv)->DefruleArray[obji].lastJoin =
    CL_BloadJoinPointer (br->lastJoin);
  DefruleBinaryData (theEnv)->DefruleArray[obji].disjunct =
    CL_BloadDefrulePointer (DefruleBinaryData (theEnv)->DefruleArray,
			    br->disjunct);
  DefruleBinaryData (theEnv)->DefruleArray[obji].salience = br->salience;
  DefruleBinaryData (theEnv)->DefruleArray[obji].localVarCnt =
    br->localVarCnt;
  DefruleBinaryData (theEnv)->DefruleArray[obji].complexity = br->complexity;
  DefruleBinaryData (theEnv)->DefruleArray[obji].auto_Focus = br->auto_Focus;
  DefruleBinaryData (theEnv)->DefruleArray[obji].executing = 0;
  DefruleBinaryData (theEnv)->DefruleArray[obji].afterBreakpoint = 0;
#if DEBUGGING_FUNCTIONS
  DefruleBinaryData (theEnv)->DefruleArray[obji].watchActivation =
    CL_AgendaData (theEnv)->CL_WatchActivations;
  DefruleBinaryData (theEnv)->DefruleArray[obji].watchFiring =
    DefruleData (theEnv)->CL_WatchRules;
#endif
}
...
/*************************************/
static void
UpdateJoin (Environment * theEnv, void *buf, unsigned long obji)
{
  ...
  DefruleBinaryData (theEnv)->JoinArray[obji].joinFromTheRight =
    bj->joinFromTheRight;
  DefruleBinaryData (theEnv)->JoinArray[obji].patternIsNegated =
    bj->patternIsNegated;
  DefruleBinaryData (theEnv)->JoinArray[obji].patternIsExists =
    bj->patternIsExists;
  DefruleBinaryData (theEnv)->JoinArray[obji].depth = bj->depth;
  DefruleBinaryData (theEnv)->JoinArray[obji].rhsType = bj->rhsType;
  DefruleBinaryData (theEnv)->JoinArray[obji].networkTest =
    HashedExpressionPointer (bj->networkTest);
  DefruleBinaryData (theEnv)->JoinArray[obji].secondaryNetworkTest =
    HashedExpressionPointer (bj->secondaryNetworkTest);
  DefruleBinaryData (theEnv)->JoinArray[obji].leftHash =
    HashedExpressionPointer (bj->leftHash);
  DefruleBinaryData (theEnv)->JoinArray[obji].rightHash =
    HashedExpressionPointer (bj->rightHash);
  DefruleBinaryData (theEnv)->JoinArray[obji].nextLinks =
    CL_BloadJoinLinkPointer (bj->nextLinks);
  DefruleBinaryData (theEnv)->JoinArray[obji].lastLevel =
    CL_BloadJoinPointer (bj->lastLevel);

  if (bj->joinFromTheRight == true)
    {
      DefruleBinaryData (theEnv)->JoinArray[obji].rightSideEntryStructure =
	(void *) CL_BloadJoinPointer (bj->rightSideEntryStructure);
    }
  else
    {
      DefruleBinaryData (theEnv)->JoinArray[obji].rightSideEntryStructure =
  ...
  DefruleBinaryData (theEnv)->JoinArray[obji].rightMatchNode =
    CL_BloadJoinPointer (bj->rightMatchNode);
  DefruleBinaryData (theEnv)->JoinArray[obji].ruleToActivate =
    CL_BloadDefrulePointer (DefruleBinaryData (theEnv)->DefruleArray,
			    bj->ruleToActivate);
  ...
  CL_AddBetaMemoriesToJoin (theEnv,
			    &DefruleBinaryData (theEnv)->JoinArray[obji]);
}
...
UpdateLink (Environment * theEnv, void *buf, unsigned long obji)
{
  ...
  DefruleBinaryData (theEnv)->LinkArray[obji].enterDirection =
    bj->enterDirection;
  DefruleBinaryData (theEnv)->LinkArray[obji].next =
    CL_BloadJoinLinkPointer (bj->next);
  DefruleBinaryData (theEnv)->LinkArray[obji].join =
    CL_BloadJoinPointer (bj->join);
}
...
/************************************************************/
void
CL_UpdatePatternNodeHeader (Environment * theEnv,
			    struct patternNodeHeader *theHeader,
			    struct bsavePatternNodeHeader *the_BsaveHeader)
{
  struct joinNode *theJoin;

  theHeader->singlefieldNode = the_BsaveHeader->singlefieldNode;
  theHeader->multifieldNode = the_BsaveHeader->multifieldNode;
  theHeader->stopNode = the_BsaveHeader->stopNode;
  theHeader->beginSlot = the_BsaveHeader->beginSlot;
  theHeader->endSlot = the_BsaveHeader->endSlot;
  theHeader->selector = the_BsaveHeader->selector;
  theHeader->initialize = 0;
  theHeader->marked = 0;
  theHeader->firstHash = NULL;
  theHeader->lastHash = NULL;
  theHeader->rightHash = HashedExpressionPointer (the_BsaveHeader->rightHash);

  theJoin = CL_BloadJoinPointer (the_BsaveHeader->entryJoin);
...
/**************************************/
/* CL_Clear_Bload: Defrule clear routine  */
...
CL_Clear_Bload (Environment * theEnv)
{
  ...
  for (i = 0; i < DefruleBinaryData (theEnv)->NumberOfJoins; i++)
    {
      CL_FlushBetaMemory (theEnv, &DefruleBinaryData (theEnv)->JoinArray[i],
			  LHS);
      CL_ReturnLeftMemory (theEnv, &DefruleBinaryData (theEnv)->JoinArray[i]);
      CL_FlushBetaMemory (theEnv, &DefruleBinaryData (theEnv)->JoinArray[i],
			  RHS);
  ...
  if (space != 0)
    CL_genfree (theEnv, DefruleBinaryData (theEnv)->ModuleArray, space);
  DefruleBinaryData (theEnv)->NumberOf_DefruleModules = 0;
...
/*******************************************************/
/* CL_Bload_DefruleModuleReference: Returns the defrule    */
/*   module pointer for using with the bload function. */
/*******************************************************/
void *
CL_Bload_DefruleModuleReference (Environment * theEnv, unsigned long theIndex)
{
