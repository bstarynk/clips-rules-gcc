...
/***********************************************************/
void
CL_AllocateExpressions (Environment * theEnv)
{
  size_t space;

  CL_GenReadBinary (theEnv, &ExpressionData (theEnv)->NumberOfExpressions,
		    sizeof (long));
  if (ExpressionData (theEnv)->NumberOfExpressions == 0L)
	...
	(struct expr *) CL_genalloc (theEnv, space);
    }
}
...
/**********************************************/
void
CL_RefreshExpressions (Environment * theEnv)
{
  if (ExpressionData (theEnv)->ExpressionArray == NULL)
    return;
 ...
 *********************************************************/
static void
UpdateExpression (Environment * theEnv, void *buf, unsigned long obji)
{
  ...
  switch (bexp->type)
	...
	CL_BloadData (theEnv)->FunctionArray[bexp->value];
      break;

    case GCALL:
#if DEFGENERIC_CONSTRUCT
      ExpressionData (theEnv)->ExpressionArray[obji].value =
	GenericPointer (bexp->value);
#else
    ...
    default:
      if (CL_EvaluationData (theEnv)->PrimitivesArray[bexp->type] == NULL)
	break;
  ...
  if (theIndex == ULONG_MAX)
    ...
    {
      ExpressionData (theEnv)->ExpressionArray[obji].nextArg =
	(struct expr *) &ExpressionData (theEnv)->ExpressionArray[theIndex];
    }
  ...
  if (theIndex == ULONG_MAX)
    ...
    {
      ExpressionData (theEnv)->ExpressionArray[obji].argList =
	(struct expr *) &ExpressionData (theEnv)->ExpressionArray[theIndex];
    }
}
...
/*********************************************/
void
CL_Clear_BloadedExpressions (Environment * theEnv)
{
  unsigned long i;
  size_t space;
  ...
  if (space != 0)
    CL_genfree (theEnv, ExpressionData (theEnv)->ExpressionArray, space);
 ...
 ***************************************************/
void
CL_FindHashedExpressions (Environment * theEnv)
{
  unsigned i;
  ...
  for (i = 0; i < EXPRESSION_HASH_SIZE; i++)
    for (exphash = ExpressionData (theEnv)->ExpressionHashTable[i];
	 exphash != NULL; exphash = exphash->next)
      {
 ...
 ***************************************************/
void
CL_BsaveHashedExpressions (Environment * theEnv, FILE * fp)
{
  unsigned i;
  EXPRESSION_HN *exphash;
...
/***************************************************************/
void
CL_BsaveConstructExpressions (Environment * theEnv, FILE * fp)
{
...
/***************************************/
void
CL_BsaveExpression (Environment * theEnv, struct expr *testPtr, FILE * fp)
{
  ...
  while (testPtr != NULL)
      ...
      else
	{
	  newIndex =
	    ExpressionData (theEnv)->ExpressionCount +
	    CL_ExpressionSize (testPtr->argList);
