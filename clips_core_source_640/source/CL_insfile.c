...
/*            Env_Load_Instances, EnvRestore_Instances,         */
/*            Env_Load_InstancesFromString, and                */
/*            EnvRestore_InstancesFromString are processed.   */
/*                                                           */
...
static long CL_Instances_SaveCommandParser (UDFContext *,
					    long (*)(Environment *,
						     const char *,
						     CL_SaveScope,
						     Expression *, bool));
static struct classItem *Process_SaveClassList (Environment *, const char *,
						Expression *, CL_SaveScope,
						bool);
static void Return_SaveClassList (Environment *, struct classItem *);
static long CL_SaveOrMark_Instances (Environment *, FILE *, int,
				     struct classItem *, bool, bool,
				     void (*)(Environment *, FILE *,
					      Instance *));
static long CL_SaveOrMark_InstancesOfClass (Environment *, FILE *,
					    Defmodule *, int, Defclass *,
					    bool, int, void (*)(Environment *,
								FILE *,
								Instance *));
static void CL_SaveSingleInstanceText (Environment *, FILE *, Instance *);
static void ProcessFileErrorMessage (Environment *, const char *,
				     const char *);
#if BSAVE_INSTANCES
...
static long CL_LoadOrRestore_Instances (Environment *, const char *, bool,
					bool);

#if BLOAD_INSTANCES
static bool VerifyBinaryHeader (Environment *, const char *);
static bool CL_LoadSingleBinaryInstance (Environment *);
static void Binary_LoadInstanceError (Environment *, CLIPSLexeme *,
				      Defclass *);
static void CreateSlotValue (Environment *, UDFValue *,
			     struct bsaveSlotValueAtom *, unsigned long);
 ...
 ***************************************************/
void
CL_SetupInstanceFileCommands (Environment * theEnv)
{
#if BLOAD_INSTANCES || BSAVE_INSTANCES
  CL_AllocateEnvironmentData (theEnv, INSTANCE_FILE_DATA,
			      sizeof (struct instanceFileData), NULL);
...
#if (! RUN_TIME)
  CL_AddUDF (theEnv, "save-instances", "l", 1, UNBOUNDED, "y;sy",
	     CL_Save_InstancesCommand, "CL_Save_InstancesCommand", NULL);
  CL_AddUDF (theEnv, "load-instances", "l", 1, 1, "sy",
	     CL_Load_InstancesCommand, "CL_Load_InstancesCommand", NULL);
  CL_AddUDF (theEnv, "restore-instances", "l", 1, 1, "sy",
	     Restore_InstancesCommand, "Restore_InstancesCommand", NULL);

#if BSAVE_INSTANCES
  CL_AddUDF (theEnv, "bsave-instances", "l", 1, UNBOUNDED, "y;sy",
	     CL_Binary_Save_InstancesCommand,
	     "CL_Binary_Save_InstancesCommand", NULL);
#endif
#if BLOAD_INSTANCES
  CL_AddUDF (theEnv, "bload-instances", "l", 1, 1, "sy",
	     CL_Binary_Load_InstancesCommand,
	     "CL_Binary_Load_InstancesCommand", NULL);
#endif
 ...
 ****************************************************************************/
void
CL_Save_InstancesCommand (Environment * theEnv,
			  UDFContext * context, UDFValue * returnValue)
{
  returnValue->integerValue =
    CL_CreateInteger (theEnv,
		      CL_Instances_SaveCommandParser (context,
						      CL_Save_InstancesDriver));
}

/******************************************************
  NAME         : CL_Load_InstancesCommand
 ...
 ******************************************************/
void
CL_Load_InstancesCommand (Environment * theEnv,
			  UDFContext * context, UDFValue * returnValue)
{
  ...
  instanceCount = CL_Load_Instances (theEnv, fileFound);
 ...
 ***************************************************/
long
CL_Load_Instances (Environment * theEnv, const char *file)
{
  return (CL_LoadOrRestore_Instances (theEnv, file, true, true));
}

/***************************************************
  NAME         : CL_Load_InstancesFromString
 ...
 ***************************************************/
long
CL_Load_InstancesFromString (Environment * theEnv,
			     const char *theString, size_t theMax)
{
  long theCount;
  ...
  theCount = CL_LoadOrRestore_Instances (theEnv, theStrRouter, true, false);
 ...
 *********************************************************/
void
Restore_InstancesCommand (Environment * theEnv,
			  UDFContext * context, UDFValue * returnValue)
{
  const char *fileFound;
  UDFValue theArg;
  ...
  instanceCount = Restore_Instances (theEnv, fileFound);
  if (CL_EvaluationData (theEnv)->CL_EvaluationError)
    {
 ...
 ***************************************************/
long
Restore_Instances (Environment * theEnv, const char *file)
{
  return (CL_LoadOrRestore_Instances (theEnv, file, false, true));
}

/***************************************************
  NAME         : Restore_InstancesFromString
 ...
 ***************************************************/
long
Restore_InstancesFromString (Environment * theEnv,
			     const char *theString, size_t theMax)
{
  long theCount;
  const char *theStrRouter = "*** load-instances-from-string ***";
  ...
  theCount = CL_LoadOrRestore_Instances (theEnv, theStrRouter, false, false);
...
/*******************************************************
  NAME         : CL_Binary_Load_InstancesCommand
  DESCRIPTION  : H/L interface for loading
 ...
 *******************************************************/
void
CL_Binary_Load_InstancesCommand (Environment * theEnv,
				 UDFContext * context, UDFValue * returnValue)
{
  const char *fileFound;
  UDFValue theArg;
  ...
  instanceCount = CL_Binary_Load_Instances (theEnv, fileFound);
  if (CL_EvaluationData (theEnv)->CL_EvaluationError)
    {
 ...
 ****************************************************/
long
CL_Binary_Load_Instances (Environment * theEnv, const char *theFile)
{
 ...
 *******************************************************/
long
CL_Save_Instances (Environment * theEnv,
		   const char *file, CL_SaveScope saveCode)
{
  return CL_Save_InstancesDriver (theEnv, file, saveCode, NULL, true);
}

/*******************************************************
  NAME         : CL_Save_InstancesDriver
  DESCRIPTION  : CL_Saves current instances to named file
  INPUTS       : 1) The name of the output file
                 2) A flag indicating whether to
                    save local (current module only)
 ...
 *******************************************************/
long
CL_Save_InstancesDriver (Environment * theEnv,
			 const char *file,
			 CL_SaveScope saveCode,
			 Expression * classExpressionList, bool inheritFlag)
{
  FILE *sfile = NULL;
  ...
  SetFast_Save (theEnv, sfile);
  instanceCount = CL_SaveOrMark_Instances (theEnv, sfile, saveCode, classList,
					   inheritFlag, true,
					   CL_SaveSingleInstanceText);
...
/****************************************************************************
  NAME         : CL_Binary_Save_InstancesCommand
  DESCRIPTION  : H/L interface for saving
                   current instances to a binary file
  INPUTS       : None
  RETURNS      : The number of instances saved
  SIDE EFFECTS : CL_Instances saved (in binary fo_rmat) to named file
  NOTES        : H/L Syntax :
                 (bsave-instances <file> [local|visible [[inherit] <class>+]])
 *****************************************************************************/
void
CL_Binary_Save_InstancesCommand (Environment * theEnv,
				 UDFContext * context, UDFValue * returnValue)
{
  returnValue->integerValue =
    CL_CreateInteger (theEnv,
		      CL_Instances_SaveCommandParser (context,
						      CL_Binary_Save_InstancesDriver));
}

/*******************************************************
  NAME         : CL_Binary_Save_Instances
 ...
 *******************************************************/
long
CL_Binary_Save_Instances (Environment * theEnv,
			  const char *file, CL_SaveScope saveCode)
{
  return CL_Binary_Save_InstancesDriver (theEnv, file, saveCode, NULL, true);
}

/*******************************************************
  NAME         : CL_Binary_Save_InstancesDriver
  DESCRIPTION  : CL_Saves current instances to binary file
  INPUTS       : 1) The name of the output file
                 2) A flag indicating whether to
                    save local (current module only)
 ...
 *******************************************************/
long
CL_Binary_Save_InstancesDriver (Environment * theEnv,
				const char *file,
				CL_SaveScope saveCode,
				...
				bool inheritFlag)
{
  ...
  classList =
    Process_SaveClassList (theEnv, "bsave-instances", classExpressionList,
			   saveCode, inheritFlag);
  if ((classList == NULL) && (classExpressionList != NULL))
    return (0L);
  ...
  instanceCount =
    CL_SaveOrMark_Instances (theEnv, NULL, saveCode, classList, inheritFlag,
  ...
  fwrite (&InstanceFileData (theEnv)->BinaryInstanceFileSize,
	  sizeof (unsigned long), 1, bsaveFP);
  fwrite (&instanceCount, sizeof (long), 1, bsaveFP);

  CL_SetAtomicValueIndices (theEnv, false);
  CL_SaveOrMark_Instances (theEnv, bsaveFP, saveCode, classList,
...
/******************************************************
  NAME         : CL_Instances_SaveCommandParser
 ...
 ******************************************************/
static long
CL_Instances_SaveCommandParser (UDFContext * context,
				long (*saveFunction) (Environment *,
						      const char *,
						      CL_SaveScope,
						      Expression *, bool))
{
  ...
  return ((*saveFunction)
	  (theEnv, fileFound, saveCode, classList, inheritFlag));
}

/****************************************************
  NAME         : Process_SaveClassList
  ...
  NOTES        : None
 ****************************************************/
static struct classItem *
Process_SaveClassList (Environment * theEnv,
		       const char *functionName,
		       Expression * classExps,
		       CL_SaveScope saveCode, bool inheritFlag)
{
  ...
  while (classExps != NULL)
	...
	{
	  theDefclass =
	    CL_LookupDefclassAnywhere (theEnv, currentModule,
				       tmp.lexemeValue->contents);
	}
      else
	{
	  theDefclass =
	    CL_LookupDefclassByMdlOrScope (theEnv, tmp.lexemeValue->contents);
	}
...
ProcessClassListError:
  if (inheritFlag)
    CL_ExpectedTypeError1 (theEnv, functionName, argIndex,
			   "'valid class name'");
  else
    CL_ExpectedTypeError1 (theEnv, functionName, argIndex,
			   "'valid concrete class name'");

  Return_SaveClassList (theEnv, head);
...
/****************************************************
  NAME         : Return_SaveClassList
  DESCRIPTION  : Deallocates the class data object
                 list created by Process_SaveClassList
 ...
 ****************************************************/
static void
Return_SaveClassList (Environment * theEnv, struct classItem *classList)
{
 ...
 ***************************************************/
static long
CL_SaveOrMark_Instances (Environment * theEnv,
			 FILE * theOutput,
			 int saveCode,
			 struct classItem *classList,
			 bool inheritFlag,
			 bool interruptOK,
			 void (*saveInstanceFunc) (Environment *, FILE *,
						   Instance *))
{
  Defmodule *currentModule;
  int traversalID;
      ...
      if (traversalID != -1)
	{
	  for (tmp = classList;
		  ...
		  || (CL_EvaluationData (theEnv)->CL_HaltExecution
		      && interruptOK))); tmp = tmp->next)
	    instanceCount +=
	      CL_SaveOrMark_InstancesOfClass (theEnv, theOutput,
					      currentModule, saveCode,
...
/***************************************************
  NAME         : CL_SaveOrMark_InstancesOfClass
 ...
 ***************************************************/
static long
CL_SaveOrMark_InstancesOfClass (Environment * theEnv,
				FILE * theOutput,
				Defmodule * currentModule,
				int saveCode,
				Defclass * theDefclass,
				bool inheritFlag,
				int traversalID,
				void (*saveInstanceFunc) (Environment *,
							  FILE *, Instance *))
{
  ...
  if (inheritFlag)
      ...
      for (i = 0; i < theDefclass->directSubclasses.classCount; i++)
	{
	  ...
	  instanceCount +=
	    CL_SaveOrMark_InstancesOfClass (theEnv, theOutput, currentModule,
					    saveCode, subclass, true,
					    traversalID, saveInstanceFunc);
	}
    }
  return (instanceCount);
 ...
 ***************************************************/
static void
CL_SaveSingleInstanceText (Environment * theEnv,
			   FILE * fast_SaveFile, Instance * theInstance)
{
  long i;
  InstanceSlot *sp;
  const char *logicalName = (const char *) fast_SaveFile;
  ...
  CL_WriteString (theEnv, logicalName,
		  theInstance->cls->header.name->contents);
 ...
 ***************************************************/
static void
CL_WriteBinaryHeader (Environment * theEnv, FILE * bsaveFP)
{
  fwrite (InstanceFileData (theEnv)->InstanceBinaryPrefixID,
	  (STD_SIZE) (strlen
		      (InstanceFileData (theEnv)->InstanceBinaryPrefixID) +
		      1), 1, bsaveFP);
  fwrite (InstanceFileData (theEnv)->InstanceBinaryVersionID,
	  (STD_SIZE) (strlen
		      (InstanceFileData (theEnv)->InstanceBinaryVersionID) +
		      1), 1, bsaveFP);
}
 ...
 ***************************************************/
static void
MarkSingleInstance (Environment * theEnv,
		    FILE * theOutput, Instance * theInstance)
{
#if MAC_XCD
  ...
  for (i = 0; i < theInstance->cls->instanceSlotCount; i++)
      ...
      if (sp->desc->multiple)
	{
	  for (j = 0; j < sp->multifieldValue->length; j++)
	    MarkNeededAtom (theEnv,
			    sp->multifieldValue->contents[j].header->type,
 ...
 ***************************************************/
static void
MarkNeededAtom (Environment * theEnv, unsigned short type, void *value)
{
  InstanceFileData (theEnv)->BinaryInstanceFileSize +=
    sizeof (struct bsaveSlotValueAtom);
  ...
  switch (type)
    ...
    case INSTANCE_ADDRESS_TYPE:
      CL_GetFull_InstanceName (theEnv, (Instance *) value)->neededSymbol =
	true;
      break;
    }
}
 ...
 ****************************************************/
static void
CL_SaveSingleInstanceBinary (Environment * theEnv,
			     FILE * bsaveFP, Instance * theInstance)
{
  ...
  fwrite (&theInstance->cls->instanceSlotCount, sizeof (short), 1, bsaveFP);
 ...
 ***************************************************/
static void
CL_SaveAtomBinary (Environment * theEnv,
		   unsigned short type, void *value, FILE * bsaveFP)
{
  ...
  switch (type)
    ...
    case INSTANCE_ADDRESS_TYPE:
      ...
      bsa.value =
	CL_GetFull_InstanceName (theEnv, (Instance *) value)->bucket;
      break;
    default:
      bsa.value = ULONG_MAX;
    }
...
/**********************************************************************
  NAME         : CL_LoadOrRestore_Instances
  DESCRIPTION  : CL_Loads instances from named file
 ...
 **********************************************************************/
static long
CL_LoadOrRestore_Instances (Environment * theEnv,
			    const char *file, bool usemsgs, bool isFileName)
{
  UDFValue temp;
  ...
  if (isFileName)
      ...
      if ((sfile = CL_GenOpen (theEnv, file, "r")) == NULL)
	{
      ...
      svload = CL_GetFast_Load (theEnv);
      ilog = (char *) sfile;
      SetFast_Load (theEnv, sfile);
    }
  else
    {
      ilog = file;
  ...
  top =
    CL_GenConstant (theEnv, FCALL, CL_FindFunction (theEnv, "make-instance"));
  ...
  while ((DefclassData (theEnv)->ObjectParseToken.tknType != STOP_TOKEN)
	 && (CL_EvaluationData (theEnv)->CL_HaltExecution != true))
    {
 ...
 ***************************************************/
static void
ProcessFileErrorMessage (Environment * theEnv,
 ...
 *******************************************************/
static bool
VerifyBinaryHeader (Environment * theEnv, const char *theFile)
{
  char buf[20];

  CL_GenReadBinary (theEnv, buf,
		    (strlen
		     (InstanceFileData (theEnv)->InstanceBinaryPrefixID) +
		     1));
  ...
  if (strcmp (buf, InstanceFileData (theEnv)->InstanceBinaryVersionID) != 0)
    {
      ...
      CL_WriteString (theEnv, STDERR, theFile);
      CL_WriteString (theEnv, STDERR,
		      "' is not a compatible binary instances file.\n");
      return false;
    }
  return true;
}
 ...
 ***************************************************/
static bool
CL_LoadSingleBinaryInstance (Environment * theEnv)
{
  CLIPSLexeme *instanceName, *className;
  ...
  if (theDefclass->instanceSlotCount != slotCount)
    {
      Binary_LoadInstanceError (theEnv, instanceName, theDefclass);
      return false;
    }
  ...
  /* ====================================
     ...
     ==================================== */
  bsArray =
    (struct bsaveSlotValue *) CL_gm2 (theEnv,
				      (sizeof (struct bsaveSlotValue) *
				       slotCount));
  BufferedRead (theEnv, bsArray,
		(sizeof (struct bsaveSlotValue) * slotCount));
  ...
  if (totalValueCount != 0L)
	...
	CL_gm2 (theEnv,
		(totalValueCount * sizeof (struct bsaveSlotValueAtom)));
  ...
  for (i = 0, j = 0L; i < slotCount; i++)
	...
	goto CL_LoadError;
      CreateSlotValue (theEnv, &slotValue,
		       (struct bsaveSlotValueAtom *) &bsaArray[j],
		       bsArray[i].valueCount);

      if (CL_PutSlotValue
	  (theEnv, newInstance, sp, &slotValue, &junkValue,
	   "bload-instances") != PSE_NO_ERROR)
	goto CL_LoadError;
 ...
 ***************************************************/
static void
Binary_LoadInstanceError (Environment * theEnv,
			  CLIPSLexeme * instanceName, Defclass * theDefclass)
{
  CL_PrintErrorID (theEnv, "INSFILE", 4, false);
  CL_WriteString (theEnv, STDERR,
		  "Function 'bload-instances' is unable to load instance [");
 ...
 ***************************************************/
static void
CreateSlotValue (Environment * theEnv,
  ...
  if (valueCount == 0)
      ...
      for (i = 0; i < valueCount; i++)
	{
	  returnValue->multifieldValue->contents[i].value =
	    GetBinaryAtomValue (theEnv, &bsaValues[i - 1]);
	}
    }
}
 ...
 ***************************************************/
static void *
GetBinaryAtomValue (Environment * theEnv, struct bsaveSlotValueAtom *ba)
{
  switch (ba->type)
    {
    case SYMBOL_TYPE:
 ...
 ***************************************************/
static void
BufferedRead (Environment * theEnv, void *buf, size_t bufsz)
{
  ...
  if (InstanceFileData (theEnv)->CurrentReadBuffer != NULL)
      ...
      if (bufsz > MAX_BLOCK_SIZE)
	{
	  ...
	  if (bufsz >
	      (InstanceFileData (theEnv)->BinaryInstanceFileSize -
	       InstanceFileData (theEnv)->BinaryInstanceFileOffset))
	    {
 ...
 *****************************************************/
static void
FreeReadBuffer (Environment * theEnv)
{
  if (InstanceFileData (theEnv)->CurrentReadBufferSize != 0L)
    {
      CL_genfree (theEnv, InstanceFileData (theEnv)->CurrentReadBuffer,
		  InstanceFileData (theEnv)->CurrentReadBufferSize);
