...
static void CL_WriteStringCallback (Environment *, const char *, const char *,
				    void *);
...
static bool CreateReadStringSource (Environment *, const char *, const char *,
				    size_t, size_t);
static void DeallocateStringRouterData (Environment *);
static String_BuilderRouter *FindString_BuilderRouter (Environment *,
						       const char *);
static bool QueryString_BuilderCallback (Environment *, const char *, void *);
static void CL_WriteString_BuilderCallback (Environment *, const char *,
					    const char *, void *);
...
/**********************************************************/
void
CL_InitializeStringRouter (Environment * theEnv)
{
  CL_AllocateEnvironmentData (theEnv, STRING_ROUTER_DATA,
			      sizeof (struct stringRouterData),
			      DeallocateStringRouterData);

  CL_AddRouter (theEnv, "string", 0, QueryStringCallback,
		CL_WriteStringCallback, ReadStringCallback,
		UnreadStringCallback, NULL, NULL);
  CL_AddRouter (theEnv, "string_Builder", 0, QueryString_BuilderCallback,
		CL_WriteString_BuilderCallback, NULL, NULL, NULL, NULL);
}
...
/*******************************************/
static void
DeallocateStringRouterData (Environment * theEnv)
{
...
/*********************************************************************/
static bool
QueryStringCallback (Environment * theEnv,
...
/**********************************************************/
static void
CL_WriteStringCallback (Environment * theEnv,
			...
			const char *str, void *context)
{
  ...
  if (head->read_WriteType != WRITE_STRING)
    return;

  if (head->maximumPosition == 0)
    return;

  if ((head->currentPosition + 1) >= head->maximumPosition)
    return;

  CL_genstrncpy (&head->writeString[head->currentPosition],
		 str,
		 (STD_SIZE) (head->maximumPosition - head->currentPosition) -
		 1);
...
/********************************************************/
static int
ReadStringCallback (Environment * theEnv,
		    const char *logicalName, void *context)
{
  struct stringRouter *head;
  int rc;
  ...
  if (head->read_WriteType != READ_STRING)
    return (EOF);
...
/************************************************************/
static int
UnreadStringCallback (Environment * theEnv,
...
/************************************************/
bool
CL_OpenStringSource (Environment * theEnv,
		     const char *name,
		     const char *str, size_t currentPosition)
{
  ...
  return (CreateReadStringSource
	  (theEnv, name, str, currentPosition, maximumPosition));
}
...
/******************************************************/
bool
CL_OpenTextSource (Environment * theEnv,
		   const char *name,
		   const char *str,
		   size_t currentPosition, size_t maximumPosition)
{
  if (str == NULL)
    {
      currentPosition = 0;
  ...
  return CreateReadStringSource (theEnv, name, str, currentPosition,
				 maximumPosition);
}
...
/******************************************************************/
static bool
CreateReadStringSource (Environment * theEnv,
			const char *name,
			const char *str,
			size_t currentPosition, size_t maximumPosition)
{
  ...
  if (FindStringRouter (theEnv, name) != NULL)
    return false;
...
/**********************************************/
bool
CL_CloseStringSource (Environment * theEnv, const char *name)
{
...
/******************************************************************/
bool
CL_OpenStringDestination (Environment * theEnv,
			  const char *name, char *str, size_t maximumPosition)
{
  struct stringRouter *newStringRouter;
  char *theName;
...
/***************************************************/
bool
CL_CloseStringDestination (Environment * theEnv, const char *name)
{
...
/*******************************************************************/
static struct stringRouter *
FindStringRouter (Environment * theEnv, const char *name)
{
  struct stringRouter *head;
...
/*********************************************/
bool
OpenString_BuilderDestination (Environment * theEnv,
			       const char *name, String_Builder * theSB)
{
  String_BuilderRouter *newStringRouter;
  char *theName;

  if (FindString_BuilderRouter (theEnv, name) != NULL)
    return false;
  ...
  newStringRouter->next =
    StringRouterData (theEnv)->ListOfString_BuilderRouters;
...
/*****************************************/
bool
CloseString_BuilderDestination (Environment * theEnv, const char *name)
{
...
/**********************************************/
static struct string_BuilderRouter *
FindString_BuilderRouter (Environment * theEnv, const char *name)
{
...
/*********************************************/
static bool
QueryString_BuilderCallback (Environment * theEnv,
			     const char *logicalName, void *context)
{
  String_BuilderRouter *head;
...
/*********************************************/
static void
CL_WriteString_BuilderCallback (Environment * theEnv,
				const char *logicalName,
				const char *str, void *context)
{
  String_BuilderRouter *head;
