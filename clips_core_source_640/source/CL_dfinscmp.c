...
static bool DefinstancesToCode (Environment *, const char *, const char *,
				char *, unsigned int, FILE *, unsigned int,
				unsigned int);
static void CloseDefinstancesFiles (Environment *, FILE *, FILE *,
				    unsigned int);
static void CL_DefinstancesModuleToCode (Environment *, FILE *, Defmodule *,
					 unsigned int, unsigned int);
static void SingleDefinstancesToCode (Environment *, FILE *, Definstances *,
				      unsigned int, unsigned int,
				      unsigned int);
static void InitDefinstancesCode (Environment *, FILE *, unsigned int,
				  unsigned int);
 ...
 ***************************************************/
void
CL_SetupDefinstancesCompiler (Environment * theEnv)
{
  DefinstancesData (theEnv)->DefinstancesCodeItem =
    CL_AddCodeGeneratorItem (theEnv, "definstances", 0,
			     ReadyDefinstancesForCode, InitDefinstancesCode,
			     DefinstancesToCode, 2);
}
 ...
 ****************************************************/
void
CL_DefinstancesCModuleReference (Environment * theEnv,
				 FILE * theFile,
				 unsigned long count,
...
/***************************************************/
static void
InitDefinstancesCode (Environment * theEnv,
		      FILE * initFP,
 ...
 *******************************************************/
static bool
DefinstancesToCode (Environment * theEnv,
		    const char *fileName,
		    const char *pathName,
		    char *fileNameBuffer,
		    unsigned int fileID,
		    ...
		    unsigned int imageID, unsigned int maxIndices)
{
  ...
  while (theModule != NULL)
	...
	CL_OpenFileIfNeeded (theEnv, moduleFile, fileName, pathName,
      ...
      if (moduleFile == NULL)
	{
	  CloseDefinstancesFiles (theEnv, moduleFile, definstancesFile,
				  maxIndices);
	  return false;
	}

      CL_DefinstancesModuleToCode (theEnv, moduleFile, theModule, imageID,
				   maxIndices);
      moduleFile =
	...
	{
	  definstancesFile =
	    CL_OpenFileIfNeeded (theEnv, definstancesFile, fileName, pathName,
				 fileNameBuffer, fileID, imageID, &fileCount,
	  ...
	  SingleDefinstancesToCode (theEnv, definstancesFile, theDefinstances,
				    imageID, maxIndices, moduleCount);
	  definstancesArrayCount++;
	  definstancesFile =
	    CL_CloseFileIfNeeded (theEnv, definstancesFile,
				  &definstancesArrayCount,
				  &definstancesArrayVersion, maxIndices, NULL,
				  NULL);
 ...
 ***************************************************/
static void
CloseDefinstancesFiles (Environment * theEnv,
			FILE * moduleFile,
			FILE * definstancesFile, unsigned int maxIndices)
{
 ...
 ***************************************************/
static void
CL_DefinstancesModuleToCode (Environment * theEnv,
			     FILE * theFile,
			     Defmodule * theModule,
			     unsigned int imageID, unsigned int maxIndices)
{
 ...
 ***************************************************/
static void
SingleDefinstancesToCode (Environment * theEnv,
			  FILE * theFile,
			  Definstances * theDefinstances,
			  unsigned int imageID,
