...
static bool ConstraintCompare (struct constraintRecord *,
...
/*****************************************************/
void
CL_InitializeConstraints (Environment * theEnv)
{
#if (! RUN_TIME) && (! BLOAD_ONLY)
  int i;
#endif

  CL_AllocateEnvironmentData (theEnv, CONSTRAINT_DATA,
			      sizeof (struct constraintData),
			      DeallocateConstraintData);
  ...
  if (ConstraintData (theEnv)->ConstraintHashtable == NULL)
    CL_ExitRouter (theEnv, EXIT_FAILURE);

  for (i = 0; i < SIZE_CONSTRAINT_HASH; i++)
    ConstraintData (theEnv)->ConstraintHashtable[i] = NULL;
#endif

#if (! RUN_TIME)
  CL_AddUDF (theEnv, "get-dynamic-constraint-checking", "b", 0, 0, NULL,
	     CL_GDCCommand, "CL_GDCCommand", NULL);
  CL_AddUDF (theEnv, "set-dynamic-constraint-checking", "b", 1, 1, NULL,
	     CL_SDCCommand, "CL_SDCCommand", NULL);
#endif
}
...
/*****************************************************/
static void
DeallocateConstraintData (Environment * theEnv)
{
#if ! RUN_TIME
...
/*************************************************************/
static void
ReturnConstraintRecord (Environment * theEnv, CONSTRAINT_RECORD * constraints)
{
  if (constraints == NULL)
    return;
...
/***************************************************/
static void
DeinstallConstraintRecord (Environment * theEnv,
...
/******************************************/
void
CL_RemoveConstraint (Environment * theEnv,
		     struct constraintRecord *theConstraint)
{
  ...
  tmpPtr =
    ConstraintData (theEnv)->ConstraintHashtable[theConstraint->bucket];
  while (tmpPtr != NULL)
    {
...
/***********************************/
unsigned long
CL_HashConstraint (struct constraintRecord *theConstraint)
{
  unsigned short i = 0;
  ...
  for (tmpPtr = theConstraint->classList; tmpPtr != NULL;
       tmpPtr = tmpPtr->nextArg)
    {
      count += CL_GetAtomicHashValue (tmpPtr->type, tmpPtr->value, i++);
    }

  for (tmpPtr = theConstraint->restrictionList; tmpPtr != NULL;
       tmpPtr = tmpPtr->nextArg)
    {
      count += CL_GetAtomicHashValue (tmpPtr->type, tmpPtr->value, i++);
    }

  for (tmpPtr = theConstraint->minValue; tmpPtr != NULL;
       tmpPtr = tmpPtr->nextArg)
    {
      count += CL_GetAtomicHashValue (tmpPtr->type, tmpPtr->value, i++);
    }

  for (tmpPtr = theConstraint->maxValue; tmpPtr != NULL;
       tmpPtr = tmpPtr->nextArg)
    {
      count += CL_GetAtomicHashValue (tmpPtr->type, tmpPtr->value, i++);
    }

  for (tmpPtr = theConstraint->minFields; tmpPtr != NULL;
       tmpPtr = tmpPtr->nextArg)
    {
      count += CL_GetAtomicHashValue (tmpPtr->type, tmpPtr->value, i++);
    }

  for (tmpPtr = theConstraint->maxFields; tmpPtr != NULL;
       tmpPtr = tmpPtr->nextArg)
    {
      count += CL_GetAtomicHashValue (tmpPtr->type, tmpPtr->value, i++);
    }
...
/**********************************************/
static bool
ConstraintCompare (struct constraintRecord *constraint1,
		   struct constraintRecord *constraint2)
{
  ...
  if ((constraint1->anyAllowed != constraint2->anyAllowed) ||
      ...
      || (constraint1->instanceAddressesAllowed !=
	  constraint2->instanceAddressesAllowed)
      || (constraint1->externalAddressesAllowed !=
	  constraint2->externalAddressesAllowed)
      || (constraint1->voidAllowed != constraint2->voidAllowed)
      || (constraint1->multifieldsAllowed != constraint2->multifieldsAllowed)
      || (constraint1->singlefieldsAllowed !=
	  constraint2->singlefieldsAllowed)
      || (constraint1->factAddressesAllowed !=
	  constraint2->factAddressesAllowed)
      ...
      || (constraint1->instanceNameRestriction !=
	  constraint2->instanceNameRestriction))
    {
      return false;
    }
  ...
  for (tmpPtr1 = constraint1->classList, tmpPtr2 = constraint2->classList;
    ...
    {
      if ((tmpPtr1->type != tmpPtr2->type)
	  || (tmpPtr1->value != tmpPtr2->value))
	{
	  return false;
	}
    }
  ...
  for (tmpPtr1 = constraint1->restrictionList, tmpPtr2 =
       constraint2->restrictionList; (tmpPtr1 != NULL) && (tmpPtr2 != NULL);
       tmpPtr1 = tmpPtr1->nextArg, tmpPtr2 = tmpPtr2->nextArg)
    {
      if ((tmpPtr1->type != tmpPtr2->type)
	  || (tmpPtr1->value != tmpPtr2->value))
  ...
  if (((constraint1->multifield == NULL) && (constraint2->multifield != NULL))
      || ((constraint1->multifield != NULL)
	  && (constraint2->multifield == NULL)))
    {
      return false;
    }
  ...
  return (ConstraintCompare
	  (constraint1->multifield, constraint2->multifield));
}
...
/************************************/
struct constraintRecord *
CL_AddConstraint (Environment * theEnv,
		  struct constraintRecord *theConstraint)
{
  ...
  for (tmpPtr = ConstraintData (theEnv)->ConstraintHashtable[hashValue];
       tmpPtr != NULL; tmpPtr = tmpPtr->next)
    {
  ...
  theConstraint->next =
    ConstraintData (theEnv)->ConstraintHashtable[hashValue];
...
/*************************************************/
static void
InstallConstraintRecord (Environment * theEnv,
			 CONSTRAINT_RECORD * constraints)
{
...
/**********************************************/
void
CL_SDCCommand (Environment * theEnv,
	       UDFContext * context, UDFValue * returnValue)
{
  UDFValue theArg;

  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_GetDynamicConstraintChecking (theEnv));
  ...
  CL_SetDynamicConstraintChecking (theEnv,
				   theArg.value != FalseSymbol (theEnv));
}
...
/**********************************************/
void
CL_GDCCommand (Environment * theEnv,
	       UDFContext * context, UDFValue * returnValue)
{
  returnValue->lexemeValue =
    CL_CreateBoolean (theEnv, CL_GetDynamicConstraintChecking (theEnv));
...
/******************************************************/
bool
CL_SetDynamicConstraintChecking (Environment * theEnv, bool value)
{
  bool ov;
...
/******************************************************/
bool
CL_GetDynamicConstraintChecking (Environment * theEnv)
{
